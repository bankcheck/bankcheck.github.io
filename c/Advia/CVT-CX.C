...
int ConvertFlag(char *code)
{
    ...
    fp = fopen(g_szFlagFile, "rb");
    if (fp == NULL) {
        WriteLog("Can't open file %s", g_szFlagFile);
        return 0;
    }
...
/*int ConvertAdviaCode(char *fname, int seq, char *patient, char *order)
{
    FILE *fp;
