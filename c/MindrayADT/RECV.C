...
//	first = NULL;
	...
	while(1) {
		...
		//WriteLog("%d: Thread %d running", no, g_client[no].dThreadID);
...
/*Change to db
							if (g_client[no].out == NULL) {
								sprintf(tmpfname, "%sADT%d", g_szTempPath, no);
...
/*Change to db
							...
							if (bLast) {
								...
								if (tp != NULL) {
									...
									sprintf(fname, "%sADT_%s_%04d%02d%02d%02d%02d%02d", g_szOutBoxPath, g_client[no].ip, tp->tm_year+1900, tp->tm_mon+1, tp->tm_mday,
										tp->tm_hour, tp->tm_min, tp->tm_sec);

								} else {

									sprintf(fname, "%sADT_%s_%lx", g_szOutBoxPath, g_client[no].ip, GetTickCount());
...
//Send
			if (first == NULL) {
				...
				sprintf(path, "%sADT_%s_*", g_szInBoxPath, g_client[no].ip);
