...
// Execute statement with parameters
    ...
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
		...
		dbdisconnect();
		dbconnect();
		WriteLog("SQL FAILED: CALL HL7_RECV('OMNICELL','RXC',%s,%s,null,out);", g_client[no].ip, data);
	}
...
void reply(long refid, int no)
{
	...
	SQLCHAR SQL[300] =  "select h.message_id, m.message from hl7_header h, hl7_message m where h.message_id = m.message_id and h.recv_app = 'OMNICELL' and event = 'RXC' and client = ? and refid = ? order by m.seq_no";
...
//Update status
	sprintf(SQL, "update hl7_header set status = 'D' where message_id = ? and recv_app = 'OMNICELL' and event = 'RXC' and client = ? ");
