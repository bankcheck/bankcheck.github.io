...
DWORD RXFThreadProc(LPVOID pParm)
{
...
_TRY_EXCEPTION_BLOCK_BEGIN()
    ...
    while (1) {
		...
		sprintf(path, "%sRXF_*", g_szRXFInBoxPath);
...
//
		...
		while (first != NULL) {
			...
			sprintf(fname, "%s%s", g_szRXFInBoxPath, first->name);
			//sprintf(bakname, "%sRXF\\%s", g_szBackupInBoxPath, first->name);
...
//get response
			...
			retry = 0;
			while (retry < g_nMaxRetry) {
				if ((recv_size = recv(g_RXF.s, server_reply, 2000, 0)) != SOCKET_ERROR) {
					...
					WriteLog("RECV %s: %s", g_RXF.name, server_reply);
...
//Backup and remove message file
			if (bSuccess || (message_retry >= g_nMaxRetry)) {
				...
				if (strcmp(message_type, "ACK") == 0) {
					...
					sprintf(tmpname, "%sRXF__%s", g_szADTInBoxPath, message_id);
					sprintf(bakname, "%sRXF__%s", g_szBackupInBoxPath, message_id);
