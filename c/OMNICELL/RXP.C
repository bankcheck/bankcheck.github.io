...
DWORD RXPThreadProc(LPVOID pParm)
{
...
_TRY_EXCEPTION_BLOCK_BEGIN()
    ...
    while (1) {
		...
		//sprintf(path, "%s*", g_szRXPInBoxPath);
		sprintf(path, "%sRXP_*", g_szRXPInBoxPath);
...
//
		...
		while (first != NULL) {
			...
			sprintf(fname, "%s%s", g_szRXPInBoxPath, first->name);
			//sprintf(bakname, "%sRXP\\%s", g_szBackupInBoxPath, first->name);
...
//get response
			...
			retry = 0;
			while (retry < g_nMaxRetry) {
				if ((recv_size = recv(g_RXP.s, server_reply, 2000, 0)) != SOCKET_ERROR) {
					...
					WriteLog("RECV %s: %s", g_RXP.name, server_reply);
...
//Backup and remove message file
			if (bSuccess || (message_retry >= g_nMaxRetry)) {
				...
				if (strcmp(message_type, "ACK") == 0) {
					...
					//sprintf(tmpname, "%s_%s", g_szADTInBoxPath, message_id);
					//sprintf(bakname, "%sRXP\\_%s", g_szBackupInBoxPath, message_id);
					sprintf(tmpname, "%sRXP__%s", g_szADTInBoxPath, message_id);
					sprintf(bakname, "%sRXP__%s", g_szBackupInBoxPath, message_id);
