...
_TRY_EXCEPTION_BLOCK_BEGIN()
    ...
    no = (int)pParm;
	setsockopt(g_client[no].s, SOL_SOCKET, SO_RCVTIMEO, (char *)&g_timeout, sizeof(int)); //setting the receive timeout
	//WriteLog("RECV Thread: no=%d", no);
	//WriteLog("RECV Thread: ip=%s", g_client[no].ip);

	threadID = g_client[no].tick;

	while(threadID == g_client[no].tick) {
		if (g_bQuit || WaitForSingleObject(g_hServerStopEvent, g_dwSleep) == WAIT_OBJECT_0)
			break;

		if (threadID != g_client[no].tick) {
			break;
		}
			...
			if ((recv_size = recv(g_client[no].s, client_message, 2000, 0)) != SOCKET_ERROR) {
				...
				if (client_message[0] == ACK) {
					...
					while (fgets(buf, 1024, fp) != NULL) {
						...
						sprintf(databuf, "%c%d%s%c\0", STX, frame, buf, ETX);

						sprintf(packet, "%s%02X%c%c\0", databuf, GetCheckSum(databuf), CR, LF);
