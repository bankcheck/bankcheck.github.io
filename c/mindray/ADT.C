...
//Send
		...
		sprintf(path, "%s*", g_szADTInBoxPath);
...
//Backup and remove message file
			if (bSuccess || (message_retry >= g_nMaxRetry)) {
				...
				if (strcmp(message_type, "ACK") == 0) {
					...
					sprintf(tmpname, "%s%s", g_szADTInBoxPath, message_id);
					sprintf(bakname, "%sADT\\%s", g_szBackupInBoxPath, message_id);
