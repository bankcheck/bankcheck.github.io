...
DWORD EMRThreadProc(LPVOID pParm)
{
...
//Send
		...
		sprintf(path, "%s*", g_szEMRInBoxPath);
...
//
		...
		while (first != NULL) {
			...
			sprintf(fname, "%s%s", g_szEMRInBoxPath, first->name);
...
//get response
			...
			retry = 0;
			while (retry < g_nMaxRetry) {
				if ((recv_size = recv(g_EMR.s, server_reply, 2000, 0)) != SOCKET_ERROR) {
					...
					WriteLog("RECV %s: %s", g_EMR.name, server_reply);
...
//Backup and remove message file
			if (bSuccess || (message_retry >= g_nMaxRetry)) {
				...
				if (strcmp(message_type, "ACK") == 0) {
					...
					sprintf(tmpname, "%s%s", g_szEMRInBoxPath, message_id);
					sprintf(bakname, "%sEMR\\%s", g_szBackupInBoxPath, message_id);
