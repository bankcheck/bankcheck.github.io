...
char        g_szPrgName[80] = "HKAH C8000 Server1";
...
char        g_szInBoxPath[260] = "C:\\MOXA\\c8000s1\\INBOX\\";
char        g_szLogPath[260] = "C:\\MOXA\\c8000s1\\LOG\\";
char        g_szBackupInBoxPath[260] = "C:\\MOXA\\c8000s1\\INBOX.BAK\\";
char        g_szOutBoxPath[260] = "C:\\MOXA\\c8000s1\\OUTBOX\\";
char        g_szCodeFile[260] = "D:\\MOXA\\c8000s1\\CODE.DAT";
char        g_szTempPath[260] = "D:\\MOXA\\c8000s1\\TEMP\\";
    ...
    while (1) {
		...
		for (i = 0; i < SOMAXCONN; i++) {
			if (strcmp(g_client[i].ip, ip) == 0) {
				...
				if (CloseHandle(g_client[i].hThread) == 0) {
					AddToEventLog(EVENTLOG_ERROR_TYPE, "Can't close thread.");
				}

				strcpy(g_client[i].ip, ip);
				
				g_client[i].s = temp;
				setsockopt(g_client[i].s, SOL_SOCKET, SO_RCVTIMEO, (char *)&g_timeout, sizeof(int)); //setting the receive timeout
				WriteLog("Socket %d accepted, ip=%s", i, g_client[i].ip);				
		...
		if (!found) {
			...
			if (CloseHandle(g_client[curr].hThread) == 0) {
				AddToEventLog(EVENTLOG_ERROR_TYPE, "Can't close thread.");
			}
