...
int bFindProp(xmlNodePtr curNode, char *prop, char *value)
{
	xmlAttrPtr attrPtr;
	xmlChar* szAttr;
	if (!xmlHasProp(curNode,BAD_CAST prop)) 
		return 0;
	else {
		attrPtr = curNode->properties;
		while (attrPtr != NULL) {
			if (!xmlStrcmp(attrPtr->name, BAD_CAST prop)) {
				szAttr = xmlGetProp(curNode, BAD_CAST prop);
				strcpy(value, szAttr);
			}
...
xmlNodePtr FindNode(xmlNodePtr startNode, char *name) {
	xmlNodePtr curNode = startNode;
	while (curNode != NULL) {
