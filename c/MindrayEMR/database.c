...
void reply(long refid, int no)
{
	...
	SQLCHAR SQL[300] =  "select h.message_id, m.message from hl7_header h, hl7_message m where h.message_id = m.message_id and h.recv_app = 'MINDRAY' and event = 'EMR' and client = ? and refid = ? and status = 'I' order by m.seq_no";
...
//Update status
	sprintf(SQL, "update hl7_header set status = 'D' where message_id = ? and recv_app = 'MINDRAY' and event = 'EMR' and client = ? ");
