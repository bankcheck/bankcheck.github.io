...
//
		...
		while (first != NULL) {
			...
			sprintf(fname, "%s", first->name);
			sprintf(filename, "%s%s", g_szInBoxPath, first->name);

			WriteLog("Read file: %s", filename);

			fp = fopen(filename, "rt");
			if (fp == NULL) {
				WriteLog("Can't open file: %s", filename);
				first = first->next;
				continue;
			}
			...
			while (fgets(buf, 1024, fp) != NULL) {
				...
				while (retry < g_nMaxRetry) {
					if (SendPacket(&g_client, message) == 0)
						break;
...
//get response
			...
			while (retry < g_nMaxRetry) {
				if ((recv_size = recv(g_client.s, server_reply, 2000, 0)) != SOCKET_ERROR) {
					...
					WriteLog("RECV %s: %s", g_client.name, server_reply);
...
//Backup and remove message file
			if (bSuccess || (message_retry >= g_nMaxRetry)) {
				...
				if (strcmp(message_type, "ACK") == 0) {
					...
					sprintf(bakname, "%s%s", g_szBackupInBoxPath, fname);
										
					CopyFile(filename, bakname, FALSE);
					WriteLog("BACKUP: %s -> %s", filename, bakname);
