...
char        g_szProgID[40] = "alinity";
char        g_szPrgName[80] = "HKAH Sysmex Server";
...
char        g_szCodeFile[260] = "C:\\MOXA\\sysmex\\CODE.DAT";
char        g_szRateFile[260] = "C:\\MOXA\\sysmex\\RATE.DAT";
char        g_szTempPath[260] = "C:\\MOXA\\sysmex\\TEMP\\";
DWORD       g_dwSleep = 1000;
int			g_heartbeat = 60000;
OVERLAPPED  g_o;
short		g_nPort = 5001;
short		g_nMaxRetry = 3;
//short		g_nMsgID;
...
//init buffer file
	for (i = 0; i < SOMAXCONN; i++) {
		sprintf(g_client[i].fname, "%sbuf%d.dat", g_szTempPath, i);
		WriteLog("%d:Delete: %s", i, g_client[i].fname);
	}
