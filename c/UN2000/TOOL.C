...
int getDecimal(char *code)
{
    ...
    fp = fopen(g_szDecFile, "rt");
    if (fp == NULL) {
        WriteLog("Can't open file %s", g_szDecFile);
        return -1;
    }
...
int isResult(char *fmt, char *inst)
{
    char buf[1024];
    FILE *fp;
    char *p;
	char ResultFormat[11];

   // strcpy(code, item);

    fp = fopen(g_szFormatFile, "rb");
    if (fp != NULL) {

		while (1) {
			if (fgets(buf, 1024, fp) == NULL) {
				strcpy(ResultFormat, g_ResultFormat);
				break;
			}
			...
			if (strcmp(inst, p) == 0) {
				p = strtok(NULL, " \t=,\r\n");
				if (p == NULL) {
					WriteLog("Error setting for %s", inst);
					continue;
				}

				strcpy(ResultFormat, p);
				//p = strtok(NULL, " \t=,\r\n");
				fclose(fp);
				break;
			}
