		...
		for (i=2; i<245; i++) {
			if (buf[i] == STX) {
				...
				if (function == 'O') {
					...
					//Background
					if (cmd[0] == '\0') {
						WriteLog("Background data");
						//sprintf(cmd, "BACKGRND\0");
						bProcess = 0;
					} else {
						p = strtok(cmd, "^ \"*/:<>?\\|");
						strcpy(LabNo, p);
				...
				if ((function == 'C') && (bProcess)) {
					ParseDelimiter(token, 4, '|', cmd);
					p = strtok(cmd, "^");
						...
						if (strcmp(result, "YES") == 0) {
							...
							if (out == NULL) {
								...
								if (tp != NULL)
									...
									sprintf(DateTime, "20000101000000\0");
