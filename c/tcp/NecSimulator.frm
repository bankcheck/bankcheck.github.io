...
Private Sub Command1_Click()
    If (Winsock1.State = sckConnected) Then
        Winsock1.SendData (Trim(Me.txtSend.Text))
        ShowData List1, "T>" & Me.txtSend.Text, Me
        txtSend.Text = ""
    Else
        'ShowData List1, CStr(Winsock1.State), Me
    End If
...
Private Sub Form_Load()
    ...
    sLocalPort = RetrieveFileValue("config", "LocalPort", "4", App.Path & "\Simulator.ini")
    sRemotePort = RetrieveFileValue("config", "RemotePort", "4", App.Path & "\Simulator.ini")
    sIP = RetrieveFileValue("config", "RemoteIP", "127.0.0.1", App.Path & "\Simulator.ini")
    sServer = RetrieveFileValue("config", "Server", "0", App.Path & "\Simulator.ini")

    iMax = 0

    If val(sServer) Then
        Winsock1(0).LocalPort = val(sLocalPort)
        Winsock1(0).Listen
        ShowData List1, "Port:" & sLocalPort & " Listening", Me
    
    Else
        Winsock1(0).RemoteHost = sIP
        Winsock1(0).LocalPort = val(sLocalPort)
        Winsock1(0).RemotePort = val(sRemotePort)
        Winsock1(0).Connect
        'sIP, val(sRemotePort)
    End If
...
Private Sub Winsock1_Connect(Index As Integer)
    ShowData List1, sIP & ":" & sRemotePort & " Connected", Me
End Sub

Private Sub Winsock1_ConnectionRequest(Index As Integer, ByVal requestID As Long)
    ShowData List1, "Incoming Connection - " & Winsock1.RemoteHostIP, Me
    If Index = 0 Then
        iMax = iMax + 1
        Load Winsock1(iMax)
        Winsock1(iMax).LocalPort = 0
        Winsock1(iMax).Accept requestID
    End If
