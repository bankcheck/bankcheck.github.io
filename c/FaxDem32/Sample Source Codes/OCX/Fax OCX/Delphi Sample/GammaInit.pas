...
type
  TGammaLinkInit = class(TForm)
    ...
    procedure TestChannelClick(Sender: TObject);
  private
...
procedure TGammaLinkInit.TestChannelClick(Sender: TObject);
var error:longint;
begin
     Screen.Cursor:=-11;
     error:=MainForm.FAX1.TestModem(GammaChannelsListBox.Items[GammaChannelsListBox.ItemIndex]);
     Screen.Cursor:=-2;
     if (error=0) then GChannelTypeLabel.Caption:=MainForm.FAX1.Model
       else GChannelTypeLabel.Caption:='Error code: '+IntToStr(error);
end;
