...
type
  TMainForm = class(TForm)
    ...
    Init1: TMenuItem;
    Comm1: TMenuItem;
    Brooktroutchannel1: TMenuItem;
    Gammalinkchannel1: TMenuItem;
    Send1: TMenuItem;
    Close1: TMenuItem;
    About1: TMenuItem;
    AboutOCXdemo1: TMenuItem;
    SavePictureDialog: TSaveDialog;
    Dialogicchannel1: TMenuItem;
    NMSchannel1: TMenuItem;
    EventListBox: TListBox;
    ShowFaxManager1: TMenuItem;
    Clear1: TMenuItem;
    N1: TMenuItem;
    FAX1: TFAX;
    ...
    procedure CLickGammaLinkChannel(Sender: TObject);
    procedure Brooktroutchannel1Click(Sender: TObject);
    procedure OnActivate(Sender: TObject);
    procedure ClickDialogicChannel(Sender: TObject);
    procedure OnNMSChannel(Sender: TObject);
    procedure OnEndPage(Sender: TObject; const szPort: WideString;
      lPageNumber: Integer);
    procedure OnEndReceive(Sender: TObject; const PortName: WideString;
    ...
    procedure FAX1RemoteID(Sender: TObject; const Port,
      RemoteID: WideString);
    procedure FAX1Ring(Sender: TObject; const PortName: WideString);
    procedure FAX1StartPage(Sender: TObject; const szPort: WideString;
      lPageNumber: Integer);
    procedure FAX1StartReceive(Sender: TObject;
    ...
    procedure ShowFaxManager1Click(Sender: TObject);
...
public
    BaudRate:shortint;
    SpeakerMode:shortint;
    SpeakerVolume:shortint;
    DialMode:BOOL;
    EnableECM:shortint;
    EnableBFT: shortint;
    EnableDebug:BOOL;
    ActualFaxPort:string;
end;
const
...
uses OpenDlg, SendDlg, AboutDlg, CloseDlg, FaxDlg, GammaInit, BrookTrout,
  dlginit, NMSInit;
...
procedure TMainForm.CLickGammaLinkChannel(Sender: TObject);
var dlg:TGammaLinkInit;
begin
        dlg:=TGammaLinkInit.Create(self);
        dlg.ShowModal;
        dlg.free;
end;

procedure TMainForm.Brooktroutchannel1Click(Sender: TObject);
var dlg:TBrooktroutInit;
begin
        dlg:=TBrooktroutInit.Create(self);
     dlg.ShowModal;
     dlg.free;
end;
...
begin
    ...
    if Length(FAX1.AvailableGammaChannels)>0 then
        GammaLinkChannel1.Enabled:=True
    else
        GammaLinkChannel1.Enabled:=False;

    if Length(FAX1.AvailableBrooktroutChannels)>0 then
        BrooktroutChannel1.Enabled:=True
    else
        BrooktroutChannel1.Enabled:=False;

    if Length(FAX1.AvailableDialogicChannels)>0 then
        DialogicChannel1.Enabled:=True
    else
        DialogicChannel1.Enabled:=False;

    if Length(FAX1.AvailableNMSChannels)>0 then
        NMSChannel1.Enabled:=True
    else
        NMSChannel1.Enabled:=False;                            
    EventListBox.Items.Add('This sample will show how to send faxes with different priority. The sample will send ten faxes.');
    EventListBox.Items.Add('Each fax is created from a randomly loaded page from the "Testqueue.tif" file.');
    EventListBox.Items.Add('The priority level of the fax is set based on the random page number loaded TIFF file.');
    EventListBox.Items.Add('The fax objects are added to the queue. Higher priority faxes will be sent before lower priority faxes.');
    EventListBox.Items.Add('At the receiving end, the faxes should be received in priority order (higher first),');
    EventListBox.Items.Add('regardless of the random order the fax was put on the queue. To use the sample');
   EventListBox.Items.Add('open a COM port or fax channel than start sending the faxes using the "Send" menu item.');


end;

procedure TMainForm.ClickDialogicChannel(Sender: TObject);
var dlg:TDialogic;
begin
        dlg:=TDialogic.Create(self);
        dlg.ShowModal;
        dlg.free;
end;

procedure TMainForm.OnNMSChannel(Sender: TObject);
var dlg:TNMS;
begin
        dlg:=TNMS.Create(self);
        dlg.ShowModal;
        dlg.free;
end;
...
begin
    ...
    MainForm.EventListBox.Items.Add(line);
     ...
     {NrOfReceivedPgs:=NrOfReceivedPgs+1;}
end;


procedure TMainForm.OnEndReceive(Sender: TObject;
  const PortName: WideString; FaxID: Integer; const RemoteID: WideString);
var
   Extension:string[3];
   FileType,i:shortint;
   NrOfReceivedPgs:byte;
   temp:byte;
   line:string;
   error:integer;
begin
//if (PortName[1]='G')or(PortName[1]='C')or(PortName[1]='N')then NrOfReceivedPgs:=NrOfReceivedPgs-1;
    ...
    begin
         ...
         begin
              FAX1.FaxFileName:=SavePictureDialog.FileName;
              Extension:=copy(FAX1.FaxFileName,Length(FAX1.FaxFileName)-2,3);
              if UpperCase(Extension)='BMP' then FileType:=PAGE_FILE_BMP;
              if UpperCase(Extension)='TIF' then FileType:=PAGE_FILE_TIFF_G31D;

              NrOfReceivedPgs:=FAX1.GetFaxParam(FaxID,FP_PAGENUM);
              if (MainForm.FAX1.IsDemoVersion()) and (NrOfReceivedPgs>1) then
              begin
                   MessageDlg('The DEMO version of the Fax OCX supports only 1 page faxes. ONLY the first page will be saved.',
                                  mtWarning,[mbOK],0);
                   NrOfReceivedPgs:=1;
              end;
              if FileType<>0  then
              begin
                   For i:=1 to NrOfReceivedPgs do
                   begin
                        error := FAX1.GetFaxPage( FaxID, i, FileType, TRUE );
                   end;
              end
              else
              begin
                   MessageDlg('Invalid Filename!',mtWarning,[mbOK],0);
              end;
         end;

         MessageDlg('Number of received pages is:'+IntToStr(NrOfReceivedPgs),
                        mtWarning,[mbOK],0);

         if FAX1.ClearFaxObject(FaxID)<>0 then
         begin
              line := '  -  Error deleting faxobject! ID = ' + IntToStr(FaxID);
              MainForm.EventListBox.Items.Add(line);
         end;
    end;
...
begin
     ...
     if (portType='G')and(lStatus<>0) then
             MessageDlg('GammaLink error : Error code:'+IntToStr(lStatus),mtWarning,[mbOK],0);
    if Length(szDTMF)>0 then
             MessageDlg('DTMF digits:'+szDTMF,mtWarning,[mbOK],0);
end;
...
procedure TMainForm.FAX1RemoteID(Sender: TObject; const Port,
  RemoteID: WideString);
  var line:String;
begin
...
procedure TMainForm.FAX1Ring(Sender: TObject; const PortName: WideString);
    var line:String;
begin
...
procedure TMainForm.FAX1StartReceive(Sender: TObject;
  const PortName: WideString);
  var line:String;
begin
...
var dlg:TFaxConfigDlg;
begin
    dlg:=TFaxConfigDlg.Create(self);
    dlg.ShowModal;
    dlg.free;
end;

procedure TMainForm.ShowFaxManager1Click(Sender: TObject);
begin
    FAX1. DisplayFaxManager := Not FAX1.DisplayFaxManager;

 if FAX1.DisplayFaxManager then
    MainForm.ShowFaxManager1.Checked := True
 else
    MainForm.ShowFaxManager1.Checked := False;
