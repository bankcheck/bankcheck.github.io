// OpenFaxBoardsDlg.cpp : implementation file
//

#include "stdafx.h"
#include "OpenFaxBoards.h"
#include "OpenFaxBoardsDlg.h"
#include "commcl.h"
#include "OpenDialogic.h"
#include "DlgFaxClose.h"
#include "BOpen.h"
#include "DlgNms.h"
#include "DlgGOpen.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

/////////////////////////////////////////////////////////////////////////////
// CAboutDlg dialog used for App About

class CAboutDlg : public CDialog
{
public:
	CAboutDlg();

// Dialog Data
	//{{AFX_DATA(CAboutDlg)
	enum { IDD = IDD_ABOUTBOX };
	//}}AFX_DATA

	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CAboutDlg)
	protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
	//}}AFX_VIRTUAL

// Implementation
protected:
	//{{AFX_MSG(CAboutDlg)
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};

CAboutDlg::CAboutDlg() : CDialog(CAboutDlg::IDD)
{
	//{{AFX_DATA_INIT(CAboutDlg)
	//}}AFX_DATA_INIT
}

void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CAboutDlg)
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CAboutDlg, CDialog)
	//{{AFX_MSG_MAP(CAboutDlg)
		// No message handlers
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// COpenFaxBoardsDlg dialog

COpenFaxBoardsDlg::COpenFaxBoardsDlg(CWnd* pParent /*=NULL*/)
	: CDialog(COpenFaxBoardsDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(COpenFaxBoardsDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
	// Note that LoadIcon does not require a subsequent DestroyIcon in Win32
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void COpenFaxBoardsDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(COpenFaxBoardsDlg)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(COpenFaxBoardsDlg, CDialog)
	//{{AFX_MSG_MAP(COpenFaxBoardsDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BROOKTROUT, OnBrooktrout)
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_DIALOGIC, OnDialogic)
	ON_BN_CLICKED(IDC_GAMMALINK, OnGammalink)
	ON_BN_CLICKED(IDC_NMS, OnNms)
	ON_BN_CLICKED(IDC_CLOSE, OnClose)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// COpenFaxBoardsDlg message handlers

BOOL COpenFaxBoardsDlg::OnInitDialog()
{
	CDialog::OnInitDialog();

	// Add "About..." menu item to system menu.

	// IDM_ABOUTBOX must be in the system command range.
	ASSERT((IDM_ABOUTBOX & 0xFFF0) == IDM_ABOUTBOX);
	ASSERT(IDM_ABOUTBOX < 0xF000);

	CMenu* pSysMenu = GetSystemMenu(FALSE);
	if (pSysMenu != NULL)
	{
		CString strAboutMenu;
		strAboutMenu.LoadString(IDS_ABOUTBOX);
		if (!strAboutMenu.IsEmpty())
		{
			pSysMenu->AppendMenu(MF_SEPARATOR);
			pSysMenu->AppendMenu(MF_STRING, IDM_ABOUTBOX, strAboutMenu);
		}
	}

	// Set the icon for this dialog.  The framework does this automatically
	//  when the application's main window is not a dialog
	SetIcon(m_hIcon, TRUE);			// Set big icon
	SetIcon(m_hIcon, FALSE);		// Set small icon
	
	// TODO: Add extra initialization here
	
	return TRUE;  // return TRUE  unless you set the focus to a control
}

void COpenFaxBoardsDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	if ((nID & 0xFFF0) == IDM_ABOUTBOX)
	{
		CAboutDlg dlgAbout;
		dlgAbout.DoModal();
	}
	else
	{
		CDialog::OnSysCommand(nID, lParam);
	}
}

// If you add a minimize button to your dialog, you will need the code below
//  to draw the icon.  For MFC applications using the document/view model,
//  this is automatically done for you by the framework.

void COpenFaxBoardsDlg::OnPaint() 
{
	if (IsIconic())
	{
		CPaintDC dc(this); // device context for painting

		SendMessage(WM_ICONERASEBKGND, (WPARAM) dc.GetSafeHdc(), 0);

		// Center icon in client rectangle
		int cxIcon = GetSystemMetrics(SM_CXICON);
		int cyIcon = GetSystemMetrics(SM_CYICON);
		CRect rect;
		GetClientRect(&rect);
		int x = (rect.Width() - cxIcon + 1) / 2;
		int y = (rect.Height() - cyIcon + 1) / 2;

		// Draw the icon
		dc.DrawIcon(x, y, m_hIcon);
	}
	else
	{
		CDialog::OnPaint();
	}
}

// The system calls this to obtain the cursor to display while the user drags
//  the minimized window.
HCURSOR COpenFaxBoardsDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

void COpenFaxBoardsDlg::OnBrooktrout() 
{
	CBOpen dlgBrook;
	COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();

	if ( pApp->BrooktroutChannels && pApp->BrooktroutChannels[0] )
		dlgBrook.DoModal();	
   	else AfxMessageBox("There is no available Brooktrout Channel!");

   
}

int COpenFaxBoardsDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	if (CDialog::OnCreate(lpCreateStruct) == -1)
		return -1;
	COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();
	if(SetupFaxDriver(NULL))
        AfxMessageBox("Setting fax driver has failed!");	
	pApp->m_nDialogicBoard = D_GetBoardNum();
	pApp->BrooktroutChannels = GetChannelInfo();
	pApp->m_nNmsBoard = NMS_GetBoardNum();
	pApp->m_nGammaChannel = G_GetChannelNum();
	
	return 0;
}

void COpenFaxBoardsDlg::OnDestroy() 
{
	CDialog::OnDestroy();
	
COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();

    for(int i=0;i<MAX_FAXPORTS;i++){
        if ( pApp->FaxPorts[i] ) {
            if ( pApp->FaxPorts[i]->IsOpen() ) {
                pApp->FaxPorts[i]->AbortFax();
            }
        }
    }
    
    EndOfFaxDriver(TRUE);	
}

void COpenFaxBoardsDlg::OnDialogic() 
{
	COpenDialogic dlgDialogic;
	COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();
	if ( pApp->m_nDialogicBoard )
		dlgDialogic.DoModal();	
	else AfxMessageBox("There is no available Dialogic Channel!");
}

void COpenFaxBoardsDlg::OnGammalink() 
{
	COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();
	CDlgGOpen dlgGamma;
	if ( pApp->m_nGammaChannel != 0 )
		dlgGamma.DoModal();	
	else AfxMessageBox("There is no available GammaLink Channel!");
}

void COpenFaxBoardsDlg::OnNms() 
{
	CDlgNms dlgNMS;
	COpenFaxBoardsApp *pApp = (COpenFaxBoardsApp *)AfxGetApp();
	if ( pApp->m_nNmsBoard )
		dlgNMS.DoModal();	
	else AfxMessageBox("There is no available NMS Channel!");
}

void COpenFaxBoardsDlg::OnClose() 
{
	CDlgFaxClose dlgClose;
	dlgClose.DoModal();	
}
