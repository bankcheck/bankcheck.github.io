// SCCallMonitorDlg.cpp : implementation file
//

#include "stdafx.h"
#include "SCCallMonitor.h"
#include "SCCallMonitorDlg.h"
...
CSCCallMonitorDlg::CSCCallMonitorDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CSCCallMonitorDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSCCallMonitorDlg)
	//}}AFX_DATA_INIT
...
void CSCCallMonitorDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSCCallMonitorDlg)
...
BEGIN_MESSAGE_MAP(CSCCallMonitorDlg, CDialog)
	//{{AFX_MSG_MAP(CSCCallMonitorDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
/////////////////////////////////////////////////////////////////////////////
// CSCCallMonitorDlg message handlers

BOOL CSCCallMonitorDlg::OnInitDialog()
{
...
void CSCCallMonitorDlg::CloseResources()
{
...
long CSCCallMonitorDlg::OnVoiceMsg(WPARAM event,LPARAM lParam)
{
BOOL bError = FALSE;
    switch(event)
    {
        ...
        case    MFX_VOICE_CONNECT:
            ...
            else if(m_n_States == STATE_ANSWERING_CALL2)
            {
                ...
                if(m_hVoiceHandle!=-1 ) {
                    mdm_SCBUS_ListenTo(m_hVoiceHandle, m_hLineHandle, SCBUS_SIMPLEX);
                    RecordMessage(m_hModemObj);
                }
                if(m_hVoiceHandle2!=-1 ) {
                    mdm_SCBUS_ListenTo(m_hVoiceHandle2, m_hLineHandle2, SCBUS_SIMPLEX);
                    RecordMessage(m_hModemObj2);
                }
                GetDlgItem(IDC_HANGUP2)->EnableWindow(TRUE);
            }
    }
...
void CSCCallMonitorDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CSCCallMonitorDlg::OnPaint() 
{
	if (IsIconic())
	{
...
HCURSOR CSCCallMonitorDlg::OnQueryDragIcon()
{
	return (HCURSOR) m_hIcon;
}

void CSCCallMonitorDlg::StatusMsg(LPCSTR msg,...)
{
    char    szTemp[1024];
    va_list pArg;
...
void CSCCallMonitorDlg::OnSelchangeLinetype() 
{
...
void CSCCallMonitorDlg::OnSelchangeLinetype2() 
{
...
bool CSCCallMonitorDlg::OpenCall1Resources()
{
...
bool CSCCallMonitorDlg::OpenCall2Resources()
{
    CString szLineResName;
    int linetype = LINETYPE_ANALOG;
    if(m_n_States <= STATE_OPENINGPORTS) {
...
void CSCCallMonitorDlg::OnStart() 
{
...
void CSCCallMonitorDlg::OnClear() 
{
    m_lbMessages.ResetContent();
}

void CSCCallMonitorDlg::OnCopy() 
{
...
void CSCCallMonitorDlg::OnHangup1() 
{
...
void CSCCallMonitorDlg::OnHangup2() 
{
    if(m_hModemObj2) {
        if(m_n_States == STATE_CONNECTED) {
            mdm_SCBUS_StopListeningTo(m_hLineHandle, m_hLineHandle2, SCBUS_FULLDUPLEX);
...
void CSCCallMonitorDlg::OnCancel() 
{
    bStop = true;
    bExit = true;
...
void CSCCallMonitorDlg::EnableControls(BOOL Enabled)
{
...
void CSCCallMonitorDlg::OnStopwait() 
{
    bStop = true;	
    if(bLine1Connected && bLine2Connected)
    {
...
void CSCCallMonitorDlg::RecordMessage(MODEMOBJ Modem)
{
    VOICEINFO       voiceInfo;
    DWORD           dwRetVal        = 0;
    BOOL            bOK             = TRUE;
	...
	char *pC = strrchr(g_szExePath, '\\');
	if( pC )
	{
		*pC = 0;
	}
    CString szMessage;
    szMessage.Format("%s\\VOICE\\MAIL_%d_%02d_%02d_%02d_%02d_%02d.WAV",
            g_szExePath,
    ...
    StatusMsg("Recording call to %s", szMessage);
    memset(&voiceInfo,0,sizeof(voiceInfo));
    VOICE_DEVICE((&voiceInfo)) = MDM_LINE;
    VOICE_DELIM((&voiceInfo))  = '#';
    VOICE_TIME((&voiceInfo))   = 300;
    strcpy(VOICE_FILE((&voiceInfo)), szMessage);

    if(Modem) {
        mdm_SetSilenceDetectionParams(Modem, MSD_DISABLED, 0);
        Modem->RecordVoice(&voiceInfo);
    }
}

void CSCCallMonitorDlg::OnDestroy() 
{
