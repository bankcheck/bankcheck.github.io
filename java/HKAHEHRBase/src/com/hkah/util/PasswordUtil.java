...
public class PasswordUtil {
	...
	public static String cis2RisPassword(String password) {
		String org = "LPRTVXZbcfehjlAFop4ruwy61N8v9QMqWYsCEnUaHI0x52diKmOgGt3SzkDJ7B";
		String ris = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";

		String cis_decrypt = null;
		StringBuffer ris_password = new StringBuffer();
		cis_decrypt = PasswordUtil.cisDecryption(password);
		for (int j = 0; j < cis_decrypt.length(); j++) {
			for (int k = 0; k < org.length(); k++) {
				if (cis_decrypt.charAt(j) == org.charAt(k)) {
					ris_password.append(ris.charAt(k));
				}
			}
		}
		return ris_password.toString();
	}
