create or replace FUNCTION NHS_GET_MEDREC_CUR_PAPERPRI (
	v_PATNO VARCHAR2
)
...
AS
	...
	CURSOR RUN_OUTCUR IS
		SELECT L.MRLDESC, M.MRMDESC, NVL(m.autoadd, 0), H.MRHVOLLAB, M.MRMID, H.MRHID --dental 20180123
...
BEGIN
	OPEN RUN_OUTCUR;
	LOOP
		FETCH RUN_OUTCUR INTO v_CurrentLocation, v_MediaType, v_isAuto, v_mrhVolLab, v_MRMID, v_mrhid;  --dental 20180123
    ...
    END IF;
		...
		OPEN OUTCUR FOR
			SELECT v_MedRecGetCur, v_CurrentLocation, v_MediaType, v_isAuto, v_mrhVolLab,v_mrhid  FROM DUAL; --dental 20180123
	ELSE
		BEGIN
	...
	OPEN OUTCUR FOR
		SELECT v_MEDRECGETCUR, v_CURRENTLOCATION, v_MEDIATYPE, v_ISAUTO, v_MRHVOLLAB,v_mrhid --dental 20180122
		FROM DUAL;
...
EXCEPTION
	...
	OPEN OUTCUR FOR
		...
		FROM DUAL;
	RETURN OUTCUR;
END NHS_GET_MEDREC_CUR_PAPERPRI;
/