create or replace FUNCTION "NHS_LIS_STATUSVIEWPB" (
	v_PATNO     IN VARCHAR2,
...
BEGIN
	SQLSTR := '
		SELECT '''',
			DECODE(B.BPBSTS, ''D'', ''C'', ''N'', ''N'') AS BPBSTS,
			B.BPBNO,
			B.PATNO,
			DECODE(NVL(P.TITDESC, ''''), '''', '''',  ''<font color=blue>'' || P.TITDESC || ''</font> '') || UPPER(B.PATFNAME || '' '' || B.PATGNAME) AS PATNAME,
			B.BPBCNAME AS PATCNAME,
			(SELECT D.DOCFNAME || '' '' || D.DOCGNAME || '' ('' || B.DOCCODE || '')''
				FROM DOCTOR D
				WHERE D.DOCCODE = B.DOCCODE) AS DOCNAME,
			DECODE(NVL(B.SEX, ''''), '''', '''', ''M'', ''<font color=blue>M</font>'', ''F'', ''<font color=red>F</font>'', B.SEX),
			...
			B.BEDCODE,
			TO_CHAR(B.BEDTIME,''DD/MM/YYYY HH24:MI'') AS BEDTIME,
			...
			TO_CHAR(B.EDITDATE,''DD/MM/YYYY'') AS EDITDATE,
			B.ISREFUSED,
			B.PBPID,
			B.USRID,
			B.EDITUSER,
