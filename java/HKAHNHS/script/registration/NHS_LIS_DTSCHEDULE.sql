create or replace FUNCTION           "NHS_LIS_DTSCHEDULE" (
...
sqlbuffer := sqlbuffer || ' AND S.DOCLOCID = ''3'' ';

sqlbuffer := sqlbuffer || ' ORDER BY S.SCHSDATE, D.DOCCODE';

DBMS_OUTPUT.PUT_LINE(sqlbuffer);
	OPEN outcur FOR sqlbuffer;
	RETURN OUTCUR;
END NHS_LIS_DTSCHEDULE;
/