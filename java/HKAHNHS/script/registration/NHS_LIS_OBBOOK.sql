CREATE OR REPLACE FUNCTION "NHS_LIS_OBBOOK"(
V_PATNO     IN VARCHAR2,
...
V_BPBEFDATE IN VARCHAR2,
V_BPBETDATE IN VARCHAR2)
	...
	IF V_STATUS = 'W' THEN
		SQLSTR := '
			SELECT '''',
				...
				TO_CHAR(B.OB_MODIFIED_DATE, ''DD/MM/YYYY'') as editdate,
				''''   AS ISREFUSED,
				'''' AS PBPID,
				'''' as USRID,
				B.OB_MODIFIED_USER as EDITUSER,
				B.BPBTYPE,
				B.OB_DOCTOR_CODE,
				'''' as fordelivery,
				B.OB_LASTNAME,
		...
		IF V_DOCNAME IS NOT NULL THEN
			SQLSTR := SQLSTR || ' AND B.OB_DOCTOR_CODE IN (SELECT DOCCODE FROM doctor WHERE DOCFNAME || '' '' || DOCGNAME LIKE ''' || V_DOCNAME || '%'')';
		END IF;
	...
	ELSIF V_STATUS = 'N' OR V_STATUS = 'F' OR V_STATUS = 'D' THEN
		...
		SQLSTR := '
			SELECT '''',
				...
				''''
			FROM BEDPREBOK B
			LEFT JOIN USR U1 ON U1.USRID = B.USRID
			LEFT JOIN USR U2 ON U2.USRID = B.EDITUSER
			LEFT JOIN PATIENT P ON P.PATNO = B.PATNO
			LEFT JOIN DOCTOR D ON D.DOCCODE = B.DOCCODE
			LEFT JOIN BEDPREBOK_EXTRA E ON B.PBPID = E.PBPID
		...
		IF V_BPBEFDATE IS NOT NULL THEN
			SQLSTR := SQLSTR || ' AND B.BPBEDATE >= TO_DATE(''' || V_BPBEFDATE || ' 00:00:00'',''dd/mm/yyyy HH24:MI:SS'')';
		END IF;

		IF V_BPBETDATE IS NOT NULL THEN
			SQLSTR := SQLSTR || ' AND B.BPBEDATE <= TO_DATE(''' || V_BPBETDATE || ' 23:59:59'',''dd/mm/yyyy HH24:MI:SS'')';
		END IF;
		...
		ELSIF V_ORDERBY = 'ED' THEN
			SQLSTR := SQLSTR || ' ORDER BY B.BPBEDATE, B.BPBPNAME';
