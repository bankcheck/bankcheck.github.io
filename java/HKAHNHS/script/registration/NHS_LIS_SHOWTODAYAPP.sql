create or replace FUNCTION "NHS_LIS_SHOWTODAYAPP" (
	v_patno IN VARCHAR2
)
...
BEGIN
	OPEN OUTCUR FOR
  SELECT SIT.STENAME, S.DOCCODE, D.DOCFNAME, D.DOCGNAME, TO_CHAR(B.BKGSDATE, 'DD/MM/YYYY'),
  ...
  AND B.PATNO = v_patno 
  AND TRUNC(B.BKGSDATE, 'DD') = TRUNC(SYSDATE,'DD')
  AND S.DOCLOCID IN (1,2) 
  AND BKGSTS = 'N' 
  ORDER BY TO_CHAR(B.BKGSDATE, 'HH24:MI');
  RETURN OUTCUR;
END NHS_LIS_SHOWTODAYAPP;
/