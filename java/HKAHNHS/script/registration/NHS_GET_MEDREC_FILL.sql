CREATE OR REPLACE FUNCTION NHS_GET_MEDREC_FILL (
	v_PATNO IN VARCHAR2
)
RETURN
	TYPES.CURSOR_TYPE
...
BEGIN
	IF TRIM(v_PATNO) IS NOT NULL THEN
		v_medrec_refcur := NHS_GET_MEDREC_CUR(v_PATNO);
		LOOP
	...
	OPEN OUTCUR FOR
		SELECT v_fillMedicalRecord, v_MRLocation, v_MediaType, v_isAuto, v_isSameDate, v_mrhVolLab
		FROM DUAL;
	RETURN OUTCUR;
END NHS_GET_MEDREC_FILL;
/
