CREATE OR REPLACE FUNCTION "NHS_GET_BOOKINGCOUNT"(V_EDC    IN VARCHAR2,
...
BEGIN
  ...
  SELECT PARAM1 INTO V_WARDCODE FROM SYSPARAM WHERE PARCDE = 'obwardcode';
  ...
  SELECT PARAM1 INTO V_monthlyWaitingStarts FROM SYSPARAM WHERE PARCDE = 'obwaitcnt' AND ROWNUM = 1;
  SELECT PARAM1 INTO V_sOBWaitingAuto FROM SYSPARAM WHERE PARCDE = 'OBAUTOWAIT' AND ROWNUM = 1;
