CREATE OR REPLACE FUNCTION "NHS_ACT_CREDITPACKAGE" (
...
BEGIN
	...
	SELECT count(1) INTO v_noOfRec FROM CreditPkg WHERE PKGCODE = v_PKGCODE;

	IF v_action = 'ADD' THEN
		IF v_noOfRec = 0 THEN
			INSERT INTO CreditPkg(
				PKGCODE,
				PKGNAME,
				PKGCNAME,
				PKGRLVL,
...
--			o_errmsg := 'Record already exists.';
	...
	ELSIF v_action = 'DEL' THEN
		IF v_noOfRec > 0 THEN
			DELETE CreditPkg WHERE PKGCODE = v_PKGCODE;
		ELSE
			o_errcode := -1;
