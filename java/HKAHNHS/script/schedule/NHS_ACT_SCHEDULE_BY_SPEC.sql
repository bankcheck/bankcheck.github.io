create or replace FUNCTION "NHS_ACT_SCHEDULE_BY_SPEC" (
...
BEGIN
	v_noOfRec := 0;
	o_ErrMsg := 'OK';
	v_logRmk :='<[NHS_ACT_SCHEDULE_BY_SPEC 1]V_SPEC:'||v_spec||' v_SchSDate:'||v_SchSDate||
	     'v_SchEDate:'||v_SchEDate||'v_IsBlock:'||v_IsBlock||'v_UserID:'||v_UserID||'>';

	-- check existing records
	SELECT COUNT(1) INTO v_noOfRec
	FROM   schedule sch, slot slt, docspclink l, doctor d
	WHERE  slt.sltstime BETWEEN TO_DATE(v_SchSDate, 'DD/MM/YYYY') AND TO_DATE(v_SchEDate, 'DD/MM/YYYY') + 1
	AND    slt.sltcnt > 0
	AND    sch.schid = slt.schid
	AND    sch.SCHSTS = 'N'
	AND   ((L.SPCCODE IN (
				SELECT regexp_substr(v_SPEC, '[^,]+', 1, LEVEL) token
				FROM dual
				CONNECT BY LEVEL <= LENGTH(V_SPEC) - LENGTH(REPLACE(V_SPEC, ',', '')) + 1)
	AND    sch.DOCCODE = L.DOCCODE)
	OR     (D.SPCCODE IN (
				SELECT regexp_substr(v_SPEC, '[^,]+', 1, LEVEL) token
				FROM dual
				CONNECT BY LEVEL <= LENGTH(V_SPEC) - LENGTH(REPLACE(V_SPEC, ',', '')) + 1)
	AND sch.doccode = d.doccode));

	IF v_noOfRec > 0 THEN
		IF v_IsBlock = 'N' THEN
			v_logRmk :=v_logRmk||'<[NHS_ACT_SCHEDULE_BY_SPEC 3] POP block app alert>';
			o_ErrMsg := 'Appointment already exists within the date range! Block the Appointment?';
			o_ErrCode := NHS_ACT_SYSLOG('ADD', 'ACT_SCHEDULE', 'ACT_SCH_BY_SPEC', v_logRmk, v_UserID, NULL, o_ErrMsg1);
			RETURN -100;
		ELSE
			v_IsCallBlock := TRUE;
			-- BlockExistApp
		...
		END IF;
	END IF;
	v_logRmk :=v_logRmk||'<[NHS_ACT_SCHEDULE_BY_SPEC 5:DELETE FROM Slot 1]>';

	-- DeleteExistSch
	DELETE FROM Slot
	WHERE  schid IN (
		SELECT s.schid
		FROM   Schedule s, DocSpcLink l
		WHERE  schsts = 'N'
		AND    s.schsdate >= TO_DATE(v_SchSDate, 'DD/MM/YYYY')
		AND    s.schedate <= TO_DATE(v_SchEDate, 'DD/MM/YYYY') + 1
		AND    s.doccode = l.doccode
		AND    l.spccode IN (
					SELECT regexp_substr(v_SPEC, '[^,]+', 1, LEVEL) token
					FROM dual
					CONNECT BY LEVEL <= LENGTH(V_SPEC) - LENGTH(REPLACE(V_SPEC, ',', '')) + 1
	...
	v_logRmk :=v_logRmk||'<[NHS_ACT_SCHEDULE_BY_SPEC 5:DELETE FROM Slot 2>';
	DELETE FROM Slot
	WHERE  schid IN (
		SELECT s.schid
	...
	v_logRmk :=v_logRmk||'<[NHS_ACT_SCHEDULE_BY_SPEC 5:DELETE FROM SCHEDULE 1>';
	...
	v_logRmk :=v_logRmk||'<[NHS_ACT_SCHEDULE_BY_SPEC 5:DELETE FROM SCHEDULE 2>';
	DELETE FROM Schedule
	WHERE  schid IN (
		SELECT s.schid
		FROM   Schedule s, Doctor d
