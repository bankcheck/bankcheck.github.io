create or replace FUNCTION "NHS_RPT_CHRGSMYWB_SUB1"(
...
*/    
  ...
  SQLTMP1 := '
    SELECT STNTDATE,
           DECODE(STNSTS,''C'',NHS_GET_SLIPTX_CANCEL_STNSDATE(slpno,stnseq,''' || V_GENDATE || ''') ,TRUNC(STNSDATE,''DD'')) STNCDATE,
           SLPNO,
           STNSEQ,
           STNBAMT,
           STNNAMT,
  ...
  SQLWHERE1 :=') IP WHERE IP.ITMCODE = ''PAYME'' 
    ...
    AND DECODE(IP.STNSTS,''C'',NVL(NHS_GET_SLIPTX_CANCEL_STNSDATE(slpno,stnseq,''' || V_GENDATE || '''),TRUNC(SYSDATE,''DD'')) ,NVL(TRUNC(STNCDATE,''DD''),TRUNC(SYSDATE,''DD''))) <TO_DATE(''' || V_GENDATE || ' 000000'',''DD/MM/YYYY HH24MISS'')';
--   TO_DATE(''' || V_GENDATE || ' 000000'',''DD/MM/YYYY HH24MISS'') CANNOT SHOW PAYMENT ON SAME DATE, NEED CHANGE TO  TO_DATE(''' || V_GENDATE || ' 000001'',''DD/MM/YYYY HH24MISS'') 
  SQLWHERE3 :=') IP WHERE IP.ITMCODE = ''REF'' 
    AND IP.STNCDATE IS NOT NULL 
    AND IP.SLPNO IN (''' || V_ALLSLPNO || ''')
    AND DECODE(IP.STNSTS,''C'',NVL(NHS_GET_SLIPTX_CANCEL_STNSDATE(slpno,stnseq,''' || V_GENDATE || '''),TRUNC(SYSDATE,''DD'')) ,NVL(TRUNC(STNCDATE,''DD''),TRUNC(SYSDATE,''DD''))) <TO_DATE(''' || V_GENDATE || ' 000000'',''DD/MM/YYYY HH24MISS'')';    
 
--  SQLWHERE2 := ' AND trunc(DECODE(STNSTS,''C'',NVL(NHS_GET_SLIPTX_CANCEL_STNSDATE(slpno,stnseq,''' || V_GENDATE || '''),TRUNC(SYSDATE,''DD'')) ,NVL(TRUNC(STNCDATE,''DD''),TRUNC(SYSDATE,''DD''))))>=trunc(TO_DATE(''' || LASTPRTDATE || ''',''DD/MM/YYYY''),''dd'')';
...
--  END IF;
  ...
  SQLWHEREU1 :=') IP WHERE IP.STNCDATE IS NOT NULL 
    AND IP.SLPNO IN (''' || V_ALLSLPNO || ''')
    AND DECODE(IP.STNSTS,''C'',NHS_GET_SLIPTX_CANCEL_STNCDATE(slpno,stnseq,''' || V_GENDATE || ''') ,TRUNC(STNCDATE,''DD'')) <trunc(TO_DATE(''' || V_GENDATE || ' 000000'',''DD/MM/YYYY HH24MISS''),''dd'')
...
--    UNION ALL 
  ...
  SQLSTR := '
    ...
    FROM ('||
    'SELECT TO_CHAR(STNTDATE, ''DD/MM/YYYY'') AS STNTDATE,SUM(AMOUNT) AS AMOUNT, DESCRIPTION, SUM(QTY) AS QTY,
