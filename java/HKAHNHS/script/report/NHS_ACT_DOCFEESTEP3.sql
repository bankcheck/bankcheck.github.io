CREATE OR REPLACE FUNCTION NHS_ACT_DOCFEESTEP3 (
...
BEGIN
	...
	IF cp_status = C_STAGE_6 THEN
		OPEN CUR1 FOR
			select slpno from
			(
			  select slpno from df_slip
				minus
			  select slpno from df_sliptx
			) tmp
			where (cp_nextno is null or slpno >= cp_nextno)
			order by slpno;
		LOOP
			FETCH CUR1 INTO V_SLPNO;
			EXIT WHEN CUR1%NOTFOUND;
			
			update slip set slpadoc = cp_dateend
        ...
        v_return := NHS_ACT_DOCFEE_UPDCHKPT('MOD', NULL, NULL, C_STAGE_7, NULL, NULL, NULL, NULL, NULL, v_ERRMSG);
    	v_STATUS := C_STAGE_7;
    	COMMIT;
	END IF;
    --------------
