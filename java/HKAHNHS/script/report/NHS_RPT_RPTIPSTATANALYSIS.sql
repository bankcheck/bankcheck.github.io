create or replace
FUNCTION NHS_RPT_RPTIPSTATANALYSIS(
  v_SteCode VARCHAR2,
...
BEGIN
  ...
  OPEN outcur FOR
	SELECT t1.spccode, t1.spcname, t1.fg, nvl(t2.NOOFDISCHARGED,0) AS NOOFDISCHARGED, 
			nvl(t3.NOOFMID,0) AS NOOFMID, nvl(t4.NOOF24,0) AS NOOF24, NVL(t5.noofdeath, 0) AS NOOF_DEATH
	FROM (
		...
		(
			SELECT SPCCODE,FG,NOOFDISCHARGED 
			FROM (
					-- START UNION t2.1
					select d.spccode,'A' as FG,COUNT(1) AS NoOfDischarged
					...
					UNION
					-- START UNION t2.2
					SELECT  spccode,'B' AS FG,COUNT(1) AS NoOfDischarged
					FROM (
							-- START UNION t2.2.1
					...
					UNION
					-- START UNION t2.3
					SELECT spccode,'C' AS FG,COUNT(1) AS NoOfDischarged
					FROM (
							-- START UNION t2.3.1
							SELECT d.spccode, r.regid
							FROM inpat i, reg r, doctor d,Patient p
							...
							and r.regsts = 'N'
							AND r.stecode= v_SteCode
							and r.regMDdate >= to_date(v_StartDate,'DD/MM/YYYY')
							AND r.regMDdate < to_date(v_EndDate,'DD/MM/YYYY') + 1
							AND r.patno=p.patno
							AND i.doccode_d = d.doccode
							AND trunc(r.regdate)=trunc(p.PATBDATE)
							AND TRUNC(i.inpddate)> TRUNC(r.regMDdate)
							-- END UNION t2.3.2
							UNION
							-- START UNION t2.3.3
							select d.spccode, r.regid
			...
			FROM (
					...
					FROM (
							...
							select d.spccode, dh.regid, i.inpddate, dh.dhsdate, dh.dhsedate,
									trunc(least(nvl(dh.dhsedate,to_date(v_EndDate,'DD/MM/YYYY')+1),
									to_date(v_EndDate,'DD/MM/YYYY')+1)) - trunc(greatest(nvl(r.regMDdate,dh.dhsdate),
									dh.dhsdate,to_date(v_StartDate,'DD/MM/YYYY'))) as cnt
					...
					FROM (
							...
							and (dh.dhsedate is null or dh.dhsedate >= to_date(v_StartDate,'DD/MM/YYYY'))
							AND r.inpid = i.inpid
							and (i.inpddate is null or (trunc(i.inpddate) > r.regmddate and trunc(r.regdate) <> trunc(i.inpddate) and trunc(i.inpddate) > to_date(v_StartDate,'DD/MM/YYYY')))
							AND p.patno=r.patno
							AND trunc(r.regdate)=trunc(p.PATBDATE)
							AND (r.regmddate IS NOT NULL AND trunc(r.regMDdate) < TO_DATE(TO_CHAR(TRUNC(TO_DATE(v_EndDate,'DD/MM/YYYY')+1),'DD/MM/YYYY')||' 00:00:00','DD/MM/YYYY :HH24:MI:SS'))
							-- END UNION t3.2.1
							UNION
							-- START UNION t3.2.2
							select d.spccode, dh.regid, i.inpddate, dh.dhsdate, dh.dhsedate,
							...
							and dh.dhsdate <= TO_DATE(TO_CHAR(TRUNC(TO_DATE(v_EndDate,'DD/MM/YYYY')),'DD/MM/YYYY')||' 23:59:59','DD/MM/YYYY :HH24:MI:SS')
							and (dh.dhsedate is null or dh.dhsedate >= to_date(v_StartDate,'DD/MM/YYYY'))
							and p.patno=r.patno  and trunc(r.regdate)<>trunc(p.PATBDATE) and r.inpid = i.inpid    and (((trunc(i.inpddate) > to_date(v_EndDate,'DD/MM/YYYY'))
					...
					FROM (
							select d.spccode, dh.regid, i.inpddate, dh.dhsdate, dh.dhsedate,
									trunc(least(nvl(dh.dhsedate,to_date(v_EndDate,'DD/MM/YYYY')+1),
									nvl(r.regMDdate,to_date(v_EndDate,'DD/MM/YYYY')+1),
									to_date(v_EndDate,'DD/MM/YYYY')+1)) - trunc(greatest(dh.dhsdate,to_date(v_StartDate,'DD/MM/YYYY'))) as cnt
							FROM dochist dh, doctor d, reg r, inpat i ,patient p
							WHERE dh.DocCode = d.DocCode
							AND dh.regid = r.regid (+)
							AND r.regsts = 'N'
							...
							and (
									i.inpddate is null 
									or	(
											trunc(r.regdate) <> trunc(i.inpddate)
											and trunc(i.inpddate) > to_date(v_StartDate,'DD/MM/YYYY') 
										) 
								)
							AND p.patno=r.patno
							and trunc(r.regdate)=trunc(p.PATBDATE)
							AND (r.regMDdate IS NULL OR trunc(r.regMDdate) > to_date(v_StartDate,'DD/MM/YYYY'))
					) tmp
					where cnt>=0
					GROUP BY spccode
					-- END UNION t3.3
		...
		(
			SELECT spccode,fg,noof24 
			FROM (
					-- START UNION t4.1
					select  spccode,'A' as FG,COUNT(1) AS NoOf24
					...
					UNION
					-- START UNION t4.2
					SELECT  spccode,'B' AS FG,COUNT(1) AS NoOf24
					FROM (
							-- START UNION t4.2.1
							SELECT spccode
					...
					UNION
					-- START UNION t4.3
					select  d.spccode,'C' as FG,COUNT(1) AS NoOf24
					FROM dochist dh,inpat i, reg r, doctor d ,Patient p
					WHERE dh.DocCode = d.DocCode
					AND dh.regid = r.regid (+)
					AND r.regtype = 'I'
					...
					and dh.dhsdate < TO_DATE(v_EndDate,'DD/MM/YYYY')+1
					AND r.patno=p.patno
					AND i.sdscode IS NULL
					and trunc(p.PATBDATE)=trunc(r.regdate)
					GROUP BY d.spccode
					-- END UNION t4.3
			)
		) t4,(
				SELECT spccode,fg,noofdeath 
				FROM (
						-- START UNION t5.1
