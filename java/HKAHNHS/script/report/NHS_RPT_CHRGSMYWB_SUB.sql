create or replace FUNCTION "NHS_RPT_CHRGSMYWB_SUB"(
  ...
  SQLTMP1 := '
    SELECT ST.STNTDATE STNTDATE,
      ...
      WHERE ST.DSCCODE = DPS.DSCCODE(+) AND SUBSTR(ST.GLCCODE, 1, 4) = DP.DPTCODE(+)
       ...
       AND NOT EXISTS (SELECT 1 FROM SLIPTX ST2 WHERE ST.SLPNO = ST2.SLPNO AND st2.stnsts = ''U'' AND st2.stnbamt <0 AND EXISTS (SELECT 1 FROM SLIPTX ST3 WHERE ST.SLPNO = ST3.SLPNO AND TO_CHAR(ST3.STNSEQ) = ST.STNDESC AND ST3.STNNAMT*-1=ST.STNNAMT AND ST3.STNSTS=''C''))
       AND NOT EXISTS (SELECT 1 FROM SLIPTX ST2 WHERE ST.SLPNO = ST2.SLPNO AND st2.stnsts = ''C'' AND st2.stnbamt >0 AND EXISTS (SELECT 1 FROM SLIPTX ST3 WHERE ST.SLPNO = ST3.SLPNO AND TO_CHAR(ST.STNSEQ) = ST3.STNDESC AND ST3.STNNAMT=ST.STNNAMT*-1 AND ST3.STNSTS=''U''))       
       AND ST.SLPNO IN (' || V_ALLSLPNO || ') ' || V_SPRNRANGE || ' 
       AND trunc(STNCDATE,''dd'') <trunc(TO_DATE(''' || V_GENDATE || ' 000001'',''DD/MM/YYYY HH24MISS''),''dd'')';

  SQLTMP2 := ' AND trunc(DECODE(ST.STNSTS,''C'',NHS_GET_SLIPTX_CANCEL_STNCDATE(slpno,stnseq,''' || V_GENDATE || ''') ,trunc(STNCDATE,''dd'')))>=trunc(TO_DATE(''' || LASTPRTDATE || ''',''DD/MM/YYYY''),''dd'')';       

--  IF V_GENDATE = LASTPRTDATE OR LASTPRTDATE IS NULL THEN
    SQLTMP := SQLTMP1;
--  ELSE
--    SQLTMP := SQLTMP1||SQLTMP2;
--  END IF;
