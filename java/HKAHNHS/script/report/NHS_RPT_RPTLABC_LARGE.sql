create or replace
FUNCTION "NHS_RPT_RPTLABC_LARGE" (
...
AS
	...
	typeBStart NUMBER(3);
	typeCStart NUMBER(3);
  typeC1Start NUMBER(3);
	typeDStart NUMBER(3);
	prtType VARCHAR(30);
	medIDCnt NUMBER(3);
	TYPECCNT VARCHAR(10);
  typeC1Cnt VARCHAR(10);
...
BEGIN
	...
	IF (totalLbl - v_typeBCnt - typeCCnt - typeC1Cnt) < v_typeDCnt THEN
		typeDCnt2 := TO_CHAR(totalLbl - v_typeBCnt - typeCCnt- typeC1Cnt);
	ELSE
		typeDCnt2 := v_typeDCnt;
	END IF;

	typeACnt := totallbl - v_typeBCnt - typeCCnt - typeC1Cnt - v_typeDCnt;
  ...
  typeC1Start := typeCStart + typeCCnt;
	typeDStart := typeC1Start + typeC1Cnt;
