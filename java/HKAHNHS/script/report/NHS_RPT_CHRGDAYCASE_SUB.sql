create or replace
FUNCTION "NHS_RPT_CHRGDAYCASE_SUB"(
...
BEGIN
  SQLTMP := '
    SELECT ST.STNTDATE STNTDATE,
           ...
           UPPER(DECODE(ST.STNRLVL, 1, DPS.DSCDESC||''  '',
                                    2, DP.DPTNAME||''  '',
                                    3, ST.STNDESC || '' '',
                                    4, PK.PKGNAME||CPK.PKGNAME||'' '',
                                    5, PK.PKGNAME||CPK.PKGNAME||'' '',
                                    6, DPS.DSCDESC||'' ''||ST.STNDESC||''  ''||ST.STNDESC1,
                                    7, PK.PKGNAME||CPK.PKGNAME||'' '',
                                    NULL)) DESCRIPTION,
           ST.UNIT AS QTY,
           DECODE(ST.STNRLVL,5,''D'',4,''W'',ST.ITMTYPE) AS ITMTYPE,
           DECODE(ST.STNRLVL,4,''D'',ST.STNTYPE) STNTYPE,
  ...
  SQLSTR := '
    ...
    ORDER BY DECODE(STNTYPE, ''D'', 1, ''O'', 2, ''R'', 3, ''C'', 4, ''S'', 5, ''I'', 6, ''P'', 7, 8), ITMTYPE, DOCNAME, DESCRIPTION ';

  OPEN OUTCUR FOR SQLSTR;
  RETURN OUTCUR;
END NHS_RPT_CHRGDAYCASE_SUB;
/