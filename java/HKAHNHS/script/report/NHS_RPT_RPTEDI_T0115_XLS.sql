create or replace FUNCTION "NHS_RPT_RPTEDI_T0115_XLS"(V_ALLSLPNO IN VARCHAR2)
...
BEGIN
  ...
  SQLSTR := 'select 
    ...
    S.SLPNO as invNo,
    TO_CHAR(NHS_GET_EDIAXDATE(S.SLPNO),''dd/mm/yyyy'') as invdate,
    S.Slpplyno as memid,
    P.PATFNAME ||'' ''||P.PATGNAME as memName,
    TO_CHAR(R.REGDATE,''dd/mm/yyyy'') as incDate,
    ...
    (
        ...
        HS.HPRMK1,
       DECODE(NHS_GET_COUNTEDIPACKAGE(STX.SLPNO,''T0115''),0,''NA'',''AIA'') as aPackage,
        ...
        FROM
          (SELECT ST.SLPNO,
            ...
            ST.ITMCODE,
            (CASE
            WHEN (SELECT COUNT(1) FROM HPSTATUS WHERE ST.PKGCODE IN (SELECT HPSTATUS FROM HPSTATUS WHERE  ''PACKAGE_T0115''  = HPKEY AND HPTYPE   = ''EDI''  AND HPACTIVE    = -1))> 0
            THEN ST.PKGCODE
            ELSE '''' END) as PKGCODE,
            ST.STNRLVL,
            ST.STNNAMT,
            ...
            UPPER(DECODE(
            CASE
              WHEN (SELECT COUNT(1) FROM HPSTATUS WHERE ST.PKGCODE IN (SELECT HPSTATUS FROM HPSTATUS WHERE  HPKEY = ''T0115'' AND HPTYPE = ''EDI'' AND HPRMK1 = ''PACKAGE'' AND HPACTIVE    = -1))> 0
              THEN 1
              ELSE ST.STNRLVL
            END, 1, DPS.DSCDESC
            ||''  '', 2, DP.DPTNAME
  ...
  LEFT JOIN HPSTATUS HS ON HS.HPKEY = ''T0115'' AND HS.HPTYPE   = ''EDI'' AND (DSCCODE = HS.HPSTATUS OR DSCCODE||''_''||ITMTYPE = HS.HPSTATUS) AND HPACTIVE    = -1)
