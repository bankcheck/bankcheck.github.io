create or replace FUNCTION "NHS_RPT_ADMISSIONLABEL_WBOSPG" (
	v_PATNO VARCHAR2
)
...
BEGIN
	...
	OPEN OUTCUR FOR
		select
			DECODE(GET_REAL_STECODE(),'HKAH','HKAH - SR',GET_REAL_STECODE()), p.patno, p.patfname || ' ' || p.patgname AS patname,
			...
			v_lblrmk AS lblrmk,
			pe.birhtordspg
		From  Patient P, patient_extra pe
		Where p.patno = v_PATNO
		AND   p.patno = pe.patno(+);

	RETURN OUTCUR;
END NHS_RPT_ADMISSIONLABEL_WBOSPG;
/
