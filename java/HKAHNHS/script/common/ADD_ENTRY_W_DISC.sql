create or replace function ADD_ENTRY_W_DISC
-- ******************************COPYRIGHT NOTICE*******************************
...
-- ******************************************************************************
-- v_override = Y : Both amt & disc, A : Amt only, D : Disc only
...
--    v_ref_no  IN  VARCHAR2,        /*v_cpsid    IN  NUMBER,*/v_discount    IN  NUMBER := 0,
-- re-add the v_cpsid by abraham tsui : because the new mode of override can partly override disc and leave amout retrieve from db
    v_ref_no  IN  VARCHAR2,        v_cpsid    IN  NUMBER,   v_discount    IN  NUMBER := 0,    v_override_disc IN  VARCHAR2 := 'N',
...
-- Comment: Assign SlipTx.STNNAMT
    IF v_override_disc = 'Y' THEN
       rs_sliptx.STNDISC := v_discount ;
    END IF ;
