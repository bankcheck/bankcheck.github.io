CREATE OR REPLACE PROCEDURE CREATE_SLIP (
...
begin
	...
	dbms_output.put_line ('slipno:' || rs_slip.slpno);
	if Patient_No is null  then -- if patient_no is null, insert null to slpfname & slpgname
		insert into slip
		...
		(rs_slip.slpno, Null, Surname, Given_Name, rs_slip.slptype, Doctor_Code, rs_slip.dptcode, rs_slip.slpsts,
		...
		SLPHDISC,SLPDDISC,SLPSDISC,SLPSEQ,SLPPSEQ,SLPPAMT,
		SLPCAMT,SLPDAMT,STECODE,USRID) values
		(rs_slip.slpno, Patient_No, Null, Null, rs_slip.slptype, Doctor_Code, rs_slip.dptcode, rs_slip.slpsts,
		rs_slip.slphdisc, rs_slip.slpddisc, rs_slip.slpsdisc, rs_slip.slpseq, rs_slip.slppseq, rs_slip.slppamt,
		rs_slip.slpcamt, rs_slip.slpdamt, rs_slip.stecode, rs_slip.usrid);
		Slip_No := rs_slip.slpno;
	end if;
