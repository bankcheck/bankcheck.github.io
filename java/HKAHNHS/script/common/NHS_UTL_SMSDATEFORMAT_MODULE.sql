create or replace FUNCTION "NHS_UTL_SMSDATEFORMAT_MODULE" (
	i_Language IN VARCHAR2,
	i_SMSDate IN DATE,
	i_MODULE_ID IN VARCHAR2
)
	RETURN VARCHAR2
AS
...
BEGIN
	IF i_Language = 'zh-HK' OR i_Language = 'zh-CN' THEN
		...
		IF v_SMSTIME_MI = '00' THEN
			v_SMSDate := NHS_UTL_ERRORMESSAGE('MOBILEFORMAT', i_MODULE_ID || '_NOMINUTE', i_Language);
		ELSE
			v_SMSDate := NHS_UTL_ERRORMESSAGE('MOBILEFORMAT', i_MODULE_ID, i_Language);
		END IF;
	...
	ELSE
		SELECT TO_CHAR(i_SMSDate, 'YYYY', 'NLS_DATE_LANGUAGE = ''American'''),
			...
			TO_CHAR(i_SMSDate, 'AM', 'NLS_DATE_LANGUAGE = ''American''')
		INTO v_SMSTIME_YYYY, v_SMSTIME_MON, v_SMSTIME_DD, v_SMSTIME_DAY, v_SMSTIME_HH, v_SMSTIME_MI, v_SMSTIME_AM
		FROM DUAL;

		v_SMSDate := NHS_UTL_ERRORMESSAGE('MOBILEFORMAT', i_MODULE_ID, 'en');
	END IF;
...
END NHS_UTL_SMSDATEFORMAT_MODULE;
/