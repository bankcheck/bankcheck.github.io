CREATE OR REPLACE FUNCTION "NHS_UTL_CASHIERENTRY"(
...
AS
	...
	S9000_TXN_CARD_REFUND VARCHAR2(1) := '2';
	S9000_TXN_EPS_SALE VARCHAR2(1) := '5';
	S9090_TXN_OCT_SALE VARCHAR2(1) := '0';
	S9090_TXN_OCT_REFUND VARCHAR2(1) := '2';
	S9090_TXN_CUP_SALE VARCHAR2(1) := 'a';
	S9090_TXN_CUP_REFUND VARCHAR2(1) := 'l';
	S9090_TXN_QR_SALE VARCHAR2(1) := 'A';
	S9090_TXN_QR_REFUND VARCHAR2(1) := 'C';
...
BEGIN
	...
	IF i_TransactionType = CASHTX_TXNTYPE_RECEIVE THEN
		IF i_tmpReceiptNumber IS NULL THEN
			v_ReceiptNumber := NHS_UTL_CASHIERRECEIPTNUMBER;
		ELSE
	...
	ELSIF v_PaymentType = CASHTX_PAYTYPE_OCTOPUS THEN
		IF i_TransactionType = CASHTX_TXNTYPE_PAYOUT THEN
			v_CardTxType := S9090_TXN_OCT_REFUND;
		ELSE
