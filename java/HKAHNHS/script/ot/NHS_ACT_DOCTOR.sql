...
--			select SEQ_DOCTOR.NEXTVAL into v_DOCCODE from dual;
			...
			IF o_noOfRec = 0 THEN
				...
				(DOCCODE, STODECLARATION, DOC_CREATED_DATE, DOC_CREATED_USER, DOC_UPDATED_DATE, DOC_UPDATED_USER)
				VALUES
				(V_DOCCODE, TO_NUMBER(v_STODECLARATION), SYSDATE, v_USRID, SYSDATE, v_USRID);
			ELSE
...
--					USRID = v_USRID,
				...
				IF o_noOfRec = 0 THEN
					INSERT INTO DOCTOR_EXTRA
					(DOCCODE, STODECLARATION, DOC_UPDATED_DATE, DOC_UPDATED_USER)
					VALUES
					(V_DOCCODE, TO_NUMBER(v_STODECLARATION), SYSDATE, v_USRID);
				ELSE
					UPDATE DOCTOR_EXTRA set
						STODECLARATION = TO_NUMBER(v_STODECLARATION),
						DOC_UPDATED_DATE = SYSDATE,
