CREATE OR REPLACE
FUNCTION "NHS_GET_OTSLTSLIPSEARCH"(
  V_PATNO				IN VARCHAR2,
  ...
  SQLSTR_SELECT1 VARCHAR2(150);
  SQLSTR_SELECT2 VARCHAR2(150);
...
BEGIN
  ...
  -- SearchResult reg
  SQLSTR := '';
  SQLSTR_SELECT1 := 'select s.slpNo as slpNo,regdate,nvl(i.inpddate,'''') as regmddate ,D.DOCFNAME || '' '' || D.DOCGNAME as doccode ';
  SQLSTR_SELECT2 := 'select s.slpNo as slpNo,to_date('''') as regdate,to_date('''') as regmddate ,D.DOCFNAME || '' '' || D.DOCGNAME as doccode ';
