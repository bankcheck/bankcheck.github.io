 ...
 */
public abstract class DlgDIReportApprove extends DialogBase implements ConstantsTableColumn {
	...
	public DlgDIReportApprove(MainFrame owner) {
	...
	public void showDialog(String regid, String rptid, String rptdoc) {//XRGID, XRPID, DOCCODE
		xrgid = regid;
		xrpid = rptid;
		doccode = rptdoc.toUpperCase();
	...
	private void checkApproveUser(){
		...
		if (!doccode.equals(usr1.substring(2))) {
			Factory.getInstance().addSystemMessage("The approve user should be the reported doctor.");
			return;
		}
	...
	private void approveFunction(String usrid, String loginuser) {
		QueryUtil.executeMasterAction(getUserInfo(), "DIREPORTAPPROVE", QueryUtil.ACTION_APPEND, 
				new String[] { xrgid, xrpid, doccode, usrid, loginuser},
				new MessageQueueCallBack() {
			@Override
			public void onPostSuccess(MessageQueue mQueue) {
				if (mQueue.success()) {
					Factory.getInstance().addSystemMessage("Approved success.");
					post(true);
					//dispose();
				}else{
					Factory.getInstance().addSystemMessage("Report cannot be approve.");
					//dispose();
					post(false);
				}
			}
	...
	public LabelBase getLabel_Info() {
		if (Label_Info == null) {
			...
			Label_Info.setText("Are you sure to Approve?");
	...
	public LabelBase getLabel_Note() {
		if (Label_Note == null) {
			...
			Label_Note.setText(	"Note: <br> The approve user should be the reported doctor.");
