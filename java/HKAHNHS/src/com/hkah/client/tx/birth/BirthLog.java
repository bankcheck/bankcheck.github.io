...
public class BirthLog extends MasterPanel {

	private static final String SENT_REJECT = "Reject";
	private static final String SENT_MANUAL = "Manual";
	...
	public String getTxCode() {
		...
		return ConstantsTx.BIRTHLOG_TXCODE;
	}

	/* >>> ~2~ Set Transaction Title ====================================== <<< */
	@Override
	public String getTitle() {
		return ConstantsTx.BIRTHLOG_TITLE;
	}
	...
	protected void initAfterReady() {
		try {
			dRemind = Integer.valueOf( getSysParameter("dayRemind") );
		} catch (Exception e) {
			dRemind = 0;
		}

		if (dRemind > 0) {
			QueryUtil.executeMasterFetch(getUserInfo(), "DHBIRTHCOUNT",
					new String[] {},
				...
				public void onPostSuccess(MessageQueue mQueue) {
					...
					if (mQueue.success()) {
						int recCount = Integer.parseInt(mQueue.getContentField()[0]);

						if (Integer.parseInt(mQueue.getContentField()[1]) >= dRemind && recCount > 0) {
							MessageBoxBase.alert("Message", "There are " + recCount + " DH Birth records waiting for confirmation", null);
						}
					}
				}
			});
...
*/
	...
	protected void getFetchOutputValues(String[] outParam) {
		...
		System.err.println("[row]:"+getListTable().getSelectedRow());
		showHistory();
	}
	...
	protected void actionValidationPostReady(String actionType) {
		...
		setParameter("BB_PATNO", getListSelectedRow()[0]);

		if (NOT_CONFIRMED.equals(getListSelectedRow()[8]) || SENT_REJECT.equals(getListSelectedRow()[8])) {
			setParameter("sCanEdit","YES");
		} else {
			setParameter("sCanEdit","NO");
		}
		if (NOT_CONFIRMED.equals(getListSelectedRow()[8])) {
			setParameter("sCanConfirm","YES");
		} else {
			setParameter("sCanConfirm","NO");
		}

		showPanel(new BirthEntry());
	}
	...
	private void checkButtonEnable() {
		if (getListTable().getRowCount() > 0) {
			getMH().setEnabled(!isDisableFunction("cmdManual", "BirthLog") && SENT_REJECT.equals(getListSelectedRow()[8]));
			getExport().setEnabled(!isDisableFunction("cmdExport", "BirthLog") && SENT_MANUAL.equals(getListSelectedRow()[8]));
			getRemarks().setEnabled(!isDisableFunction("cmdRemarks", "BirthLog"));
			getClose().setEnabled(!isDisableFunction("cmdClose", "BirthLog") && SENT_MANUAL.equals(getListSelectedRow()[8]));
		} else {
	...
	private void showHistory() {
		if (getListTable().getRowCount() > 0) {
			getHistoryTable().setListTableContent(ConstantsTx.EBIRTHHIS_TXCODE, new String[] {getListSelectedRow()[2]});
		} else {
			...
			MessageBoxBase.confirm(MSG_PBA_SYSTEM, "The batch: " + batchno + " will be handled manually.\nAre you sure to continue?",
				...
				public void handleEvent(MessageBoxEvent be) {
					if (Dialog.YES.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
						QueryUtil.executeMasterAction(getUserInfo(), ConstantsTx.BIRTHCLOSE_TXCODE, getActionType(),
			...
			box.addCallback(new Listener<MessageBoxEvent>() {
				...
				public void handleEvent(MessageBoxEvent be) {
					...
					if (str != null && !str.trim().isEmpty()) {
						...
						QueryUtil.executeMasterAction(getUserInfo(), ConstantsTx.EBIRTHHIS_TXCODE, QueryUtil.ACTION_APPEND, inParam,
								new MessageQueueCallBack() {
							@Override
							public void onPostSuccess(MessageQueue mQueue) {
								// TODO Auto-generated method stub
...
/*
	    ...
	    length = GetIniSetting("default", "Certificate", 100, sCertFn, 100, INI_PBA)
	    ...
	    sBatchNo = GrdSearch.TextMatrix(GrdSearch.Row, 8)

	    If MsgBox("Are you sure to export the batch: " & sBatchNo & "?", vbQuestion + vbYesNo) = vbNo Then Exit Sub

	    sSql = " SELECT dtl.*,xml.sourcecode,xml.filerefno " & _
	           " FROM ebirthdtl dtl,ebirthxml xml " & _
	    ...
	    If Not CreateXML(True, cdExport.FileName, dServerDate, rstManual) Then
	        Exit Sub
	    End If
	    ...
	    Set rst_close = MainDef.Database.OpenRecordset("select bb_patno from ebirthdtl where batchno = '" & sBatchNo & "'", dbOpenSnapshot)
	    rst_close.MoveFirst
	    While Not rst_close.EOF
	        sSql = InsertEbirthhistory("" & rst_close("bb_patno"), "Y", "Exported", CurrentUser.UserID)
...
/*
    ...
    Dim xmlObj As New MSXML2.DOMDocument 'MSXML2.xm.DOMDocument

    Dim Node As MSXML2.IXMLDOMNode
    Dim ChildNode1 As MSXML2.IXMLDOMNode
    Dim ChildNodex As MSXML2.IXMLDOMNode

    Dim sFileName As String
    Dim CollReturnNo As New Collection
    Dim sItem As String

    Dim lSNCheckSum As Double
    Dim lDOBCheckSum As Double
    Dim lRecordCount As Long
    Dim sDOB As String
    Dim sXmlPathAndFileName As String
    Dim sCurReturnNo As String
    ...
    sXmlPathAndFileName = cdExport.FileName

    ' XML Header
    xmlObj.loadXML ("<BATCH></BATCH>")
    xmlObj.documentElement.appendChild GetFHSNode(data_rst("sourcecode"), data_rst("filerefno"), GetServerDate(MainDef.Database))

    Set Node = xmlObj.createNode(1, "MESSAGEBATCH", "")
    Set ChildNode1 = xmlObj.createNode(1, "MESSAGES", "")

    If Not data_rst.EOF Then

        data_rst.MoveFirst

        Do While Not data_rst.EOF
            lRecordCount = lRecordCount + 1
            sCurReturnNo = "" & data_rst("birthreturnno")

            Set ChildNodex = getMESSAGEBATCH_MESSAGE_ADT_A08Node(sCurReturnNo, data_rst)
            ChildNode1.appendChild ChildNodex
            sItem = CStr(data_rst.Fields("ebirthid")) & "_" & sCurReturnNo

            lSNCheckSum = lSNCheckSum + CDbl(Mid(sCurReturnNo, 2))

            If "" & data_rst.Fields("bb_DOB") <> "" Then
                sDOB = Format(data_rst.Fields("bb_DOB"), "YYYYMMDDHHMMSS")
                lDOBCheckSum = lDOBCheckSum + CDbl(sDOB)
            End If

            CollReturnNo.Add sItem, CStr(data_rst.Fields("ebirthid")) 'record current return No.
    ...
    Node.appendChild getBTSNode(CStr(lRecordCount), CStr(lSNCheckSum), CStr(lDOBCheckSum), CurrentUser.UserID)
    xmlObj.documentElement.appendChild Node
    xmlObj.Save sXmlPathAndFileName
    Set CollReturnNo = Nothing
    CreateXML = True
 */
	}
...
/*
Private Sub encryptFile(ByRef fn As String, ByVal receiverCert As String)    'encrypt file
...
/*
		Private Sub LoadFile(FileName, Buffer)
...
/*
Private Sub savefile(FileName, Buffer)
    Dim fso As New FileSystemObject
 ...
 */
	...
	protected void performList(final boolean showMessage) {
		...
		if (getSendDateFrom().getText().trim() != null && getSendDateFrom().getText().trim().length()>0) {
			if (!DateTimeUtil.isValidDate(getSendDateFrom().getText()) && validation) {
				msg = "Invalid Date.";
				validation = false;
				getSendDateFrom().focus();
			}
		}

		if (getSendDateTo().getText().trim() != null && getSendDateTo().getText().trim().length()>0) {
			if (!DateTimeUtil.isValidDate(getSendDateTo().getText()) && validation) {
				msg = "Invalid Date.";
				validation = false;
				getSendDateTo().focus();
			}
		...
		if (getBBDOBFrom().getText().trim() != null && getBBDOBFrom().getText().trim().length()>0) {
			if (!DateTimeUtil.isValidDate(getBBDOBFrom().getText()) && validation) {
				msg = "Invalid Date.";
				validation = false;
				getBBDOBFrom().focus();
			}
		...
		if (getBBDOBTo().getText().trim() != null && getBBDOBTo().getText().trim().length()>0) {
			if (!DateTimeUtil.isValidDate(getBBDOBTo().getText()) && validation) {
				msg = "Invalid Date.";
				validation = false;
				getBBDOBTo().focus();
			}
		...
		if (msg != null) {
			Factory.getInstance().addErrorMessage(msg, "PBA-[" + getTitle() + "]");
		}
	...
	protected void performListPost() {
		...
		getSearchCount().setText(String.valueOf(rowCount));
		checkButtonEnable();
		if (rowCount > 0) {
	...
	public void acceptAction() {
		if (getListTable().getSelectedRow() >= 0) {
			...
			if (NOT_CONFIRMED.equals(getListTable().getSelectedRowContent()[8]) || SENT_REJECT.equals(getListTable().getSelectedRowContent()[8])) {
				setParameter("sCanEdit","YES");
			} else {
				setParameter("sCanEdit","NO");
			}
			if (NOT_CONFIRMED.equals(getListTable().getSelectedRowContent()[8])) {
				setParameter("sCanConfirm","YES");
			} else {
				setParameter("sCanConfirm","NO");
			}
	...
	public BasePanel getRightPanel() {
		if (rightPanel == null) {
	...
	public LabelBase getSendDateToDesc() {
		if (SendDateToDesc == null) {
			...
			SendDateToDesc.setBounds(428, 10, 21, 20);
		}
	...
	public TextDate getSendDateTo() {
		if (SendDateTo == null) {
			...
			SendDateTo.setBounds(452, 10, 120, 20);
		}
		return SendDateTo;
	}
	...
	public LabelBase getSortByDesc() {
		if (SortByDesc == null) {
			...
			SortByDesc.setBounds(576, 10, 53, 20);
		}
		return SortByDesc;
	}
	...
	public BLogSortby getSortBy() {
		if (SortBy == null) {
			...
			SortBy.setBounds(630, 10, 114, 20);
		}
		return SortBy;
	}
	...
	public LabelBase getMotherDesc() {
		if (MotherDesc == null) {
			...
			MotherDesc.setBounds(10, 40, 64, 20);
		}
		return MotherDesc;
	}
	...
	public LabelBase getBBDOBToDesc() {
		if (BBDOBToDesc == null) {
			...
			BBDOBToDesc.setBounds(428, 40, 22, 20);
		}
		return BBDOBToDesc;
	}
	...
	public TextDate getBBDOBTo() {
		if (BBDOBTo == null) {
			...
			BBDOBTo.setBounds(452, 40, 120, 20);
		}
		return BBDOBTo;
	}
	...
	public LabelBase getStatusDesc() {
		if (StatusDesc == null) {
			...
			StatusDesc.setBounds(576, 40, 53, 20);
		}
		return StatusDesc;
	}
	...
	public BLogStatus getStatus() {
		if (Status == null) {
			...
			Status.setBounds(630, 40, 114, 20);
		}
		return Status;
	}
				...
				public void setQuickTip() {
					QuickTip qt = new QuickTip(this) {
						...
						private String getAttributeValue(Element target, String attr) {
							...
							if (v != null && "qtip".equals(attr)) {
								Element cellElement = (Element)target.getParentElement().getParentElement().getParentElement().getChild(7).getChild(0).getChild(0);
								v = cellElement.getInnerHTML();
							}
	...
	private String[] getDVATableColumnNames() {
		return new String[] {
				"", "", "",
				"Manual",
				"Status",
	...
	private int[] getDVATableColumnWidths() {
