...
public class DHSendQueue extends MasterPanel{

	/* >>> ~1~ Set Transaction Code ======================================= <<< */
	@Override
	public String getTxCode() {
		// tx code to communicate with server
		return ConstantsTx.DHSENDQUEUE_TXCODE;

	}
	/* >>> ~2~ Set Transaction Title ====================================== <<< */
	@Override
	public String getTitle() {
		return ConstantsTx.DHSENDQUEUE_TITLE;
	}
			...
			listTable = new EditorTableList(getColumnNames(), getColumnWidths(), getItemDetailEditor()) {
				...
				public void postSaveTable(boolean success, Integer rtnCode, String rtnMsg) {
					if (success) {
						...
						Factory.getInstance().addErrorMessage("Un-Confirm failed");
					}
				}
			};
	...
	public LabelBase getRemarkDesc() {
		if (RemarkDesc == null) {
			...
			RemarkDesc.setText("<html>Confirm Birth data will be shown in Send Queue and send to Health Department in specified time period. User could unconfirm selected birth records for <br> modifications. User could also send selected records immediately for ad-hoc purpose.</html>");
				...
				public void onClick() {
					if (!bIsSelectAll) {
						for (int i = 0; i < getEditorListTable().getRowCount(); i++) {
							getEditorListTable().setValueAt(YES_VALUE,i,6);
							bIsSelectAll = true;
						}
						getEditorListTable().getView().refresh(false);
						getEditorListTable().getView().layout();
					} else {
						for (int i = 0; i < getEditorListTable().getRowCount(); i++) {
							getEditorListTable().setValueAt(NO_VALUE,i,6);
							bIsSelectAll = false;
						}
						getEditorListTable().getView().refresh(false);
						getEditorListTable().getView().layout();
	...
	public ButtonBase getUnConfirm() {
		if (UnConfirm == null) {
			UnConfirm=new ButtonBase() {
				@Override
				public void onClick() {
					if (getEditorListTable().getRowCount() > 0) {
						int clickYes = 0;
		                for (int i = 0; i < getEditorListTable().getRowCount(); i++) {
		                	if (YES_VALUE.equals(getEditorListTable().getValueAt(i, 6))) {
		                		clickYes++;
		                	}
		                }

						if (clickYes > 0) {
							getEditorListTable().saveTable("DHBIRTHUNCONFIRM",
									new String[] {
