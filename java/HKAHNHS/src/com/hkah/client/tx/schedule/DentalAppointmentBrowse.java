...
public class DentalAppointmentBrowse extends MasterPanel{
	...
	public String getTxCode() {
		...
		return ConstantsTx.DENTALAPPOINTMENTBROWSE_TXCODE;
	}

	/* >>> ~2~ Set Transaction Title ====================================== <<< */
	@Override
	public String getTitle() {
		return ConstantsTx.DENTALAPPOINTMENTBROWSE_TITLE;
	}
...
/*	private int missedCol = getRtnSlotRowNo(getSysParameter("DENSTIME"),getSysParameter("DENSTIME"), null)[0];
	 ...
	 *
	 */
	public DentalAppointmentBrowse() {
		super();
	}
	...
	protected void initAfterReady() {
		...
		missedCol = getRtnSlotRowNo(getSysParameter("DENSTIME"),getSysParameter("DENSTIME"), null)[0];
		maxCol = getRtnSlotRowNo(getSysParameter("DENETIME"),getSysParameter("DENETIME"), null)[1];
		getScheduleTable().setColumnColor(DocSchCol_SCHDESC, "red");
	...
	**************************************************************************/
	...
	protected void searchScheduleAction(String SDate,String EDate){
		if (searchScheduleValidation()){
			getScheduleTable().setListTableContent("DTSCHEDULE",
					new String[] {
					SDate,
					EDate,
					getDrCode().getText(),
...
//					memIsUpdatePatSMS == null ? EMPTY_VALUE : memIsUpdatePatSMS,
			...
			QueryUtil.executeMasterAction(getUserInfo(), "DTBOOKING", QueryUtil.ACTION_APPEND, inParam,
					new MessageQueueCallBack() {
				@Override
				public void onPostSuccess(MessageQueue mQueue) {
					if (mQueue.success()) {
...
//						mb.getDialog().setFocusWidget(mb.getDialog().getButtonBar().getItemByItemId(Dialog.YES));
	...
	**************************************************************************/
	...
	private DentalAppointmentBrowse getThis() {
		return this;
	}
	...
	private void setGrdAppBseViewRefresh(int rowNo, int colNo) {
		...
		getDVBTable().setListTableContent("DTDVBTABLELIST", new String[] {DeptAID_sql.toString(), Factory.getInstance().getUserInfo().getUserID()});
	...
	private void DVATableRefreshHelper(final List<TableData> tempTextData, final List<TableData> tempTxtColorData, final List<TableData> tempBkgColorData,
		...
		QueryUtil.executeMasterBrowse(getUserInfo(), "DTSCHEDULE",
				new String[] {
			dvStartDate,
			dvStartDate,
			null,
...
//		                  addRow(fields);
				...
				QueryUtil.executeMasterBrowse(getUserInfo(), "DTDVATABLELIST", new String[] {dvStartDate},
						new MessageQueueCallBack() {
					@Override
					public void onPostSuccess(MessageQueue mQueue) {
						if (mQueue.success()) {
...
/*						getSelectedTime().setText(selectedTime);*/
	...
	public BasePanel getRightBtmPanel() {
		if (rightBtmPanel == null) {
			...
			rightBtmPanel.setBounds(10, 480, 786, 100);
		}
...
/*					checkStaff();
	...
	private LabelSmallBase getRemarkDesc() {
		if (remarkDesc == null) {
			...
			remarkDesc.setBounds(540, 30, 60, 20);
		}
		return remarkDesc;
	}
	...
	private ButtonBase getConfirmAppAction() {
		if (confirmAppAction == null) {
			...
			confirmAppAction.setBounds(5, 60, 123, 30);
		}
	...
	private ButtonBase getCancelAppAction() {
		if (cancelAppAction == null) {
			...
			cancelAppAction.setBounds(130, 60, 123, 30);
		}
	...
	private ButtonBase getEditAppAction() {
		if (editAppAction == null) {
			...
			editAppAction.setBounds(255, 60, 133, 30);
		}
	...
	private ButtonBase getChangeAppDetailAction() {
		if (changeAppDetailAction == null) {
			...
			changeAppDetailAction.setBounds(390, 60, 133, 30);
		}
	...
	private ButtonBase getWalkInAction() {
		if (walkInAction == null) {
			...
			walkInAction.setBounds(525, 60, 80, 30);
		}
		return walkInAction;
	}
	...
	private ButtonBase getAppBrowseAction() {
		if (appBrowseAction == null) {
			...
			appBrowseAction.setBounds(610, 60, 145, 30);
		}
	...
	public CheckBoxBase getSortByTime() {
		if (sortByTime == null) {
			...
			sortByTime.setBounds(765, 35, 30, 20);
		}
		return sortByTime;
	}
