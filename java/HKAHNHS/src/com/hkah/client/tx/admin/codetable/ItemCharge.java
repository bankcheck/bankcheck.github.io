...
public class ItemCharge extends MasterPanel {

	@Override
	public String getTitle() {
		// TODO Auto-generated method stub
		return ConstantsTx.ITEMCHARGE_TITLE;
	}

	@Override
	public String getTxCode() {
		// TODO Auto-generated method stub
		return ConstantsTx.ITEMCHARGE_TXCODE;
	}
	...
	private TextNum RightJText_ChaeperRate=null;
	...
	protected void actionValidation(String actionType) {
		if (getRightJCombo_ItcType().getText().length()==0) {
			Factory.getInstance().addErrorMessage("Empty Itc Type.",getRightJCombo_ItcType());
			actionValidationReady(actionType, false);
		} else if (getRightJText_GLCode().getText().length()==0) {
			Factory.getInstance().addErrorMessage("Empty GL Code.",getRightJText_GLCode());
			actionValidationReady(actionType, false);
		} else if (getRightJText_ChaeperRate().getText().length()==0) {
			Factory.getInstance().addErrorMessage("Empty Chaeper Rate.",getRightJText_ChaeperRate());
			actionValidationReady(actionType, false);
		} else if (getRightJText_higherRate().getText().length()==0) {
			Factory.getInstance().addErrorMessage("Empty Higher Rate.",getRightJText_higherRate());
			actionValidationReady(actionType, false);
		} else {
	...
	protected void getFetchOutputValues(String[] outParam) {
		...
		if (getItemChargeTable().getSelectedRowCount()>0) {
			...
			itid=getItemChargeTable().getSelectedRowContent()[1];
		}
	}

	private void vlidaHCC() {
		// TODO Auto-generated method stub
		if (getRightJText_higherRate().getText().trim().length()==0 &&
				getRightJText_ChaeperRate().getText().trim().length()==0 &&
				getRightJText_Contracted().getText().trim().length()==0) {
			getRightJText_higherRate().setEnabled(false);
			getRightJText_ChaeperRate().setEnabled(false);
			getRightJText_Contracted().setEnabled(false);
		}
		if (getRightJText_Contracted().getText().trim().length()!=0) {
			getRightJText_higherRate().setEnabled(true);
			getRightJText_ChaeperRate().setEnabled(true);
			getRightJText_Contracted().setEnabled(false);
		} else if (getRightJText_higherRate().getText().trim().length()!=0||getRightJText_ChaeperRate().getText().trim().length()!=0) {
			getRightJText_higherRate().setEnabled(false);
			getRightJText_ChaeperRate().setEnabled(false);
			getRightJText_Contracted().setEnabled(true);
		}
	...
	protected void showHistory() {
		if (getListTable().getRowCount()>0) {
			...
			QueryUtil.executeMasterBrowse(
					getUserInfo(), ConstantsTx.ITEMCHARGELIST_TXCODE,
	...
	private TextNum getRightJText_higherRate() {
		if (RightJText_higherRate == null) {
			RightJText_higherRate = new TextNum(126) {
				public void onBlur() {
					//vlidaHCC();
				};
			};
	...
	private TextNum getRightJText_ChaeperRate() {
		if (RightJText_ChaeperRate == null) {
			RightJText_ChaeperRate = new TextNum(126) {
				public void onBlur() {
					//vlidaHCC();
				};
			};
	...
	private TextString getRightJText_GLCode() {
		if (RightJText_GLCode == null) {
			RightJText_GLCode = new TextString() {
				public void onBlur() {
					if (RightJText_GLCode.getText()==null || RightJText_GLCode.getText().trim().length()==0) {
						return;
					}
					QueryUtil.executeMasterBrowse(getUserInfo(), ConstantsTx.LOOKUP_TXCODE,
							...
							new MessageQueueCallBack() {
								...
								public void onPostSuccess(MessageQueue mQueue) {
									if (!mQueue.success()) {
										Factory.getInstance().addErrorMessage("Invalid GLCode Code.");
	...
	private TextNum getRightJText_Contracted() {
		if (RightJText_Contracted==null) {
			RightJText_Contracted=new TextNum(126);
...
//			generalPanel.setLayout(new AbsoluteLayout());
		...
		if (ItemChargeTable == null) {
			ItemChargeTable = new TableList(getDVATableColumnNames(), getDVATableColumnWidths()) {
				public void onSelectionChanged() {
					getRightJText_ItemCode().setText(getItemChargeTable().getSelectedRowContent()[2]);
					getRightJText_higherRate().setText(getItemChargeTable().getSelectedRowContent()[9]);
					getRightJCombo_ItcType().setText(getItemChargeTable().getSelectedRowContent()[3]);
					getRightJText_PackageCode().setText(getItemChargeTable().getSelectedRowContent()[4]);
					getRightJText_ChaeperRate().setText(getItemChargeTable().getSelectedRowContent()[8]);
	...
	private ButtonBase getCreate_set() {
		if (create_set==null) {
			create_set=new ButtonBase() {
				@Override
				public void onClick() {
					new DlgCreateSet(getMainFrame(), "createset");
					searchAction();
				}
			};
