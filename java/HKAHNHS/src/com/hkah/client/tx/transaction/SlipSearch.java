...
public class SlipSearch extends SearchPanel {
	...
	public SlipSearch(BasePanel panelFrom) {
		super();
...
//		getListTable().setColumnClass(6, new ComboSlipStatus(), false);
	...
	private void updateSlipStatus(final String slpNo, final String method) {
	...
	private void lockRecord(String action) {
		if (getListTable().getSelectedRow() >= 0) {
			if ("unlock".equals(action)) {
				QueryUtil.executeMasterFetch(getUserInfo(), ConstantsTx.SLIPLOCK_TXCODE,
						new String[] { memSlpNo },
						new MessageQueueCallBack() {
					public void onPostSuccess(MessageQueue mQueue) {
						if (mQueue.success()) {
	...
	protected void lockRecordReady(final String lockType, final String lockKey, final String[] record, boolean lock, String returnMessage) {
		if (!lock) {
				...
				MessageBox mb = MessageBoxBase.confirm(MSG_PBA_SYSTEM, returnMessage + " <font color='green'>Do you want to read-only mode?</font>",
					...
					public void handleEvent(MessageBoxEvent be) {
						if (Dialog.YES.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
							...
							if (record.length > 1 && record[1] != null && "newSlip".equals(record[1])) {
								setParameter("isNewSlip", YES_VALUE);
							}
							setParameter("isLocked", NO_VALUE);
							setParameter("isReadOnly", YES_VALUE);
			...
			QueryUtil.executeMasterFetch(getUserInfo(), ConstantsTx.LOOKUP_TXCODE,
				...
				public void onPostSuccess(MessageQueue mQueue) {
					...
					MessageBox mb = MessageBoxBase.confirm(MSG_PBA_SYSTEM,  cancelMessage,
						...
						public void handleEvent(MessageBoxEvent be) {
							if (Dialog.YES.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
								updateSlipStatus(lockKey, "cancel");
							} else if (Dialog.NO.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
								unlockRecord("Slip", lockKey);
							}
						}
			...
			MessageBox mb = MessageBoxBase.confirm(MSG_PBA_SYSTEM,  "Do you want to reactive the selected slip?",
				...
				public void handleEvent(MessageBoxEvent be) {
					if (Dialog.YES.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
						updateSlipStatus(lockKey, "reactive");
					} else if (Dialog.NO.equalsIgnoreCase(be.getButtonClicked().getItemId())) {
						unlockRecord("Slip", lockKey);
					}
				}
