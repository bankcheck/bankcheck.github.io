...
public class Label_8220 {
	...
	public static String toXMLfile(HttpSession session, ArrayList result, String foFilePath, String footer) throws Exception {
		...
		try {
			...
			printWrite2XML.println("							page-height=\"32.0cm\"	");
			printWrite2XML.println("							page-width=\"20.2cm\"	");
			printWrite2XML.println("							margin-top=\"0cm\"	");
			printWrite2XML.println("							margin-bottom=\"0mm\"	");
			printWrite2XML.println("							margin-left=\"17mm\"	");
			printWrite2XML.println("							margin-right=\"17mm\"	>");
			...
			printWrite2XML.println("	<fo:table width=\"188mm\" table-layout=\"fixed\">	");
			printWrite2XML.println("		<fo:table-column width=\"94mm\" height=\"37.5mm\" />	");
			printWrite2XML.println("		<fo:table-column width=\"94mm\" height=\"37.5mm\" />	");
			...
			if (result.size() > 0) {
				...
				for (int i = 0; i < result.size(); i++) {
					...
					if (reportableListObject.getValue(0) != null && reportableListObject.getValue(0).length() > 0) {
						...
						labelFooter = ConstantsVariable.EMPTY_VALUE;
					}
					...
					// printWrite2XML.println("				<fo:table-cell padding=\"2pt\" border=\"1pt solid black\">	");
					printWrite2XML.println("				<fo:table-cell padding=\"2pt\" >	");
					printWrite2XML.println("					<fo:block space-after.optimum=\"" + (defaultFontSize + 7) + "pt\">	</fo:block>	");
					printWrite2XML.println("					<fo:block space-after.optimum=\"" + defaultFontSize + "pt\">" + clientName + "</fo:block>	");
					...
					printWrite2XML.println("					<fo:block space-after.optimum=\"" + (defaultFontSize - 1) + "pt\" font-size=\"6pt\" text-align=\"right\">" + labelFooter + "				</fo:block>	");
					printWrite2XML.println("					<fo:block space-after.optimum=\"" + (defaultFontSize + 7) + "pt\">	</fo:block>	");
