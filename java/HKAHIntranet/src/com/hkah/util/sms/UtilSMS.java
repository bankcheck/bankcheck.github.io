...
public class UtilSMS {
	...
	private static final String SMS_TWOUTPAT_ID = "adventisthosp_pbo_tw";	// Angie Ng
	...
	private static final String SMS_TWMKT_PW = "NTvc4-MNB-tFVX";
	private static final String SMS_HKMKT_ID = "adventisthosp_mkt_sr"; // SR - Monica Lee
	private static final String SMS_HKMKT_PW = "pQ5qf79sDLsB"; // 
	private static final String SMS_HKENDO_ID = "adventisthosp_endo";
	private static final String SMS_HKENDO_PW = "v5J4G93hFg"; // 
	...
	private static final String SMS_AMC1_ID = "adventisthosp_amc1"; // AMC1
	private static final String SMS_AMC1_PW = "Ab3-2VX-4aB3";

	private static final String SMS_AMC2_ID = "adventisthosp_amc2"; // AMC2
	private static final String SMS_AMC2_PW = "yIqYqkD05i0";
	...
	private static final String SMS_HKOPD_ID = "adventisthosp_opd_hk"; // HK COVID 19 result change to HKLAB 20200115 - Phoebe Lai OPD
	private static final String SMS_HKOPD_PW = "gW86auEZ";
	
	private static final String SMS_TWOPD_ID = "adventisthosp_opd_tw";	// Doretta Lo
	private static final String SMS_TWOPD_PW = "";
	...
	private static final String SMS_TWDENT_PW = "+SrUN6zz[hc.8A3J";
	...
	//hong kong marketing id
	public static final String SMS_HKMKT = "HKMKT";
	public static final String SMS_TWWC = "WC";
	public static final String SMS_AMC1 = "AMC1";
	...
	public static String getSMSID(String type) {
		...
		if (type.equals(SMS_INPAT)) {
			...
			smsID = SMS_TWMKT_ID;
		} else if (type.equals(SMS_HKMKT)) {  //new add for SR Marketing, don leung, 21/07/2022
			smsID = SMS_HKMKT_ID;
		} else if (type.equals(SMS_ENDO)) {
			if (ConstantsServerSide.isHKAH()) {
				smsID = SMS_HKENDO_ID;
			}
...
//20230316 Arran removed obsolete code to prevent confusion
		...
		try {
			URL url = new URL(null, SMS_STAT_LINK, new sun.net.www.protocol.https.Handler());
			HttpsURLConnection connection = (HttpsURLConnection) url.openConnection();
			connection.setDoInput(true);
			connection.setDoOutput(true);
			connection.setFollowRedirects(true);

//20230316 Arran add URLEncode password
			String data = "UserID="+getSMSID(type)+"&UserPassword="+URLEncoder.encode(getSMSPassword(type), "UTF-8");
...
//20180115 Arran edited for Patient Recall SMS status update
	...
	private static String sendSMS(String staffID, String[] receiver, String message, String smsLink,
		...
		String data = "UserID=" + smsID +
			"&UserPassword=" + URLEncoder.encode(smsPW, "UTF-8") +
			"&MessageType=" + msgType +
		...
		try {
			URL url = new URL(null, SMS_LINK, new sun.net.www.protocol.https.Handler());
			HttpsURLConnection connection = (HttpsURLConnection) url.openConnection();
			connection.setDoInput(true);
			connection.setDoOutput(true);
			...
			System.out.println("SMS Error: " + e.toString());
			EmailAlertDB.sendEmail("sms.alert", "Send SMS Error", "data:" + data + "\nmessage:" + e.toString());
		}
...
/*		try {
			...
			while ((inputLine = in.readLine()) != null) {
				output = output + inputLine;
				System.out.println("getComputerName():"+getComputerName()+" output:"+output);
			}
...
//		System.out.println("SMS Response Data: "+output);
	...
	public static String getPhoneNo2(String phoneNo, String couCode, String patNo,
		...
		if (patNo != null && patNo.length() > 0) {
			if (phoneNo != null && phoneNo.length() > 0) {
				if ((ConstantsServerSide.isHKAH() || ConstantsServerSide.isAMC2() || ConstantsServerSide.isAMC1() ) && (couCode != null && couCode.length() > 0)) {
					if (couCode.equals("852") || couCode.equals("853")) {
						if (phoneNo.length() == 8) {
							// send
							return couCode + phoneNo;
