...
public class FunctionControl extends ServerSideCallDB {

	protected String getAppendSQL() {
		// insert record
		return "INSERT INTO MT_FUNCTION (FUNCTION_ID, FUNCTION_DESC, CLASS_PACKAGE, CREATED_BY, CREATED_DATE, LAST_UPDATED_BY, LAST_UPDATED_DATE) VALUES (?, ?, ?, ?, SYSDATE, ?, SYSDATE)";
	}

	protected String getModifySQL() {
		// update record
		return "UPDATE MT_FUNCTION SET FUNCTION_DESC = ?, CLASS_PACKAGE = ?, LAST_UPDATED_BY = ?, LAST_UPDATED_DATE = SYSDATE WHERE FUNCTION_ID = ?";
	}

	protected String getDeleteSQL() {
		// delete record
		return "DELETE FROM MT_FUNCTION WHERE FUNCTION_ID = ?";
	}

	protected String getFetchSQL() {
		// list one record
		return "SELECT FUNCTION_ID, FUNCTION_DESC, CLASS_PACKAGE FROM MT_FUNCTION WHERE FUNCTION_ID = ?";
	}

	protected String getListSQL() {
		// list more records
		return "SELECT FUNCTION_ID, FUNCTION_DESC, CLASS_PACKAGE FROM MT_FUNCTION WHERE FUNCTION_ID LIKE ? ORDER BY FUNCTION_ID";
	}
	...
	protected boolean isValid(DataSource dataSource, String actionType, String[] inQueue) {
		...
		try {
			...
			ps = conn.prepareStatement("SELECT 1 FROM MT_FUNCTION WHERE FUNCTION_ID = ?");
