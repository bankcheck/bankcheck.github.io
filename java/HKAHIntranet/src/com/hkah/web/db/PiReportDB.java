...
//	private static String sqlStr_updatePIREPORT_PI_COMMENT = null;
	...
	private static String sqlStr_fetchPIREPORT_DESCRIPTION = null; // get incident Narrative/ description
...
//			IncidentClassification = getIncidentType(pirid);
		...
		if ("RemindSubmit".equals(irMode) && "0".equals(rptSts)) {
			...
			String incidentNarrative = fetchReportDescription(pirID);
			if (incidentNarrative != null) {
				contentStr.append("<br><br>Description:<br>"+incidentNarrative+"<br>");
			}
		...
		if (ConstantsServerSide.DEBUG) {
			System.err.println("send email from [" + emailSender + "] => [" + toEmail + "] about [" + emailTitle.toString() + "] content:["+contentStr.toString()+"]");
		}
...
//		sqlStr.append("WHERE P.PIR_ENABLED = 1 ");
		...
		// Get Description
		sqlStr.setLength(0);
		sqlStr.append("SELECT PI_VALUE FROM PI_REPORT_CONTENT WHERE PI_OPTION_ID IN ( ");
		sqlStr.append("SELECT PI_OPTION_ID  FROM PI_REPORT_OPTION  ");
		sqlStr.append("WHERE (UPPER(PI_OPTION_DESC) LIKE '%NARRATIVE DESCRIPTION%' OR UPPER(PI_OPTION_DESC) LIKE 'DESCRIPTION%') ");
		sqlStr.append("AND PI_OPTION_TYPE = 'textarea') AND ENABLE = 1 AND PIRID = ? ");
		sqlStr_fetchPIREPORT_DESCRIPTION = sqlStr.toString();
...
//		sqlStr.append("ORDER BY CREATE_DATE DESC");
	...
	public static String fetchReportDescription(String pirID) {
		ArrayList result =  UtilDBWeb.getReportableList(sqlStr_fetchPIREPORT_DESCRIPTION, new String[] {pirID});
		if (result.size() > 0) {
			ReportableListObject rowRtn = (ReportableListObject) result.get(0);
	...
	public static String GetNextRespPerson(UserBean userBean, String pirID, String rptSts, String incident_classification) {
		...
		System.out.println("[PiReport] GetNextRespPerson staffID="+staffID);
...
//			return true;
	...
	// get DeptHead
	// subhead act as depthead for admin office 610 (SR), Building Services and Maintenance Department 740 (SR)
	public static String getDeptHead(String deptCode, String getType) {
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("select case when cd.co_department_code in ('610', '740') then cd.co_department_subhead else cd.co_department_head end co_department_head, cs.co_staffname, trim(cd.co_department_subhead), cs2.co_staffname from co_departments cd ");
