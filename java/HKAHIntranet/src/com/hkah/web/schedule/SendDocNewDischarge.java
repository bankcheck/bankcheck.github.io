...
public class SendDocNewDischarge implements Job {
	
	private static String daysAgo = "2"; 
	static String MaxAttach = getSysParam("MaxAttach");
	
	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		System.out.println("===== SendDocNewDischarge v1.10 Start =====");
		main();
		System.out.println("===== SendDocNewDischarge End =====");
	}

	// ======================================================================
	private static ArrayList<ReportableListObject> getNewList() {
		...
		sqlStr.append(" (SELECT 1 FROM FS_FILE_INDEX F JOIN FS_FILE_PROFILE P ON F.Fs_File_Index_Id = P.Fs_File_Index_Id ");
		sqlStr.append("  WHERE TO_CHAR(F.FS_REGID) = TO_CHAR(D.REGID) AND F.FS_ENABLED = 0 AND F.FS_APPROVED_DATE IS NOT NULL AND P.FS_PAT_EMAIL_LOG_ID IS NOT NULL) ");
		sqlStr.append(" AND APPROVED_DATE < SYSDATE - ? ");
		
		return UtilDBWeb.getReportableListCIS(sqlStr.toString(), new String[] {daysAgo});
	}	
	
	// ======================================================================
	private static ArrayList<ReportableListObject> getNewDischarge(String doccode) {
		...
		return UtilDBWeb.getReportableListCIS(sqlStr.toString(), new String[] { doccode, daysAgo });
	}
...
*/
	...
	public static String sendNewDischarge (String doccode, String email) {
		return send(getNewDischarge(doccode), email, false);
	}
	...
	public static void main() {
		...
		ArrayList<ReportableListObject> record = getNewList();
...
//no log and update log id in debug mode				
				if (!debug) {
					...
					if (logId == null) {
						logId = UtilMail.insertDMSEmailLog("DOC-NDS", rptId, "FS", 
								patno, patfname, patgname, patcname, patsex,
								doccode, docfname, docgname, regid,
								email, rptDate, src, fname, pw );
					} else {
						UtilMail.attachDMSEmailLog("DOC-NDS", logId, rptId, "FS", 
								patno, patfname, patgname, patcname, patsex, 
								doccode, docfname, docgname, regid, 
								email, rptDate, src, fname, pw);
					}
				...
				if (debug)
					System.out.println("[SendDocNewDischarge] tmpPath= " + destPath);								
	...
	public static File getSmbFile ( String destination, String source ) {
		...
		try {
			...
			System.out.println("[SendDocNewDischarge] srcPath = " + source + 							
...
*/						
		...
		content.append("Attached is/are the Discharge Summary(ies) of your patient(s) who was/were discharged from the Hospital recently.<br />");
		...
		content.append("For example:  Doctor's HKID A123456(7), Doctor's Date of Birth = Jan 31, 1980<br />");
		content.append("Note: please ensure to use capital letter<br /><br />");
...
*/
	...
	private static boolean updateReportEmailStatus(String rptId, String logId) {
		...
		try {				
			...
			// TODO Auto-generated catch block
			System.out.println("[SendDocNewDischarge] DEBUG: " + sqlStr.toString() + " logId=" + logId + " rptId=" + rptId);
			e.printStackTrace();
		}
