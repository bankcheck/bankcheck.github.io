...
public class NotifySendPriceMkNoticeAMC2Job implements Job {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		sqlStr.append("AND ppn.site = 'AMC2' ");
		...
		sqlStr.append("AND ppn.tw_csr_code IS NULL ");
		sqlStr.append("AND ppn.site = 'AMC2' ");
		sqlStr.append("AND ppn.check_no IN (SELECT check_no FROM pms_price_notice WHERE send_flag = 0 AND tw_csr_code IS NULL AND site = 'AMC2') ");
		sqlStr.append("AND ppn.code_no IN (SELECT code_no FROM pms_price_notice WHERE send_flag = 0 AND tw_csr_code IS NULL AND site = 'AMC2') ");
		...
		message.append("*Item price increased in AMC2* <br><br>");
		...
		for (int i = 0; i < appList_ArrayList.size(); i++) {
			...
			message.append("Material Code: "+code+", AMC2 Bill Code: "+csrCode+", New Average Price: "+newPrice+", New Markup Price: "+newMKPrice+", Unit: "+unit+", Description: "+codeName+", Site: "+site+"<br>");
			message.append("<br>");
		}
