	...
	if (!createAction && !"1".equals(step)) {
		if (nurobDeliRmId != null && nurobDeliRmId.length() > 0) {
			...
			if (record.size() > 0) {
				...
				message = "Delivery record does not exist.";
				closeAction = true;
			}
		} else {
			closeAction = true;
...
<jsp:include page="../ui/posNegCMB.jsp" flush="false">
	<jsp:param name="posNeg" value="<%=nurobHiv %>" />
	<jsp:param name="allowEmpty" value="Y" />
</jsp:include>
						</select>
<%	} else { %>			
						<%
							if ("P".equals(nurobHiv)) {
						%>
							Positive
						<%	
							} else if ("N".equals(nurobHiv)) {
						%>
							Negative
						<%	
							}
...
<div class="pane">
	...
	<%	} else { %>
				<button onclick="return submitAction('update', 0);" class="btn-click"><bean:message key="button.update" /></button>
				<button class="btn-delete2"><bean:message key="button.delete" /></button>
	<%	}  %>
			</td>
		</tr>
	</table>
