...
<%! // Define Constants
    ...
    //static final String SECURE_SECRET = "625817C6765F0EE7AE968D523A7B41B5";
    static String vpc_secure = null;
    static String vpc_access = null;
    static String vpc_merc = null;
    static String vpc_return = null;
...
String SHAhashAllFields(Map fields) {
    ...
    byte[] mac = null;
  	try {
  		//byte [] b = new BigInteger(vpc_secure, 16).toByteArray();
  		byte []  b = fromHexString(vpc_secure, 0, vpc_secure.length());
...
// use MD5 is just for old testing, new testing and production should use SHAHash
//  ----------------------------------------------------------------------------
...
*/
	...
	//	if ("VIP".equals(roomType)) 				{ vpc_Amount = "30000"; }
	//	else if ("Private".equals(roomType)) 		{ vpc_Amount = "30000"; }
	//	else if ("Semi-Private".equals(roomType)) 	{ vpc_Amount = "20000"; }
	//	else if ("Standard".equals(roomType)) 		{ vpc_Amount = "10000"; }
	//} else {
	//	if ("VIP".equals(roomType)) 				{ vpc_Amount = "50000"; }
	//	else if ("Private".equals(roomType)) 		{ vpc_Amount = "50000"; }
	//	else if ("Semi-Private".equals(roomType)) 	{ vpc_Amount = "40000"; }
	//	else if ("Standard".equals(roomType)) 		{ vpc_Amount = "30000"; }
	//}
	
	// get params from DB
	//AdmissionDB.updatePaymentMethod(admissionID, paymentType, paymentTypeOther, creditCardType, vpc_Amount) ;
	ArrayList record = AdmissionDB.getVpcParams();
	if (record != null && record.size() > 0) {
		ReportableListObject row = (ReportableListObject) record.get(0);
		vpc_secure = row.getValue(0);
		vpc_access = row.getValue(1);
		vpc_merc = row.getValue(2);
		vpc_return = row.getValue(3);
	}

	fields.put("vpc_Version","1") ;
	fields.put("vpc_Command","pay") ;
	//fields.put("vpc_AccessCode","D0E9C019") ;
	//fields.put("vpc_Merchant","TESTHASEMIGS") ;
	fields.put("vpc_AccessCode",vpc_access) ;
	fields.put("vpc_Merchant",vpc_merc) ;
	
//20181210 Arran added for Chinese	
	if ("chi".equals(language)) {
		fields.put("vpc_Locale","zh_HK");
	} else {
		fields.put("vpc_Locale","en");
	}
			
	//fields.put("vpc_OrderInfo","hkah pop testing order") ;
	fields.put("vpc_OrderInfo","hkah pop order") ;
	fields.put("vpc_MerchTxnRef", admissionID ) ;
	
//20180816 Arran convert amount to x100 	
	fields.put("vpc_Amount", vpc_Amount + "00") ;
	
	//fields.put("vpc_ReturnURL","http://160.100.2.99:8080/intranet/registration/admission_payment_return.jsp") ;
	//fields.put("vpc_ReturnURL",vpc_return) ;
	//fields.put("vpc_ReturnURL","http://"+request.getServerName()+":"+request.getLocalPort()+"/intranet/registration/admission_payment_return.jsp" +"?language="+language ) ;
	fields.put("vpc_ReturnURL",vpc_return+language ) ;
	...
	if (vpc_secure != null && vpc_secure.length() > 0) {
	...
	System.out.println("[REG] Visa/Master payment: " + buf.toString());
