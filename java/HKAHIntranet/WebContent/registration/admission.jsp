...
Locale locale_TW = Locale.TRADITIONAL_CHINESE;
...
String hatsPatFName = null;
String hatsPatGName = null;
String hatsPatCName = null;
String hatsPatDob = null;
String hatsPatSex = null;
String hatsPatID = null;
String hatsPatDocType = null;
String hatPatientDeclare = null;
...
//20181011 Arran added for credit card payment info
String rtnCode = null;
...
// check user right
try {
	System.err.println("[step]:"+step+";[command]:"+command);
	if ("1".equals(step)) {
		if (createAction || updateAction) {
			...
			if (AdmissionDB.update(userBean, admissionID, patno,
					...
					System.out.println("DEBUG: admission.jsp(224) createAction = " + createAction+" "+mktSrc);
				if (createAction) {
					// send email notify
					...
					if (admissionID != null && !userBean.isLogin()) {
						...
						// for testing only, please active the follow code once change from development to production
			...
			System.out.println(new Date() + "[DEBUG] admission.jsp createHATS aID=<"+admissionID+">");
			...
			System.err.println("[admissionID]:"+admissionID);
			...
			System.out.println(new Date() + "[DEBUG] admission.jsp updateBedNoAction aID=<"+admissionID+">");
	...
	if (!createAction && !"1".equals(step)) {
		...
		if (admissionID != null && admissionID.trim().length() > 0) {
			...
			if (record.size() > 0) {
				...
				patidno = row.getValue(7);
				if (patidno.length() > 7) {
					int sid = patidno.indexOf('(');
					int eid = patidno.indexOf(')');
					patidno1 = patidno.substring(0, sid);
					patidno2 = patidno.substring(sid + 1, eid);
				} else {
					patidno1 = patidno;
					patidno2 = "";
				}
...
//20181011 Arran added for credit card payment info
				rtnCode = row.getValue(100);
				receiptNo = row.getValue(101);
				transNo = row.getValue(111);
				hatPatientDeclare = row.getValue(112);
				...
				if(noOfPat>0){
					...
					hatsPatFName = row2.getValue(11);
					hatsPatGName = row2.getValue(12);
					hatsPatCName = row2.getValue(4);
					hatsPatDob = row2.getValue(10);
					hatsPatSex = row2.getValue(1);
					hatsPatID = row2.getValue(2);
					hatsPatDocType = row2.getValue(19);
				}
			} else {
				closeAction = true;
			}
...
<%			if (patno == null || patno.length() == 0 || "-1".equals(patno) ) {
				System.out.println("DEBUG: admission.jsp Possible Hospital No. = click aid:"+admissionID);
%>
...
<%				} %>
			...
			<button onclick="return showDialog2('<%=patno %>');">Show Dialog</button>
<%			} %>
<%		} %>
		</td>
	</tr>
...
<%	if (createAction || updateAction) { %>
			<table width="100%">
				<tr>
					<td>
						<input type="radio" name="patidType" value="hkid"<%=patidno1 !=null && patidno1.length() > 0?" checked":"" %> onclick="validHKID();">Hong Kong I.D. Card<br />香港身份證號碼
...
<%	} else { %>
			Hong Kong I.D. Card<br />香港身份證號碼 <span class="infoResult" id="patidno"><%=patidno==null?"":patidno %></span><br /><br />
			Passport No.<br />護照號碼 <span class="infoResult" id="patpassport"><%=patpassport==null?"":patpassport %></span>
<%	} %>
		</td>
		<td width="2%" valign="top">&nbsp;</td>
		<td height="40" valign="top" bgcolor="#F9F9F9">
...
<%	} else { %>
			<span class="infoResult" id="patbdate"><%=patbdate==null?"":patbdate %></span>
<%	} %>
		</td>
	</tr>
	<tr>
...
<%	if (createAction || updateAction) { %>
			<input type="text" name="patfname" value="<%=patfname==null?"":patfname %>" maxlength="40" size="40">
<%	} else { %>
			<span class="infoResult" id="patfname"><%=patfname==null?"":patfname %></span>
<%	} %>
...
<%	if (createAction || updateAction) { %>
			<input type="text" name="patgname" value="<%=patgname==null?"":patgname %>" maxlength="40" size="40">
<%	} else { %>
			<span class="infoResult" id="patgname"><%=patgname==null?"":patgname %></span>
<%	} %>
...
<%	} else { %>
			<span class="infoResult" id="patcname"><%=patcname==null?"":patcname %></span>
<%	} %>
...
<%	} else {
			if ("M".equals(patsex)) {
				%><span class="infoResult" id="patsex"><bean:message key="label.male" /></span><%
			} else if ("F".equals(patsex)) {
				%><span class="infoResult" id="patsex"><bean:message key="label.female" /></span><%
			} else {
...
<%	if (createAction || updateAction) { %>
					<select name="mktSrc">
						<option value=""></option>
						<jsp:include page="../ui/mktDescCMB.jsp" flush="false">
						<jsp:param name="mktSrc" value="<%=mktSrc %>" />
						<jsp:param name="language" value="<%=language %>" />
						</jsp:include>
					</select>
<%	} else { %>
					<%=AdmissionDB.getMktSrcDesc(mktSrc) %>
<%	} %>
				</td></tr>
			</table>
		</td>
		...
		<td colspan="5" height="20">
			<table width="700" border="0" cellspacing="0" cellpadding="0">
				...
				<td valign="top" width="95%">
					I have read and agreed to the terms and conditions detailed on the "Daily Room Rate and Advance Payment" and then advise on the following (<a href="http://www.hkah.org.hk/new/eng/hospitalization_fi.htm" target="_blank">Click here</a>)<br />
...
<%	if (createAction || updateAction) { %>
	    	<table width="100%" border="0" cellspacing="1" cellpadding="0">
				<tr>
					<td height="30" bgcolor="#F9F9F9"><input name="paymentType" type="radio" value="UPON_ARRIVAL"<%=paymentType==null||"UPON_ARRIVAL".equals(paymentType)?" checked":"" %> />
						Upon Arrival
					</td>
				...
				<tr>
					<td height="30" bgcolor="#F9F9F9"><input name="paymentType" type="radio" value="VISA_MASTER"<%="VISA_MASTER".equals(paymentType)?" checked":"" %> />
						Online Payment - Visa/MasterCard</td>
					<td height="30" bgcolor="#F9F9F9"><input name="paymentType" type="radio" value="UNION_PAY"<%="UNION_PAY".equals(paymentType)?" checked":"" %> />
						Online Payment - UnionPay
					</td>
					<td height="30" bgcolor="#F9F9F9"><input name="paymentType" type="radio" value="CREDIT CARD"<%="CREDIT CARD".equals(paymentType)?" checked":"" %> />
						...
						<select name="creditCardType">
							...
							<option value="MasterCard"<%="MasterCard".equals(creditCardType)?" selected":"" %>>MasterCard</option>
							...
							<option value="UnionPay"<%="UnionPay".equals(creditCardType)?" selected":"" %>>UnionPay</option>
						</select>
					</td>
				</tr>
...
<%	} else {
			if ("CASH".equals(paymentType)) {
				...
				%><span class="infoResult">Upon Arrival</span><%
...
//20181011 Arran added for online admission
			} else if ("VISA_MASTER".equals(paymentType)) {
				%><span class="infoResult">Credit Card 信用卡</span><%
				if (creditCardType != null && creditCardType.length() > 0) {
					%><span class="infoResult"> - <%=creditCardType %>
					<br/>
					Payment Result - <%="0".equals(rtnCode)?"Success":"Fail" %>
					<br/>
					Credit Card Receipt No. <%=receiptNo %> Order ID / No. <%=transNo %>
					<br/>
					<a href="<%=AdmissionDB.getVpcAdminURL() %>" target="_blank"><%=AdmissionDB.getVpcAdminURL() %></a>
					</span><%
				}
			} else if ("UNION_PAY".equals(paymentType)) {
				%><span class="infoResult">Union Pay�:
				<br/>
				Payment Result - <%="00".equals(rtnCode)?"Success":"Fail" %>
				<br/>
				Credit Card Receipt No. <%=receiptNo %> Order ID / No. <%=transNo %>
				<br/>
				<a href="<%=AdmissionDB.getUpopAdminURL() %>" target="_blank"><%=AdmissionDB.getUpopAdminURL() %></a>
				</span><%
			} else if ("UPON_ARRIVAL".equals(paymentType)) {
				%><span class="infoResult">Pay upon arrival</span><%
			}
		} %>
		</td>
	</tr>
	...
	<tr>
		<td colspan="5" height="20">
			<table width="700" border="0" cellspacing="0" cellpadding="0">
				...
				<td valign="top" width="95%">
				<%=MessageResources.getMessage(locale, "adm.payMethod14")%>
				</td>
				</tr>
				<tr>
				<td valign="top" width="5%">
				...
				<td valign="top" width="95%">
				<%=MessageResources.getMessage(locale_TW, "adm.payMethod14")%>
				</td>
				</tr>
...
//				document.form1.patbdate.value = document.form1.hats_patbdate.value;
	...
	function showDialog(urlparam) {
		var windowObjectReference;
		var patno = '<%=patno%>';
		var admNo = '<%=admissionID%>';
		var strWindowFeatures = "menubar=no,location=no,resizable=no,scrollbars=no,status=no,width=600,height=500";
		var URL = "../registration/dlgPatSecCheck.jsp?command=view&patNo=" + patno+"&admNo="+ admNo + urlparam;
	  	windowObjectReference = window.open(URL, "CNN_WindowName", strWindowFeatures);
	}

	function checkModify(){
		var hatspatfname = '<%=hatsPatFName%>';
		var hatspatgname = '<%=hatsPatGName%>';
		var hatspatcname = '<%=hatsPatCName%>';
		var hatspatdob = '<%=hatsPatDob%>';
		var hatspatsex = '<%=hatsPatSex%>';
		var hatspatid = '<%=hatsPatID%>';
		var patfname = document.getElementById("patfname").innerHTML;
		var patgname = document.getElementById("patgname").innerHTML;
		var patcname = document.getElementById("patcname").innerHTML;
		var patdob = document.getElementById("patbdate").innerHTML;
		var patsex = document.getElementById("patsex").innerHTML;
		if('Male'==patsex){
			patsex = 'M';
		}else if('Female'==patsex){
			patsex = 'F';
		}
		var patidno = '<%=patidno%>';
		var patpassport = document.getElementById("patpassport").innerHTML;
		var urlparam = '';
		if (patfname.toUpperCase() != hatspatfname.toUpperCase() && patfname.length>0){
			urlparam = '&patfname='+patfname;
		}
		if(patgname.toUpperCase() != hatspatgname.toUpperCase() && patgname.length>0){
			urlparam = urlparam+'&patgname='+patgname;
		}
		if(patcname != hatspatcname && patcname.length>0){
			urlparam = urlparam+'&patcname='+patcname;
		}
		if(patdob != hatspatdob && patdob.length>0){
			urlparam = urlparam+'&patdob='+patdob;
		}
		if(patsex != hatspatsex && patsex.length>0){
			urlparam = urlparam+'&patsex='+patsex;
		}
		if(patidno!=null&&patidno.length>0){
			if(patidno != hatspatid && patidno.length>0){
				urlparam = urlparam+'&patidno='+patidno;
			}
		}else if(patpassport!=null&&patpassport.length>0){
			if(patpassport != hatspatid && patpassport.length>0){
				urlparam = urlparam+'&patpassport='+patpassport;
			}
		}
		return urlparam;
	}
	...
	function showDialog2(patno) {
		...
		var rtnvalue = true;
		var urlparam = '';
		if (patfname.toUpperCase() != hatspatfname.toUpperCase()){
			urlparam = '&patfname='+patfname;
			rtnvalue = false;
		}
		if(patgname.toUpperCase() != hatspatgname.toUpperCase()){
			urlparam = urlparam+'&patgname='+patgname;
			rtnvalue = false;
		}
		if(patcname != hatspatcname){
			urlparam = urlparam+'&patcname='+patcname;
			rtnvalue = false;
		}
		if(patdob != hatspatdob){
			urlparam = urlparam+'&patdob='+patdob;
			rtnvalue = false;
		}
		if(patsex != hatspatsex){
			urlparam = urlparam+'&patsex='+patsex;
			rtnvalue = false;
		}
		if(patidno!=null){
			if(patidno != hatspatid){
				urlparam = urlparam+'&patidno='+patidno;
				rtnvalue = false;
			}
		}else if(patpassport!=null){
			if(patpassport != hatspatid){
				urlparam = urlparam+'&patpassport='+patpassport;
				rtnvalue = false;
			}
		}
