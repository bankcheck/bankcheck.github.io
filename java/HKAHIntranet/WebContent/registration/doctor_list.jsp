...
if (ConstantsServerSide.isTWAH()) {
	...
	sqlStr.append("        TO_CHAR(SH.SCHEDATE + 1/1440, 'HH24:MI') ETIME ");
	...
	// M := Courtesy
	sqlStr.append(" AND   (DR.DOCTYPE != 'M' OR DR.DOCCODE IN (SELECT HPKEY FROM HPSTATUS@HAT WHERE HPTYPE = 'ONLINE_DR' AND HPSTATUS = 'INCLUDE')) ");
	...
	sqlStr.append("        DOCCODE, SPCNAME_E, SPCNAME_C, ");
	sqlStr.append("        DOCFNAME, DOCGNAME, ");
	sqlStr.append("        DOCCNAME, ");
	sqlStr.append("        ECERT1, ECERT2, ECERT3, ECERT4, ECERT5, ");
	sqlStr.append("        ECERT6, ECERT7, ECERT8, ECERT9, ECERT10, ");
	sqlStr.append("        CCERT1, CCERT2, CCERT3, CCERT4, CCERT5, ");
	sqlStr.append("        CCERT6, CCERT7, CCERT8, CCERT9, CCERT10, ");
	sqlStr.append("        DOCTYPE, SPCCODE, DOCSDATE, ");
	sqlStr.append("        TITTLE, ");
	sqlStr.append("        SDATE, ");
	sqlStr.append("        STIME, ");
	sqlStr.append("        ETIME ");
	sqlStr.append(" FROM   cis.dr_schedule ");
	sqlStr.append("	ORDER BY SPCNAME_E, DOCCODE, SDATE, STIME ");
} else {
	sqlStr.append("SELECT DISTINCT ");
	sqlStr.append("        DR.DOCCODE, CIS.F_GET_ENG_SPCNAME(DR.DOCCODE), CIS.F_GET_CHN_SPCNAME(DR.DOCCODE), ");
	sqlStr.append("        DR.DOCFNAME, DR.DOCGNAME, ");
...
System.out.println("[doctor_list] sql="+sqlStr.toString());
...
System.out.println("[doctor_list] record size="+record.size());
...
<%
		...
		sameDate = docCode_curr.equals(docCode_prev) ? (dateStr != null && dateStr.equals(row.getValue(30))) : false;
...
dateStr = row.getValue(30);
			...
			dateStr = null;	// no schedule for this doc, reset
		}
...
%>
	<%
	if (!docCode_curr.equals(docCode_prev)) {
		dateStr = null;
	}
	%>
