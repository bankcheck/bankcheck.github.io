...
//20181203 added for "How did you hear about us"
String mktSrc = null;
String pathtel = null;
String patotel = null;
String patmtel = null;
...
<form name="form1" enctype="multipart/form-data" action="admission_client_submit_vpc.jsp?language=<%=language %>" method="post">
<table width="900" border="0" cellpadding="0" cellspacing="0">
	<tr>
	...
	<tr>
		...
		<td height="40" width="40%" valign="top" >
			<font color="red">*</font><div id="admDoctor" style="width:400px;"><%=MessageResources.getMessage(session, "adm.admDoctor") %>
			<span id="search_span">
				<button type="button" onclick="clickSearch();return false;"><%=MessageResources.getMessage(session, "button.search") %></button>
			</span>

			</div>
			<span id="doc_span">
			<select name="admissiondoctor" id="admissiondoctor" style="width:400px;">
				<option value=""></option>
...
<table width="900" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td colspan="5" height="25" style="background-color:#aa0066;">
			<table width="686" border="0" align="left" cellpadding="0" cellspacing="0">
				<tr>
	...
	<tr>
		...
		<td width="32%" height="40" valign="top" >
			...
			<select name="titleDesc">
				...
				<option value="MRS" <%=("MRS.".equals(titleDesc) || "MRS".equals(titleDesc))?" selected":"" %>><%=MessageResources.getMessage(session, "label.mrs") %></option>
...
<table id="optionalPersonalInformation"cellpadding="1" border="0">
	<tr>
		...
		<td height="40" valign="top" ><font color="red">*</font>
			...
			<select name="patmsts">
				<option value=""></option>
				<option value="OT" <%="OT".equals(patmsts)?" selected":"" %>><%=MessageResources.getMessage(session, "label.others") %></option>
...
<table width="900" border="0" cellpadding="0" cellspacing="0" id="optionalInformation1">
	<tr>
		<td height="20" colspan="5">&nbsp;</td>
	</tr>
	<tr >
...
<table width="900" border="0" cellpadding="0" cellspacing="0" id="optionalInformation2">
	<tr>
		<td height="20" colspan="5">&nbsp;</td>
	</tr>
	<tr>
...
$(document).ready(function() {
	...
	function submitAction() {
		...
		if (document.form1.patmsts.value == '' && document.form1.patmstsOther.value == '') {
		...
		if (document.form1.patmsts.value == 'OT' && document.form1.patmstsOther.value == '') {
			if (msg.length == 0) focusField = document.form1.patmsts;
			$(document.form1.patmsts).parent().find('div#patmsts').addClass("hightLight");
			msg = msg + (count++) + '. ';
			if (document.form1.language.value =='chi'){
...
//20181203 Arran added "How did you hear about us"
	...
	function search() {
		var name = encodeURIComponent(document.form1.docSearch.value);
		var code = document.form1.admissiondoctor.value;

		http.open('get', '../ui/docCodeCMB.jsp?selectFrom=Pre-addmission&docname=' + name + '&doccode=' + code + '&timestamp=<%=(new java.util.Date()).getTime() %>');

		//assign a handler for the response
		http.onreadystatechange = processResponseDocname;
	...
	function processResponseDocname() {
		//check if the response has been received from the server
		if (http.readyState == 4){
			//read and assign the response from the server
			var response = http.responseText;

			document.getElementById("doc_span").innerHTML = '<select name="admissiondoctor" id="admissiondoctor" style="width:400px;" onclick="clickDocSelect()">' + response + '</select>';
			showDocList();
		}
	}
	...
	function clickDocSelect() {
		hideDocList();
		return false;
	}

	function showDocList() {
		var list = document.getElementById("admissiondoctor");
		list.size = 5;
		return false;
	};
	...
	function showSearchText() {
		document.getElementById("search_span").innerHTML = '<input type="text" id="docSearch" name="docSearch" value="" maxlength="20"	size="25" onkeyup="search()"><img src="../images/search.gif" onclick="clickSearch()"/>';
		document.form1.docSearch.value = searchText;
		//search();
	...
	function hideSearchText() {
		searchText = document.form1.docSearch.value;
		document.getElementById("search_span").innerHTML = '<button type="button" onclick="clickSearch();return false;"><%=MessageResources.getMessage(session, "button.search") %></button>';
		hideDocList();
		showSearchFlag = true;
		return false;
	}
