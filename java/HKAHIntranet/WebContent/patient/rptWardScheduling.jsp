...
if (currDate == null || currDate.length() == 0) {
	currDate = DateTimeUtil.getCurrentDate();
}
...
if("view".equals(command)){
	ArrayList wardRecord = PatientDB.getWardScheduling(currDate);

	if(wardRecord.size()>0){
		reportFile = new File(application.getRealPath("/report/RPT_WARD_SCHEDULING.jasper"));
		...
		if (reportFile != null && reportFile.exists()) {
			...
			parameters.put("currDate",currDate);
			System.err.println("[reportMth]:"+currDate);
			JasperPrint jasperPrint =
				JasperFillManager.fillReport(
					jasperReport,
					parameters,
					new ReportMapDataSource(wardRecord, new String[]{"WRDCODE"
																		,"A1_1","A1_2","A1_3","A1_4"
																		,"P1_1","P1_2","P1_3","P1_4"
																		,"N1_1","N1_2","N1_3","N1_4"
																		,"A2_1","A2_2","A2_3","A2_4"
			...
			response.setHeader("Content-Disposition", "attachment;filename=WardScheduling_" + tsFormat.format(new Date()) + ".xls");
		...
		<div id=indexWrapper>
			<div id=mainFrame>
				<jsp:include page="../common/page_title.jsp" flush="false">
					<jsp:param name="pageTitle" value="function.WardScheduling.report" />
				...
				<form name="searchPbForm" action="rptWardScheduling.jsp" method="get">
					...
					<table cellpadding="0" cellspacing="5" class="contentFrameSearch" border="0">
						<tr class="smallText">
							...
							<td class="infoData" width="35%">
								<input type="textfield" name="currDate" id="currDate" value="<%=currDate==null?"":currDate %>" maxlength="10" size="10" onkeyup="validDate(this)" onblur="validDate(this)" disabled/>
							</td>
							<td width="20%">
				...
				<script language="javascript">				
					function submitReport(cmd) {
						...
						var currDate = document.searchPbForm.currDate.value;
						if(currDate.length!=10){
							alert("Please enter valid date");							
							
							document.searchPbForm.currDate.focus();
							return false;							
						}else{
					...
					function clearSearch() {
						document.searchPbForm.currDate.value="";
					}				
				</script>
			</div>
		</div>
