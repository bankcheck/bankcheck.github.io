...
String stnCDateFrom = request.getParameter("date_from");
String stnCDateTo = request.getParameter("date_to");
...
String dateRange = stnCDateFrom+" to "+stnCDateTo;

if("view".equals(command)){
	ArrayList TreatmentRecord = PatientDB.getPatTreatByPatient(patNo, stnCDateFrom, stnCDateTo);

	if(TreatmentRecord.size()>0){
		reportFile = new File(application.getRealPath("/report/RptTreatByPatient.jasper"));
		...
		if (reportFile != null && reportFile.exists()) {
			...
			parameters.put("WRDCODE",wrdCode);
			
			JasperPrint jasperPrint =
				JasperFillManager.fillReport(
					jasperReport,
					parameters,
					new ReportMapDataSource(TreatmentRecord, new String[]{"PATNO","PATNAME","ITMCODE","ITMNAME","AMT","COUNT_TREAT"},
					null));	
			...
			response.setHeader("Content-Disposition", "attachment;filename=TreatmentBreakdownByPat_" + tsFormat.format(new Date()) + ".xls");
		...
		<div id=indexWrapper>
			<div id=mainFrame>
				<jsp:include page="../common/page_title.jsp" flush="false">
					<jsp:param name="pageTitle" value="function.treatmentBrkDwnByPat.report" />
				...
				<form name="searchPbForm" action="rptTreatByPatient.jsp" method="get">
					<table cellpadding="0" cellspacing="5" class="contentFrameSearch" border="0">
						<tr class="smallText">					
							<td class="infoLabel" width="15%">
								<bean:message key="prompt.patientNo" />
							</td>
							<td class="infoData" width="35%">
								<input type="textfield" name="patNo" id="patNo" class="textfield" value="<%=patNo==null?"":patNo %>" maxlength="10" size="10"/>
							</td>							
						</tr>													
						<tr class="smallText">					
							<td class="infoLabel" width="15%">
							...
							<td class="infoData" width="35%">
								<input type="textfield" name="date_from" id="date_from" 
									class="datepickerfield" value="<%=stnCDateFrom==null?"":stnCDateFrom %>" maxlength="10" size="10" onkeyup="validDate(this)" onblur="validDate(this)"/>
								-
								<input type="textfield" name="date_to" id="date_to" 
									class="datepickerfield" value="<%=stnCDateTo==null?"":stnCDateTo %>" maxlength="10" size="10" onkeyup="validDate(this)" onblur="validDate(this)"/> (DD/MM/YYYY)
							</td>
							<td width="20%">
