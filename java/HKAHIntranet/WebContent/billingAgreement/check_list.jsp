...
//new data for able site filtering, don leung, 09/11/2022
...
if(recordSite.size() > 0){
	site = new String[recordSite.size()];
	for(int i = 0; i < recordSite.size();i ++){		
		ReportableListObject row = (ReportableListObject) recordSite.get(i);
		site[i] = ParserUtil.getParameter(request, row.getValue(1));
		System.out.println("position " + i + " " + site[i]);
	}
}

request.setAttribute("check_list", BillingAgreementDB.getList(
		userBean, moduleCode, status, searchBy, searchContent,
		contractDateFrom, contractDateTo, deptCodeRequest, deptCodeResponse, finalAssoDeptCodeRequests, deptCode,addDeptCode, site));
...
<jsp:include page="../ui/deptCodeCMB.jsp" flush="false">
	...
	<!-- site field for filter, don leung, 09/11/2022 -->
	<tr class="smallText">
		<td class="infoLabel" width="30%">Site</td>
		<td class="infoData" width="70%">
			<%
				if(recordSite.size() > 0){
					for(int i = 0; i < recordSite.size();i ++){
    					...
    					if(ParserUtil.getParameter(request, siteDesc) != null){%>
    						<%=siteDesc %>  <input type="checkbox" id="<%=siteDesc %>" name="<%=siteDesc %>" value="<%=siteDesc %>" checked>
    					<%}else{%>
    						<%=siteDesc %>  <input type="checkbox" id="<%=siteDesc %>" name="<%=siteDesc %>" value="<%=siteDesc %>" >
    					<%}
    					%>
...
<%	} else { %>
<%
		String deptDescRequest = rlo == null ? null : 
			(rlo.getFields19() == null || rlo.getFields19().isEmpty() ? rlo.getFields10() : rlo.getFields19());
...
%>
		<div><%=row2.getFields18() == null || row2.getFields18().isEmpty() ? row2.getFields11() : row2.getFields18() %></div>
<%
		}
%>
	</display:column>
...
<%
 		for (int i = 0; i < assoDeptRequestList.size(); i++) {
			row2 = (ReportableListObject) assoDeptRequestList.get(i);
%>
		<%=row2.getFields18() == null || row2.getFields18().isEmpty() ? row2.getFields11() : row2.getFields18() %>
<%
		}
%>
	</display:column>
	<display:column title="Responsible Department" style="width:10%">
		<%=rlo.getFields20() == null || rlo.getFields20().isEmpty() ? rlo.getFields11() : rlo.getFields20() %>
	</display:column>
