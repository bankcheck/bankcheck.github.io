...
page import="com.hkah.web.db.*"%><%!
	private ArrayList<ReportableListObject> fetchNewCase(boolean filter) {
		StringBuffer strbuf = new StringBuffer();

		strbuf.append("SELECT 1 ");
		...
		strbuf.append("AND Q.PH_DISPENSING_DATE IS NOT NULL ");
		strbuf.append("AND Q.PH_DISPENSING_DATE > SYSDATE - 1/8640 "); // 6 secs
	...
	private ArrayList<ReportableListObject> fetchQueue(boolean filter) {
		...
		strbuf.append("AND Q.PH_TICKET_DT = TO_CHAR(SYSDATE, 'YYYYMMDD') ");
		strbuf.append("AND Q.PH_DISPENSING_DATE IS NOT NULL ");
		strbuf.append("AND (Q.PH_CHARGED_DATE IS NULL OR Q.PH_CHARGED_DATE > SYSDATE - 3/1440) "); // 3 minutes
		strbuf.append("ORDER BY Q.PH_CREATED_DATE ASC, Q.PH_TICKET_QUEUE_ID ASC ");
...
ArrayList<ReportableListObject> record2 = fetchNewCase(filter);
...
<td class='box' style='background-color:<%=bgcolor %>' id='<%=ticketNo %>-all' width='<%=100/totalRecPerColumn %>%' height='160' align='center' valign='middle' class='box' onclick="performAction('<%=locID %>-<%=ticketNo.substring(4) %>', '<%=ticketNo %>');">
	<table width='100%' border='0' cellspacing='0' cellpadding='0'>
		...
		<tr>
			<td colspan="4">&nbsp;</td>
			<td colspan="4"><center><font color='red' SIZE='5'><%=isOnHold?"Processing":"&nbsp;" %></font></center></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
