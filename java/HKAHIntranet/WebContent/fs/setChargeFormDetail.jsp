...
if ("save".equals(command) && "A".equals(reqStatus) && "AY".equals(smtd)) {	
	updateAction = true;
}else if ("save".equals(command) && "B".equals(reqStatus) && "BY".equals(smtd)) {		
	submitAction = true;
...
try {
	if(updateAction){
		successUpt = FsDB.updateMenu( reqNo, "M", menu, specReq, mealID, noOfPerson, estAmount, null, null, null, mealEvent, eventID, venue, purpose, otherMeal, chargeAmount, userBean);
	    if (successUpt){
			message = "Requisition update success";
			updateAction = false;		    	   
	    }else{
	    	message = "Requisition update fail.";
			updateAction = false;
	    }
		...
		if("2".equals(requestType)){
			...
			try {
				totalAmount = Integer.parseInt(noOfPerson) * Integer.parseInt(estAmount);
			} catch (Exception e) {
				totalAmount = 0;
			}			
		}
		...
		successSubmit = FsDB.updateMenu( reqNo, "B", menu, specReq, mealID, noOfPerson, chargeAmount, null, null, null, mealEvent, eventID, venue, purpose, otherMeal, chargeAmount, userBean);
System.err.println("B");
	    if (successSubmit){
	    	...
	    	message = "Requisition Submit fail.";
			updateAction = false;
	    }
...
System.err.println("[command]:"+command+";[totalAmount]:"+totalAmount+";[chargeAmount]:"+chargeAmount);			
			recordMiscItem = FsDB.getMiscItemDtl(reqNo);
			for(int i=0;i<recordMiscItem.size();i++) {
				rowMiscItem = (ReportableListObject) recordMiscItem.get(i);
				checkNo[i] = rowMiscItem.getValue(1);
	...
	System.err.println("[chargeToName]:"+chargeToName+";[chargeTo]:"+chargeTo);
} catch (Exception e) {
	e.printStackTrace();
}
...
<form name="form2" id="form2" enctype="multipart/form-data" action="setChargeFormDetail.jsp" method=post >
...
<%} %>			
	...
	<tr class="smallText">	
		...
		<td class="infoData2" width="80%" colspan=3><input type="textfield" name="chargeTo" value="<%=chargeToName==null?"":chargeToName %>" maxlength="40" size="40" readonly="readonly"/></td>
	</tr>
<%if(otherChg!=null && otherChg!=""){ %>		
	<tr class="smallText">
		<td class="infoLabel" width="20%"><bean:message key="prompt.otherReq" /></td>
		<td class="infoData2" width="80%" colspan=3><input type="textfield" name="prompt.otherChg" value="<%=otherChg==null?"":otherChg %>" maxlength="60" size="60" readonly="readonly"/></td>
	</tr>				
<%} %>	
...
<%if(purpose!=null && purpose.length()>0){%>
	<tr class="smallText">	
		<td class="infoLabel" width="20%"><bean:message key="prompt.otherEvent" /></td>
		<td class="infoData2" width="80%" colspan=3>
			<input type="textfield" name="purpose" value="<%=purpose==null?"":purpose %>" maxlength="150" size="125" readonly="readonly"/>	
		</td>	
	</tr>	
<%	}%>			
	<tr class="smallText">
...
<%if(otherMeal!=null && otherMeal.length()>0){%>
	<tr class="smallText">	
		<td class="infoLabel" width="20%"><bean:message key="prompt.otherMeal" /></td>
		<td class="infoData2" width="80%" colspan=3>
			<input type="textfield" name="otherMeal" value="<%=otherMeal==null?"":otherMeal %>" maxlength="150" size="120"/>	
		</td>	
	</tr>	
<%	}%>	
<%if(purpose!=null && purpose.length()>0){%>
 ...
 -->				
	...
	<table border="1|0">
		...
		<tr class="smallText">
			...
			<td class="infoData2" width="18%">
				<%=checkDesc[10]==null?"":checkDesc[10] %>																											
			</td>
			<td class="infoData2" width="14%">		
			...
			<td class="infoData2" width="18%">
				<%=checkDesc[11]==null?"":checkDesc[11] %>																											
			</td>
			<td class="infoData2" width="14%">		
...
<%} %>
	<tr class="smallText">		
		...
		<td class="infoData2" width="80%" colspan=3>
<%if("A".equals(reqStatus) || "M".equals(reqStatus)){ System.err.println("[chargeAmount]:"+chargeAmount+";[totalAmount]:"+totalAmount);%>		
			$&nbsp<input type="textfield" name="chargeAmount" value="<%=chargeAmount==null||"0".equals(chargeAmount)?totalAmount:chargeAmount %>" maxlength="10" size="10"/>
<%}else{%>
...
<table cellpadding="0" cellspacing="5"
	...
	<tr class="smallText">
		<td align="center">
<%if("A".equals(reqStatus) || "M".equals(reqStatus)){ %>		
			<button onclick="return submitAction('<%=reqNo==null?"":reqNo %>','A','save');"><bean:message key="button.save" /></button>

			<button onclick="return submitAction('<%=reqNo==null?"":reqNo %>','B','save');"><bean:message key="button.saveCharge" /></button>
...
$(document).ready(function() {
	...
	function submitAction(reqNo, reqStatus, cmd) {
		...
		if(reqStatus=='A'){
			...
			var r=confirm("Confirm to submit charge?");
