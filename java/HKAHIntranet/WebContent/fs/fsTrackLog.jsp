...
ArrayList al_fs = FsDB.getTrackLog(reqNo);
request.setAttribute("FS",al_fs);
int fsSize = al_fs.size();
%>
...
<display:table id="resList" name="FS"  class="tablesorter">	     
	<display:column title="&nbsp;" media="html" style="width:2%"></display:column>	   
	<display:column style="width:10%; text-align:center" title="Record Status">	
		<div>${FS[resList_rowNum - 1].fields0}</div>
    </display:column>    				           	
    <display:column  style="width:10%; text-align:left" title="Update By">
		<div>${FS[resList_rowNum - 1].fields1}</div>
	</display:column>
    <display:column  style="width:10%; text-align:left" title="Update Date">
