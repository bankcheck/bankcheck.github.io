...
<title>OT Procedure Census</title>
<body bgcolor=#F9F3F6>
<jsp:include page="../common/page_title.jsp" flush="false">
	<jsp:param name="pageTitle" value="function.mis.census.ot" />
</jsp:include>
...
int cntVal1, cntVal2, lastMth=0, varMonth, varYear;

DecimalFormat pctFormat = new DecimalFormat();
pctFormat.applyPattern("0.00%");

// ======list of year ============
sql = " select distinct to_char(otldate,'yyyy') from ot_log@hat order by 1 desc ";
...
<!-- right area, data -->
  <td><br>
    <p align=center>
       ...
       <b>Operating Room Procedure Census (Exclude Endoscopy)</b></font>
    </p>
    ...
    <table align=center width=90% border=1 cellpadding="0" cellspacing="0">
      <tr>
       ...
       <th width=180>&nbsp;<%=prevYear%><br>
           <table width=90% border=0><tr><td align=right>Count<td align=right>YTM</tr></table></th>
...
sql = " select count(*) as cnt from ot_log@hat a, ot_proc@hat p, ot_log_proc@hat l, ot_proc@hat p2  ";
sql += "  where a.otpid=p.otpid and a.otlid=l.otlid(+) and l.otpid=p2.otpid(+) ";
sql += "    and otlsts<>'X' and nvl(p2.otpcode,p.otpcode) not like 'EU%' ";
sql += "    and to_char(otldate,'yyyymm')='"+prevMth+"' ";
...
sql += " from (select to_char(otldate,'yyyymm') as mth ";
sql += "            , to_char(otldate,'MON','NLS_DATE_LANGUAGE = AMERICAN') as mon ";
sql += "            , count(*) as cnt  ";
sql += "         from ot_log@hat a, ot_proc@hat p, ot_log_proc@hat l, ot_proc@hat p2  ";
sql += "        where a.otpid=p.otpid and a.otlid=l.otlid(+) and l.otpid=p2.otpid(+) ";
sql += "          and nvl(p2.otpcode,p.otpcode) not like 'EU%' and otlsts<>'X' ";
sql += "         and to_char(otldate,'yyyy') in ('"+prevYear+"','"+thisYear+"') ";
sql += "         and (to_char(sysdate,'yyyy')>'"+thisYear+"' or to_char(otldate,'mm')<to_char(sysdate,'mm')) ";
sql += "       group by to_char(otldate,'yyyymm'), to_char(otldate,'MON','NLS_DATE_LANGUAGE = AMERICAN')  ";
sql += "       ) temp  ";
...
while (rs.next()) {
   ...
   out.println( "  <td width=50% align=right>"+(cntVal1==0?"&nbsp;":cntVal1)+"</td>");
   out.println( "  <td width=50% align=right>"+(cntVal1==0?"&nbsp;":acc1)+"</tr></table></td>" );
   
   out.println( "<td><table width=90% align=center border=0><tr>");
   out.println( "  <td width=50% align=right>"+(cntVal2==0?"&nbsp;":cntVal2)+"</td>");
   out.println( "  <td width=50% align=right>"+(cntVal2==0?"&nbsp;":acc2)+"</tr></table></td>" );
   ...
   if ( cntVal2>0  ) {
     ...
     out.println( "<td width=50% align=right>"+pctFormat.format( 1.00 * varYear/cntVal1)+"</tr></table></td>" );
   } else {
