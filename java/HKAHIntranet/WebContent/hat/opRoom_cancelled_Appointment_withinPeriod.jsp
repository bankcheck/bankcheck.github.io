...
sqlStr.append("SELECT TO_CHAR(OTA.OTAOSDATE,'dd/MM/YYYY'), inpatReg.PATNO, ");
sqlStr.append("  	  OTA.OTAFNAME || ' ' || OTA.OTAGNAME,");
sqlStr.append("       OTA.DOCFNAME || ' ' || OTA.DOCGNAME, ");
sqlStr.append("OTA.Cancelrsncde, OTA.Otacnclrmk  ");
sqlStr.append("  FROM (SELECT * FROM OT_APP@IWEB OT ");
sqlStr.append("LEFT JOIN ");
sqlStr.append("DOCTOR@IWEB DR ON OT.DOCCODE_S = DR.DOCCODE ");
sqlStr.append("OR ");
sqlStr.append("OT.DOCCODE_E = DR.DOCCODE ");
sqlStr.append(" Where Ot.Otasts = 'C')OTA ");
sqlStr.append(" LEFT JOIN(SELECT R.Patno, R.Regdate, I.Inpddate, R.Inpid  ");
sqlStr.append("  FROM Reg@Iweb R,Inpat@Iweb I WHERE R.Inpid = I.Inpid)inpatReg ");
sqlStr.append(" ON inpatReg.Patno = OTA.Patno ");
sqlStr.append(" WHERE inpatReg.Regdate<= OTA.Otaosdate ");
sqlStr.append(" AND (inpatReg.Inpddate >= OTA.Otaosdate or inpatReg.inpddate is null) ");
if (date_from != null && date_from.length() > 0) {
	sqlStr.append("AND OTA.OTAOSDATE>= TO_DATE('");
...
sqlStr.append("ORDER BY OTA.OTAOSDATE, OTA.PATNO");
...
<form name="search_form" action="opRoom_cancelled_Appointment_withinPeriod.jsp" method="post" target="_blank">
