...
ArrayList record = PiReportDB.fetchReportDetail(grpID, parentID, pirID, editAction);
...
if(record.size() > 0) {
	...
	for(int i = 0; i < record.size(); i++) {
		...
		boolean editHasValue = (editAction && (row.getValue(11).length() > 0));
		...
		if(printCol == 0 && !optType.equals("title")) {
			if(parentCol != null)
			...
			if(optType.equals("checkbox")) {
				...
				%>
					<span optType='checkbox' category='<%=row.getValue(1) %>' 
						id='<%=row.getValue(0) %>' name='<%=row.getValue(10) %>'>
						&nbsp;<%=row.getValue(3) %>
					</span></td>
				<%
				}
				...
				%>
					...
					<input type='text' name='<%=row.getValue(0) %>_Input' value='<%=(editHasValue)?row.getValue(11):"" %>' style='width:40%'/></td>
				<%
				}
			}
				...
				if(viewAction) {
				%>
					<b><%=row.getValue(3) %></b>&nbsp;<%=(row.getValue(11).equals("Y")?"Yes":(row.getValue(11).equals("Y")?"No":"")) %>
					</td>
				<%
				}
				else {
				%>
					...
					id='<%=row.getValue(0) %>' name='<%=row.getValue(3) %>' 
					parent='<%=row.getValue(10) %>' value='Y' <%=(editHasValue)?(row.getValue(11).equals("Y")?"checked contentId='"+row.getValue(13)+"'":""):"" %>/>Yes 
					&nbsp;<input optType='yesNo' type='radio' category='<%=row.getValue(1) %>' 
					id='<%=row.getValue(0) %>' name='<%=row.getValue(3) %>' 
					parent='<%=row.getValue(10) %>' value='N' <%=(editHasValue)?(row.getValue(11).equals("N")?"checked contentId='"+row.getValue(13)+"'":""):"" %>/>No</td>
				<%
				}
				printCol = col-1;
			}
				...
				%>
					<input optType='input' style='width:70%' type='text' 
						category='<%=row.getValue(1) %>' id='<%=row.getValue(0) %>' <%=(editHasValue)?("contentId='"+row.getValue(13)+"'"):""%> 
				...
				%>
					...
					<textarea optType='textarea' category='<%=row.getValue(1) %>' 
						id='<%=row.getValue(0) %>' parent='<%=row.getValue(10) %>' <%=(editHasValue)?("contentId='"+row.getValue(13)+"'"):""%> 
				...
				%>
					<div optType='upload' class='upload' id='<%=row.getValue(0) %>' 
				...
				%>
					<div optType='upload' class='upload' id='<%=row.getValue(0) %>' 
						parent='<%=row.getValue(10) %>' category='<%=row.getValue(1) %>'> 
						<%=row.getValue(3) %>
						<input type='file' category='<%=row.getValue(1) %>' name='<%=row.getValue(0) %>' 
				...
				%>
					<input optType='checkboxInput' type='checkbox' category='<%=row.getValue(1) %>' 
						id='<%=row.getValue(0) %>' name='<%=row.getValue(10) %>' <%=(editHasValue)?("checked contentId='"+row.getValue(13)+"'"):"" %>/>&nbsp;<%=inputBefore %>
					<input style='width:20%' type='text' category='<%=row.getValue(1) %>' 
						id='<%=row.getValue(0) %>' parent='<%=row.getValue(10) %>' value='<%=(editHasValue)?row.getValue(11):""%>'/><%=inputAfter %></td>
				<% 
				}
			}
				...
				%>
					<input optType='radio' type='radio' category='<%=row.getValue(1) %>' id='<%=row.getValue(0) %>'
						name='<%=row.getValue(10) %>' <%=(editHasValue)?("checked contentId='"+row.getValue(13)+"'"):"" %>/>&nbsp;<%=row.getValue(3) %></td>
				<%
				}
			}
				...
				%>
					...
					<input optType='date' type='textfield' category='<%=row.getValue(1) %>' 
						optid='<%=row.getValue(0) %>' id='<%=row.getValue(0) %>' class='datepickerfield' <%=(editHasValue)?("contentId='"+row.getValue(13)+"'"):""%> 
				...
				%>
					<%=row.getValue(3) %>
					<input optType='datetime' type='textfield' category='<%=row.getValue(1) %>' id='<%=row.getValue(0) %>' 
						optid='<%=row.getValue(0) %>' class='datepickerfield' parent='<%=row.getValue(10) %>' value='<%=(editHasValue)?row.getValue(11).split(" ")[0]:""%>' 
						maxlength='16' size='16' <%=(editHasValue)?("contentId='"+row.getValue(13)+"'"):"" %>/>
					<jsp:include page='../ui/timeCMB.jsp' flush='false'>
						<jsp:param name='label' value='<%=row.getValue(0) %>' />
				...
				%>
					<div style="float:left;" id="addImage">
						...
						<div id='template<%=row.getValue(0) %>' style='display:none'>
