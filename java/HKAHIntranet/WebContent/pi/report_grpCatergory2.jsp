...
if ("Y".equals(group)) {
	...
	<%
		for (int i = 0; i < record.size(); i++) {
			...
			%>
				<div class="reportItem ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"
					grpID='<%=row.getValue(0) %>'>
					<%=row.getValue(2) %><span class="alert"></span>
				</div>
				<div id="<%=row.getValue(0)%>" categroy="<%=row.getValue(3)%>" class="reportContent content-frame" style="display:none">
					<div style="background-color:#C00000;color:white!important;">
	...
	%>
		...
		<div id="<%=categoryID%>" categroy="<%=category%>" class="reportContent content-frame" style="display:none">
			<div style="background-color:#C00000;color:white!important;">
