		...
		if (optType.equals("title")) {
			...
			if (optType.equals("checkbox")) {
				...
				%>
					...
					<input type='text' name='<%=row.getValue(0) %>_Input' value='<%=(editHasValue)?row.getValue(11):"" %>' style='width:50%'/></td>
				<%
				}
			}
			...
			printCol = col-1;
				...
				%>
					<input optType='checkboxDate' type='checkbox' category='<%=row.getValue(1) %>'
						id='<%=row.getValue(0) %>' name='<%=row.getValue(10) %>'
						<%=(editHasValue)?("checked contentId='"+row.getValue(13)+"'"):"" %>/>&nbsp;<%=row.getValue(3) %>
					<input type='textfield' category='<%=row.getValue(1) %>' id='<%=row.getValue(0) %>'
						optid='<%=row.getValue(0) %>' class='datepickerfield' value='<%=(editHasValue)?row.getValue(11):""%>' maxlength='16' size='16'></td>
				<%
				}
			}
				...
				<%
					if (pirID != null) {
				%>
									<jsp:include page="../pi/report_details2.jsp" flush="false">
