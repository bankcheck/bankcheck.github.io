...
function webViewerInitialized() {
  ...
  console.log('[webViewerInitialized] file='+file);
  ...
  if (file) {
    ...
    var getFileServiceUrl = '/intranet/GetFile?url='+ encodeURIComponent(file);

    xhr.onload = function() {
      //PDFViewerApplication.open(new Uint8Array(xhr.response));
      PDFViewerApplication.open(convertDataURIToBinary(xhr.response));
    };
    
    console.log('Get file by ajax url='+getFileServiceUrl);
    
    try {
      //xhr.open('GET', file);
      xhr.open('GET', getFileServiceUrl, true);
