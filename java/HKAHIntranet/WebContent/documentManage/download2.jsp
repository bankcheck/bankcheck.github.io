...
%><%!
	...
	private File getFileName(String directory, String filePrefix, String fileSuffix, String fileName) {
				...
				if (children != null) {
					try {
						...
						for (int i = 0; i < children.length; i++) {
							tempFileName = getFileName(newDirectory.getPath(), filePrefix, fileSuffix, children[i].getName());
	...
	private SmbFile getSmbFileName(String directory, String filePrefix, String fileSuffix, String fileName) {
		try  {
			String path = (directory + File.separator + fileName).replace("\\", "/");
			if (!path.startsWith("smb:")) {
				path = "smb:" + path;
			}
			
			SmbFile newDirectory = new SmbFile(path);
			if (!newDirectory.exists()) {
				return null;
			}
			if ((filePrefix == null || filePrefix.length() == 0 || newDirectory.getName().indexOf(filePrefix) >= 0)
				...
				if (newDirectory.isDirectory()) {
					SmbFile[] children = newDirectory.listFiles();
					if (children != null) {
						try {
							SmbFile latestFileName = null;
							SmbFile tempFileName = null;
							for (int i = 0; i < children.length; i++) {
								tempFileName = getSmbFileName(newDirectory.getPath(), filePrefix, fileSuffix, children[i].getName());
								if (tempFileName != null && (latestFileName == null || LastModifiedFileComparator.LASTMODIFIED_COMPARATOR.compare(latestFileName,tempFileName) < 0)) {
									latestFileName = tempFileName;
								}
							}
...
if (session == null) {
			...
			if (moduleCode != null) {
				...
				if (row != null) {
					...
					if ("N".equals(row.getValue(4))) {
						...
						//System.out.println("[download2] documentID="+documentID+", rootFolder="+rootFolder+", locationPath="+locationPath+", fileName="+fileName);
						...
						if (fileName != null && fileName.length() > 0) {
							...
							// check OS
							if (ServerUtil.isUseSamba(locationPath)) {
								//locationPath = getSmbFileName(rootFolder, filePrefix, fileSuffix, locationPath).getPath();
							} else {
								locationPath = getFileName(rootFolder, filePrefix, fileSuffix, locationPath).getPath();
							}
						}
					}
				}
	...
	System.out.println("[download2] rootFolder="+rootFolder+", locationPath=" + locationPath);
	...
	if (locationPath != null
		...
		if (ConstantsServerSide.isHKAH() && locationPath.endsWith(".pdf")
				&& !"achs".equals(userBean.getLoginID())
					&& (isDepartmentalPolicy ||
						(
							locationPath != null
						&&
							(locationPath.indexOf("policy") > 0 || locationPath.indexOf("/department") == 0 || locationPath.indexOf("/hospital") == 0 )
						&&
							locationPath.indexOf("INDEX_PDF.pdf") < 0
						)
					)
				) {
//			System.out.println("rootFolder = " + rootFolder );
			...
			response.sendRedirect(response.encodeRedirectURL("../common/convertPdfToSwf.jsp?sourcePath="+tempSourcePath+"&fileName="+tempFileName));
		} else if (locationPath.endsWith(".pdf")
					&& locationPath.indexOf("it-fs1") >= 0
					&& locationPath.indexOf("public") >= 0
					&& locationPath.indexOf("TW_Intranet") >= 0
					&& locationPath.indexOf("Dept") >= 0
					&& locationPath.indexOf("Laboratory") >= 0
					&& locationPath.indexOf("portal_document") >= 0
				) {
			String tempSourcePath = rootFolder + locationPath;
...
<%		} %>
<% if (isEmbedVideo && documentDesc != null) { %>
	<tr class="smallText">
		<td class="portletCaption">
			<%=documentDesc %>
		</td>
...
<div style="margin: 20px;">
	<p>
		<video id="video1" width="960" controls autoplay controlsList="nodownload">
...
<jsp:include page="../common/directory2.jsp" flush="false">
...
<script language="javascript">
	...
	function downloadFileByFilePath(file) {
		document.form1.action = '../documentManage/download2.jsp';
		document.form1.locationPath.value = file;
		document.form1.submit();
	}
