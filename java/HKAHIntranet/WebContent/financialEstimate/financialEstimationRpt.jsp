...
<%!
	...
	private ArrayList<ReportableListObject> fetchProc2(boolean isGP, boolean isPBO, String procedure1) {
		if (isPBO) {
			if (isGP) {
				return UtilDBWeb.getReportableListHATS("SELECT P.PROCCODE, P.PROCDESC || ' - ' || DECODE(C.HATSCODE, NULL, DECODE(C.REFCODE, NULL, '', C.REFCODE), C.HATSCODE) || DECODE(P.LOS, NULL, '', ' | Suggested LOS ' || P.LOS) FROM FIN_PROC P, FIN_CODE C WHERE P.PROCCODE = C.PROCCODE (+) AND P.TYPECODE = ? AND P.STATUS = -1 AND P.ISGP = '1' ORDER BY P.PROCDESC", new String[] { procedure1 });
			} else {
				return UtilDBWeb.getReportableListHATS("SELECT P.PROCCODE, P.PROCDESC || ' - ' || DECODE(C.HATSCODE, NULL, DECODE(C.REFCODE, NULL, '', C.REFCODE), C.HATSCODE) || DECODE(P.LOS, NULL, '', ' | Suggested LOS ' || P.LOS) FROM FIN_PROC P, FIN_CODE C WHERE P.PROCCODE = C.PROCCODE (+) AND P.TYPECODE = ? AND P.STATUS = -1 ORDER BY P.PROCDESC", new String[] { procedure1 });
			}
		} else {
			return UtilDBWeb.getReportableListHATS("SELECT P.PROCCODE, P.PROCDESC FROM FIN_PROC P WHERE P.TYPECODE = ? AND P.STATUS = -1 ORDER BY P.PROCDESC", new String[] { procedure1 });
		}
	}
	...
	private ArrayList<ReportableListObject> fetchPQuotation(String pqId) {
		return UtilDBWeb.getReportableListHATS("SELECT PATNO, REGID, DOCCODE FROM PQUOTATION WHERE PQID = ?", new String[] { pqId });
	}
	...
	private ArrayList<ReportableListObject> fetchOTCharge(String pqId) {
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("SELECT I.ITMNAME, TX.PQAMT FROM PQTX TX LEFT JOIN ITEM I ON TX.ITMCODE = I.ITMCODE WHERE SUBSTR(TX.PQBOX,0,2) IN ('OT','SU','EN','GY','OR') AND TX.PQID = '");
		sqlStr.append(pqId);
		sqlStr.append("' ORDER BY I.ITMNAME");
		return UtilDBWeb.getReportableListHATS(sqlStr.toString());
	}

	private ArrayList<ReportableListObject> fetchOtherCharge(String pqId) {
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("SELECT I.ITMNAME, TX.PQAMT FROM PQTX TX LEFT JOIN ITEM I ON TX.ITMCODE = I.ITMCODE WHERE SUBSTR(TX.PQBOX,0,2) NOT IN ('OT','SU','EN','GY','OR') AND TX.PQID = '");
		sqlStr.append(pqId);
		sqlStr.append("' ORDER BY I.ITMNAME");
		return UtilDBWeb.getReportableListHATS(sqlStr.toString());
	}
...
<%
	...
	String ProcedureSelect2 = request.getParameter("ProcedureSelect2");
	...
	String day = null;
	int dayInt = 0;
	String DocRoundFeeMinInput = null;
	String DocRoundFeeMaxInput = null;
	String ProFeeMinInput = null;
	String ProFeeMaxInput = null;
	String AnaesthetistFeeMinInput = null;
	String AnaesthetistFeeMaxInput = null;
	String OtherMin1Input = null;
	String OtherMax1Input = null;
	String OtherMin2Input = null;
	String OtherMax2Input = null;

	String RoomChgMinInput = null;
	String RoomChgMaxInput = null;
	String OTMinInput = null;
	String OTMaxInput = null;
	String OTAdditionMinInput = null;
	String OTAdditionMaxInput = null;
	String OtherMin3Input = null;
	String OtherMax3Input = null;
	String OtherAdditionMin3Input = null;
	String OtherAdditionMax3Input = null;
	String totalMinInput = null;
	String totalMaxInput = null;
	...
	if (pqId != null && pqId.length() > 0) {
		record = fetchPQuotation(pqId);
		if (record.size() > 0) {
			row = (ReportableListObject) record.get(0);
			patno = row.getValue(0);
			regid = row.getValue(1);
			doccode = row.getValue(2);

			hatsDoccode = doccode;
			userBean = createUserAccount(request, doccode);
		}
		ProcedureSelect1 = (String) session.getAttribute("ProcedureSelect1");
		ProcedureSelect2 = (String) session.getAttribute("ProcedureSelect2");
		DiagnosisText = (String) session.getAttribute("DiagnosisText");
		roomClass = (String) session.getAttribute("acmcode");
		day = (String) session.getAttribute("day");
		try {
			dayInt = Integer.parseInt(day);
		} catch (Exception e) {}

		DocRoundFeeMinInput = (String) session.getAttribute("DocRoundFeeMinInput");
		DocRoundFeeMaxInput = (String) session.getAttribute("DocRoundFeeMaxInput");
		ProFeeMinInput = (String) session.getAttribute("ProFeeMinInput");
		ProFeeMaxInput = (String) session.getAttribute("ProFeeMaxInput");
		AnaesthetistFeeMinInput = (String) session.getAttribute("AnaesthetistFeeMinInput");
		AnaesthetistFeeMaxInput = (String) session.getAttribute("AnaesthetistFeeMaxInput");
		OtherMin1Input = (String) session.getAttribute("OtherMin1Input");
		OtherMax1Input = (String) session.getAttribute("OtherMax1Input");
		OtherMin2Input = (String) session.getAttribute("OtherMin2Input");
		OtherMax2Input = (String) session.getAttribute("OtherMax2Input");

		RoomChgMinInput = (String) session.getAttribute("RoomChgMinInput");
		RoomChgMaxInput = (String) session.getAttribute("RoomChgMaxInput");
		OTMinInput = (String) session.getAttribute("OTMinInput");
		OTMaxInput = (String) session.getAttribute("OTMaxInput");
		OtherMin3Input = (String) session.getAttribute("OtherMin3Input");
		OtherMax3Input = (String) session.getAttribute("OtherMax3Input");
		totalMinInput = (String) session.getAttribute("totalMinInput");
		totalMaxInput = (String) session.getAttribute("totalMaxInput");

		record = fetchOTCharge(pqId);
		int OTAdditionFee = 0;
		for (int i = 0; i < record.size(); i++) {
			row = (ReportableListObject) record.get(i);
			try {
				OTAdditionFee += Integer.parseInt(row.getValue(1));
			} catch (Exception e) {}
		}
		OTAdditionMinInput = String.valueOf(OTAdditionFee);
		OTAdditionMaxInput = String.valueOf(OTAdditionFee);

		record = fetchOtherCharge(pqId);
		int OtherAdditionFee = 0;
		for (int i = 0; i < record.size(); i++) {
			row = (ReportableListObject) record.get(i);
			try {
				OtherAdditionFee += Integer.parseInt(row.getValue(1));
			} catch (Exception e) {}
		}
		OtherAdditionMin3Input = String.valueOf(OtherAdditionFee);
		OtherAdditionMax3Input = String.valueOf(OtherAdditionFee);
	} else if (regid != null) {
	...
	boolean isPBO = false;
	boolean isFreeTextDoc = false;
	boolean allowViewHistory = false;


if (userBean != null && userBean.isLogin()) {
	isPBO = userBean.isAccessible("function.financialEstimation.pbo");
	isFreeTextDoc = userBean.isAccessible("function.financialEstimation.pbo");
	allowViewHistory = !isGP && ((doccode != null && hatsDoccode != null && hatsDoccode.equals(doccode)) || userBean.isAccessible("function.financialEstimation.viewHistory"));
...
<%} %>
	...
	<th colspan="4" bgcolor='#D8BFD8'>
			Financial Estimation Form for Hospital Admission and Procedure / Surgery<%if (regid != null && regid.length() > 0) { %> <font color='red'>(with Price Estimation)</font><%} %>
	</th>
...
<%	} %>
	...
	<tr>
		...
		<td colspan="4">
			...
			<select name="ProcedureSelect2" id="ProcedureSelect2" onchange="return changeCode2(this);">
<%
	if (ProcedureSelect1 != null && ProcedureSelect1.length() > 0 && ProcedureSelect2 != null && ProcedureSelect2.length() > 0) {
		record = fetchProc2(isGP, isPBO, ProcedureSelect1);
		if (record.size() > 0) {
			for (int i = 0; i < record.size(); i++) {
				row = (ReportableListObject) record.get(i);
%>
				<option value="<%=row.getValue(0) %>"<%if (row.getValue(0).equals(ProcedureSelect2)) {%> selected<%}%>><%=row.getValue(1) %></option>
<%
			}
		}
	}
%>
	...
	<tr>
		...
		<td colspan="4"><input type="text" class="textBox" id="DiagnosisText" name="DiagnosisText" maxlength="50" value="<%=DiagnosisText==null?"":DiagnosisText %>" style="width: 550px;" /></td>
	</tr>
...
<%	for (int i = 0; i <= 31; i++) { %>
				<option value="<%=i %>"<%=dayInt == i?" selected":"" %>><%=i %></option>
<%	}%>
...
<%} %>
		...
		<td width="15%">
			<table>
				<tr>
					<td><input type="text" value="<%=day==null?"1":day%>" size="2" name="DrRndDay1" id="DrRndDay1" value="" onkeypress='numCheck(event)' onchange="javascript: numCheck2(this); DrRndCal();" readonly /></td>
					<td> day(s)</td>
					<td><b>x</b></td>
				</tr>
				<tr>
					<td colspan="3">
						$ <input type="text" name="DocRoundFeeMinInput" id="DocRoundFeeMinInput" value="<%=DocRoundFeeMinInput==null?"":DocRoundFeeMinInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
						~ <input type="text" name="DocRoundFeeMaxInput" id="DocRoundFeeMaxInput" value="<%=DocRoundFeeMaxInput==null?"":DocRoundFeeMaxInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput" name="ProFeeMinInput" id="ProFeeMinInput" value="<%=ProFeeMinInput==null?"":ProFeeMinInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="ProFeeMaxInput" id="ProFeeMaxInput" value="<%=ProFeeMaxInput==null?"":ProFeeMaxInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput" name="AnaesthetistFeeMinInput" id="AnaesthetistFeeMinInput" value="<%=AnaesthetistFeeMinInput==null?"":AnaesthetistFeeMinInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="AnaesthetistFeeMaxInput" id="AnaesthetistFeeMaxInput" value="<%=AnaesthetistFeeMaxInput==null?"":AnaesthetistFeeMaxInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput" name="OtherMin1Input" id="OtherMin1Input" value="<%=OtherMin1Input==null?"":OtherMin1Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="OtherMax1Input" id="OtherMax1Input" value="<%=OtherMax1Input==null?"":OtherMax1Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput" name="OtherMin2Input" id="OtherMin2Input" value="<%=OtherMin2Input==null?"":OtherMin2Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="OtherMax2Input" id="OtherMax2Input" value="<%=OtherMax2Input==null?"":OtherMax2Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
	<tr bgcolor='#FAFAD2'>
		<td colspan="2">&nbsp;</td>
...
<%}%>
	...
	<tr>
		...
		<td>
			<table>
				<tr>
					<td><input type="text" value="<%=day==null?"1":day%>" size="2" name="DrRndDay2" id="DrRndDay2" value="" onkeypress='numCheck(event)'  onchange="javascript: numCheck2(this); DrRndCal();" readonly /></td>
					<td> day(s)</td>
					<td><b>x</b></td>
				</tr>
				<tr>
					<td colspan="3">
						$ <input type="text" name="RoomChgMinInput" id="RoomChgMinInput" value="<%=RoomChgMinInput==null?"":RoomChgMinInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
						~ <input type="text" name="RoomChgMaxInput" id="RoomChgMaxInput" value="<%=RoomChgMaxInput==null?"":RoomChgMaxInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput" name="OTMinInput" id="OTMinInput" value="<%=OTMinInput==null?"":OTMinInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="OTMaxInput" id="OTMaxInput" value="<%=OTMaxInput==null?"":OTMaxInput%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
<%if (pqId != null && pqId.length() > 0) { %>
	<tr>
		...
		<td>&nbsp;</td>
		<td align="center">
			$ <input type="text" class="input sMininput" name="OTAdditionMinInput" id="OTAdditionMinInput" value="<%=OTAdditionMinInput==null?"":OTAdditionMinInput%>" onchange="javascript: MinCal();" readonly />
			~ <input type="text" class="input sMaxinput" name="OTAdditionMaxInput" id="OTAdditionMaxInput" value="<%=OTAdditionMaxInput==null?"":OTAdditionMaxInput%>" onchange="javascript: MaxCal();" readonly />
		</td>
	</tr>
<%} %>
	<tr>
		...
		<td align="center">
			$ <input type="text" class="input sMininput sinput" name="OtherMin3Input" id="OtherMin3Input" value="<%=OtherMin3Input==null?"":OtherMin3Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
			~ <input type="text" class="input sMaxinput" name="OtherMax3Input" id="OtherMax3Input" value="<%=OtherMax3Input==null?"":OtherMax3Input%>" onkeypress='numCheck(event)' onblur="javascript: numCheck2(this); InputCal();"/>
		</td>
	</tr>
<%if (pqId != null && pqId.length() > 0) { %>
	<tr>
		...
		<td>&nbsp;</td>
		<td align="center">
			$ <input type="text" class="input sMininput" name="OtherAdditionMin3Input" id="OtherAdditionMin3Input" value="<%=OtherAdditionMin3Input==null?"":OtherAdditionMin3Input%>" onchange="javascript: MinCal();" readonly />
			~ <input type="text" class="input sMaxinput" name="OtherAdditionMax3Input" id="OtherAdditionMax3Input" value="<%=OtherAdditionMax3Input==null?"":OtherAdditionMax3Input%>" onchange="javascript: MaxCal();" readonly />
		</td>
	</tr>
<%} %>
	<tr bgcolor='#FAFAD2'>
		...
		<td align="center">
			$ <input type="text" name="totalMinInput" id="totalMinInput" value="<%=totalMinInput==null?"":totalMinInput%>" readonly />
			~ <input type="text" name="totalMaxInput" id="totalMaxInput" value="<%=totalMaxInput==null?"":totalMaxInput%>" readonly />
		</td>
	</tr>
	<tr>
		<td colspan="7">
...
<%} %>
				&nbsp;<button type="button" id="confirm" name="confirm" style="font-size:15px;" onclick="printRPT();"/><b>Print Financial Estimation</b></button>
<%if (regid != null && regid.length() > 0) { %>
				&nbsp;<button type="button" id="priceQuotation" name="priceQuotation" style="font-size:15px;" onclick="gotoPriceQuotation();"/><b>Goto Price Estimation</b></button>
<%} %>
			</div>
		</td>
	</tr>
	...
	<input type="hidden" name="pqId" value="<%=pqId==null?"":pqId %>">
	<input type="hidden" name="group" value="<%=group==null?"":group %>">
	<input type="hidden" name="hats_RoomChgMin_temp" id="hats_RoomChgMin_temp">
	<input type="hidden" name="hats_RoomChgMax_temp" id="hats_RoomChgMax_temp">
</form>
...
<%if (pqId == null && ProcedureSelect1 != null && ProcedureSelect1.length() > 0) { %>
<%	if (ProcedureSelect2 != null && ProcedureSelect2.length() > 0) { %>
changeCode(document.getElementById("ProcedureSelect1"), '<%=ProcedureSelect2 %>');
<%	} else { %>
...
function changeCode(proc, proc2) {
	if (proc.value == 99) {
		$("#ProcedureSelect2_indicator").html("<input type='text' name='ProcedureText' id='ProcedureText' maxlength='200' value='' style='width: 550px;'>");
	} else {
		$.ajax({
			type: "POST",
...
function changeBed(bed, callref) {
	$.ajax({
		...
		if (values != '') {
			...
			// for restore original room chg
			document.getElementById("hats_RoomChgMin_temp").value = document.frmEstimate.hats_RoomChgMin.value;
			document.getElementById("hats_RoomChgMax_temp").value = document.frmEstimate.hats_RoomChgMax.value;
...
function gotoPriceQuotation() {
	document.getElementById('frmEstimate').action = 'financialEstimation_store.jsp';
	document.getElementById('loading').style.display = 'block';
	document.getElementById('loading').style.display = 'none';
	document.getElementById("frmEstimate").submit();
}
...
function copy2Input(type) {
	if (type == '2') {
		...
		// restore original room chg
		if (document.getElementById("RoomChgMinInput").value == '') {
			document.getElementById("RoomChgMinInput").value = document.getElementById("hats_RoomChgMin_temp").value;
		}
		if (document.getElementById("RoomChgMaxInput").value == '') {
			document.getElementById("RoomChgMaxInput").value = document.getElementById("hats_RoomChgMax_temp").value;
		}
