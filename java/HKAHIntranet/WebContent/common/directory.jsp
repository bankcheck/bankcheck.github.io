...
<%!
	...
	private static boolean checkShowFolder(UserBean userBean, String folderName, String fileName,
		...
		System.out.println("[directory] checkShowFolder start folderName="+folderName+", fileName="+fileName);
...
if (session == null) {
	...
	System.out.println("  directory rootFolder="+rootFolder+", locationPath="+locationPath);
	...
	try {
		...
		if (!isShowSubFolder && path.length() >= directory.getPath().length()) {
...
%>
<jsp:include page="directory.jsp" flush="false">
					...
					if (checkShowFolder(userBean, locationPath, file.getName(), isDepartmentalSharing,isHRImg, isFdSharing)) {	
						...
						<%}if( isHRImg && newLocationPath.contains("resized")){
								...
								<td><a href="javascript:moveDirectory('<%=StringEscapeUtils.escapeJavaScript(newLocationPath) %>')"><img src="../images/folder.gif" width="24" height="24"><%=isDepartmentalSharing?
																																																	...
																																																	children[i] %></a></td>
								<td>&nbsp;</td>
								<td><%=sdf.format(new java.util.Date(file.lastModified())) %></td>
							</tr>
						<%}
					}
				}
				} else if ("Thumbs.db".equals(children[i]) || children[i].startsWith("~")) {
...
//								(!Converter.isFileExtensionAllowed(FilenameUtils.getExtension(StringEscapeUtils.escapeJavaScript(children[i]))) && !StringEscapeUtils.escapeJavaScript(children[i]).trim().endsWith(".pdf"))) {
							...
							if ((userBean.isLogin() && (checkEditPermission(userBean, locationPath, isPolicy, isDepartmentalSharing, isVPA, isDeptResource, isFdSharing) || isIC)) || (!Converter.isFileExtensionAllowed(FilenameUtils.getExtension(StringEscapeUtils.escapeJavaScript(children[i]))) && !StringEscapeUtils.escapeJavaScript(children[i]).trim().endsWith(".pdf") && !children[i].startsWith("~"))) {
