...
<c:if test="${education == null}">
	<c:redirect url="literature_search.htm" />
</c:if>
			...
			<c:forEach var="content" items="${education.list}">
				<li>
					<c:choose>
						<c:when test="${content.eeMenuDocument != null}">
								<c:choose>
									<c:when test="${content.eeMenuDocument.eeIsUrl eq 'Y'}" >
										<a href="<c:out value='${content.eeMenuDocument.eeUrl}' />" >
											<h4>
												<font color="white"><c:out value="${content.eeDescriptionEn}" /></font>
											</h4>
										</a>
									</c:when>
									<c:otherwise>
						...
						<c:otherwise>
							<h4><font color="white"><c:out value="${content.eeDescriptionEn}" /></font></h4>
						</c:otherwise>
					</c:choose>
