...
<form name="search_form" action="test_report_list.jsp" method="post">
...
<script language="javascript">
	...
	//	http.send(null);
		...
		$.ajax({
			type: "POST",
			url: "test_report_list_result.jsp",
			data: "reportCategory=" + reportCategory + "&searchType=" + searchType + "&date_from=" + date_from + "&date_to=" + date_to + "&financialYear=" + financialYear_yy + "&eventID-elearningID=" + eventIDElearningID + "&timestamp=<%=(new java.util.Date()).getTime() %>",
			success: function(values) {
			if (values != '') {
				document.getElementById("test_report_list_result").innerHTML = values;
			}//if
			},//success
			timeout: 60000
		});//$.ajax
