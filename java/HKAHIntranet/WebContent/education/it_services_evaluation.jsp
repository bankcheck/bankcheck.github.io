...
<table width="100%" border="0">
	<tr>
		...
		<td width="80%">
			...
			<P class="survey-content">For management who oversee both campuses, please fill two survey forms. One is on HKAH & other is on TWAH.</P>
		</td>
		<td width="10%">&nbsp;</td>
	</tr>
	<tr>
		<td width="10%">&nbsp;</td>
		<td>
		<form name="form1" action="it_services_evaluation.jsp" onsubmit="return submitAction();">
<table width="100%" border="0">
	<tr class="smallText">
		<td class="infoLabel">Survey on:</td>
		<td class="infoData" colspan="7">
<%
			String checkedAttr = "checked=\"checked\"";
			String hkahChecked = null;
			String twahChecked = null;
			hkahChecked = (ConstantsServerSide.SITE_CODE_HKAH.equals(evaluationSite)) ? checkedAttr : "";
			twahChecked = (ConstantsServerSide.SITE_CODE_TWAH.equals(evaluationSite)) ? checkedAttr : "";
%>
			<input type="radio" name="evaluationSite" value="<%=ConstantsServerSide.SITE_CODE %>" <%=hkahChecked %> />HKAH / 
			<input type="radio" name="evaluationSite" value="<%=ConstantsServerSide.SITE_CODE %>" <%=twahChecked %> />TWAH 
			(Please select one)  
		</td>
	</tr>
	<tr class="smallText">
...
<%		} %>
	<tr class="smallText">
		<td class="infoData"><%=row.getValue(1) %></td>
<%			if (createAction) { %>
<%
	...
	function submitAction() {
		...
		for(i=0; i<document.forms['form1'].elements.length; i++)
		{
			if (document.forms['form1'].elements[i].type == 'radio') {
				debug += "The field name is: " + document.forms['form1'].elements[i].name + " and it’s checked is: " + document.forms['form1'].elements[i].checked + "\n";
			}
					...
					if (radioName != '') {
						if (!radioChecked) {
							if (radioName == 'evaluationSite') {
								alert('Please select the campus the survey is applying on.');
							} else {
