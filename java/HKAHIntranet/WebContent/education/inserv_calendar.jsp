...
<c:if test="${education == null}">
	<c:redirect url="inserv_calendar.htm" />
</c:if>
...
<style>
#contentPage ul { list-style: url("../images/file.gif") inside; margin: 10px 0; }
#contentPage ul li { margin: 5px 0; }
</style>
...
<table border="0" style="width:100%">
	...
	<div style="width:100%" id="staffEducationWrapper">
		<div style="width:90%" id="contentPage">			
			...
			<p style="font-size:150%;">A monthly Education Calendar with different in-service education scope is e-mailed to Department Heads/ Managers for designating their staff to attend the scheduled sessions.</p>
			<ul>
		<c:if test="${education.list != null}" > 
			<c:forEach var="content" items="${education.list}">
			<c:set var="getTestYear" value="${content.eeDescriptionEn}" scope="request"/>
<%
String fileYear = ((String)request.getAttribute("getTestYear")).split("-")[0];

Calendar currentYear = Calendar.getInstance();
int cYear = currentYear.get(Calendar.YEAR);
String cYearInString = String.valueOf(cYear);

currentYear.add(Calendar.YEAR, -1);
int pYear = currentYear.get(Calendar.YEAR);
String pYearInString = String.valueOf(pYear);

if(cYearInString.equals(fileYear) || pYearInString.equals(fileYear)){
%>					
				<li>
				<c:choose>		
