...
<%!
	...
	private boolean isOnlineCompulsory(String elearningID) {
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("SELECT 1 ");
		sqlStr.append("FROM   EE_ELEARNING EL ");
		sqlStr.append("INNER JOIN CO_EVENT E ON EL.EE_EVENT_ID = E.CO_EVENT_ID ");
		sqlStr.append("WHERE E.CO_MODULE_CODE = 'education' ");
		sqlStr.append("AND   E.CO_EVENT_CATEGORY = 'compulsory' ");
		sqlStr.append("AND   E.CO_EVENT_TYPE = 'online' ");
		sqlStr.append("AND   EL.EE_ELEARNING_ID = ? ");
		sqlStr.append("AND   EL.EE_ENABLED = 1 ");

		ArrayList<ReportableListObject> result = UtilDBWeb.getReportableList(sqlStr.toString(), new String[] { elearningID });
	...
	private Object[] isCorrectAnswer(int currentQuestion,
		...
		try {
			...
			output.append("	<td width=\"90%\"><font color='green' size='3'>" + lineWrap(questionsDesc[currentQuestion]) + "</font></td>");
	...
	private String getSwfDisplayName(String videoPath, String fileExtension) {
		String[] path = videoPath.split("/");
		String displayName = "";
		if (path != null && path.length > 1) {
			if (path[path.length - 1].indexOf(fileExtension) >= 0) {
...
boolean isDebug = ConstantsServerSide.DEBUG;
boolean isRealDebug = ConstantsServerSide.DEBUG;
int current_mm = DateTimeUtil.getCurrentMonth();
if (!isDebug && current_mm >= 11) {
	isDebug = userBean.isAllowAdmin() || userBean.isIT();
}
if (!isRealDebug && current_mm == 12) {
	isRealDebug = userBean.isAllowAdmin()/* || userBean.isSwitchUser()*/;
}
boolean isLMCCRM = false;
if (!"edu".equals(from) && CRMClientDB.getClientID(userBean.getUserName()) != null) {
	isLMCCRM= true;
}

String elearningID = request.getParameter("elearningID");
boolean lockTest = false;
if (!userBean.isAccessible("function.education.noRestrictOnlineTest") && current_mm < 10 && isOnlineCompulsory(elearningID)) {
	lockTest = true;
}
	...
	try {
		...
		if (testAction || submitAction) {
			...
			if (submitAction) {
				...
				if ((correctAnswerNumInt > 0 && correctAnswerNumInt >= passGradeInt && !userBean.isAdmin()) || isLMCCRM) {
					// save the result
					String startTime = (String) session.getAttribute("elearning.startTime");
					String enrollID = "";

					if (isLMCCRM) {
						enrollID = EnrollmentDB.add(userBean, "lmc.crm", eventID, null, "guest", CRMClientDB.getClientID(userBean.getUserName()), DateTimeUtil.getCurrentDateTime(), null, startTime);
					} else {
						enrollID = EnrollmentDB.add(userBean, "education", eventID, null, "staff", staffID, DateTimeUtil.getCurrentDateTime(), null, startTime);
					}
					...
					for (int i = 0; i < questionNumInt; i++) {
						for (int j = 0; j < answersUser[i].length; j++) {
							...
							if (isLMCCRM ) {
								ELearning.addLMCClientResultAnswer(userBean, eventID, elearningID, enrollID, questions[i], answersUser[i][j], correct ? ConstantsVariable.ONE_VALUE : ConstantsVariable.ZERO_VALUE);

							} else {
								ELearning.addStaffResultAnswer(userBean, eventID, elearningID, enrollID, questions[i], answersUser[i][j], correct ? ConstantsVariable.ONE_VALUE : ConstantsVariable.ZERO_VALUE);
							}
						}
					}
					// store user answer into db
					if (isLMCCRM) {
						ELearning.addLMCClientResult(userBean, eventID, elearningID, enrollID, String.valueOf(correctAnswerNumInt));

					} else {
						ELearning.addStaffResult(userBean, eventID, elearningID, enrollID, String.valueOf(correctAnswerNumInt));
					}
...
<%
			...
			if (Locale.TRADITIONAL_CHINESE.equals((Locale) session.getAttribute( Globals.LOCALE_KEY))
					|| Locale.SIMPLIFIED_CHINESE.equals((Locale) session.getAttribute( Globals.LOCALE_KEY))) {
			...
			// prepare question
			if (isLMCCRM) {
				record = ELearning.getCRM(elearningID);
			} else {
...
<%
		...
		%>
<jsp:include page="../common/page_title.jsp" flush="false">
	<jsp:param name="pageTitle" value="prompt.eLesson" />
	<jsp:param name="category" value="<%=category %>" />
</jsp:include>
<%
		}
...
<table cellpadding="0" cellspacing="5"
	...
	<tr class="smallText">
		...
		<td class="infoData" width="70%"><%=topic %><br /><%=topicZh %></td>
	</tr>
	<tr class="smallText">
		<td class="infoLabel" width="30%"><bean:message key='<%=ConstantsServerSide.isTWAH() ? "prompt.document.twah" : "prompt.document" %>' /></td>
		<td class="infoData" width="70%">
<%	if (documentsNum > 1 && !"0".equals(questionNum)) { %>
			<p class="cautionText"><bean:message key="message.test.readAllFiles" /></p>
<%	} %>
			<table>
<%	int docCount = 1; %>
				<tr>
					<td style="vertical-align: top;"><%=docCount++ %>.</td>
					<td>
...
<%	if (videoPaths != null && !videoPaths.isEmpty()) {
		String swfDisplayName = null;
		URLCodec urlC = new URLCodec();
		for (int i = 0; i < videoPaths.size(); i++) {
			String vPath = videoPaths.get(i);
			swfDisplayName = getSwfDisplayName(vPath , ".swf");
			String ext = vPath.substring(vPath.indexOf(".", -1), vPath.length());
			String videoName = getSwfDisplayName(vPath , ext);
			session.setAttribute(swfDisplayName, vPath);
			String link = (".flv".equals(ext) ?
					"elearning_video_flv.jsp?vPath=" + urlC.encode(vPath) + "&keepThis=true&TB_iframe=true&height=500&width=660" :
					(".mp4".equals(ext) ?
							"elearning_video_mp4.jsp?vPath=" + urlC.encode(vPath)
							:"elearning_video.jsp?swfname=" + swfDisplayName + "&keepThis=true&TB_iframe=true&height=600&width=800"));
			%>
				<tr>
					<td><%=docCount++ %>.</td>
					<td><a href="<%=link %>" title="<%=videoName %>" class="thickbox button">
						<span><bean:message key="prompt.startLesson" /></span></a>
						<font color="gray"><%=swfDisplayName %></font>
					</td>
				</tr>
<%		} %>
<%	} %>
...
<% if(!"0".equals(questionNum)){ %>
	<tr class="smallText">
		...
		<td class="infoData" width="70%"><font color="red"><%=questionNum %></font></td>
	</tr>
	<tr class="smallText">
		<td class="infoLabel" width="30%"><bean:message key="prompt.passGrade" /></td>
<%
	String correctAnswerMessageKey = null;
	if (questionNumInt == passGradeInt) {
		correctAnswerMessageKey = "message.correctAnswer.all";
	} else {
		correctAnswerMessageKey = "message.correctAnswer.minimum";
	}
%>
		<td class="infoData" width="70%"><font color="red"><bean:message key="<%=correctAnswerMessageKey %>" arg0="<%=passGrade %>" /></font></td>
	</tr>
	<tr>
		<td colspan="2">&nbsp;</td>
	</tr>
...
<%	if (correctAnswerNumInt > 0 && correctAnswerNumInt >= passGradeInt && !userBean.isAdmin()) { %>
	<tr class="smallText">
		<td align="center">
		<%if (elearningID.equals("15")) { %>
			<b><bean:message key="message.test.pass" />!</b>
		<%} else { %>
			<b><bean:message key="message.test.pass" />!
			<%if (!isLMCCRM) { %>
			<br><bean:message key="message.test.passConfirm" />
			<%} %>
			</b>
		<%} %>
		</td>
...
<%	} else {%>
	<tr class="smallText">
		<td align="center" colspan=3>
			<button type="submit"><bean:message key="message.test.fail" />!</button>
		</td>
	<tr>
<%	}
	// reset all the value
...
%><button onclick="movePage('1');return false;"><bean:message key="button.question.next" /></button><%
	...
	if ((questionPointerInt + noOfQuestionPerPage >= questionNumInt) || isRealDebug) {
%><button onclick="completePage();return false;"><bean:message key="button.question.submit" /></button><%
	}
%>
		</td>
	</tr>
...
%>
	<tr class="sessionColEven">
		<td align="right" valign="top"><font color="green" size="3">Q<%=i + 1 %>)</font></td>
		<td><font color="green" size="3"><%=lineWrap(questionsDesc[i]) %></font><br/><br/></td>
		<td>&nbsp;</td>
	</tr>
...
%>
					...
					<td width="5%">
						<font size="3"><%=alpha[j] %>)&nbsp;
						<input type="<%if (answersCorrect[i].length == 1) {%>radio<%} else { %>checkbox<%} %>" name="answer<%=i %>" value="<%=answers[i][j] %>"<%=(userAnswer || (isRealDebug && answersCorrect[questionPointerInt][0].equals(answers[i][j])))?" checked":""%> onclick="javascript:checkAnswer(<%=answers[i][j] %>);">&nbsp;
						</font>
					</td>
					<td width="90%">
						<font size="3"><%=answersDesc[i][j] %></font>
					</td>
				</tr>
<%
			}
...
<table width="100%" border="0">
	...
	<tr class="smallText">
		<td>&nbsp;</td>
		<td align="center"><font color='blue'><span id="quizMsg"><%if (!hasDocument) { %><button name="startQuiz" onclick="quiz();"><bean:message key='<%=ConstantsServerSide.isTWAH() ? "button.test.twah" : "button.test" %>' /></button><%} %></span></font></td>
		<td>&nbsp;</td>
	</tr>
</table>
<% } %>
...
<%	if (lockTest) { %>
	alert('<bean:message key='message.test.lock' />');
	document.getElementById("quizMsg").innerHTML = '<bean:message key='message.test.lock' />';
<%	} else { %>
	startQuiz();
<%	} %>
...
<%if ((isDebug || isRealDebug) && !isLMCCRM) { %>
	function checkAnswer(aid) {
		$.ajax({
			type: "POST",
			url: "elearning_test_verify.jsp",
			data: "qpi=<%=questionPointerInt %>&aid=" + aid,
			success: function(values) {
			if (values != '') {
				$("#staff-container").html("<select name='selectedTaskResponsibleByStaffID'>" + values + "</select>");
				if (values == '1') {
					document.getElementById('result' + aid).innerHTML = '<img src="../images/tick_green_small.gif">';
				} else {
					document.getElementById('result' + aid).innerHTML = '<img src="../images/cross_red_small.gif">';
				}
			}//if
			}//success
		});//$.ajax
