...
String LabNum = TextUtil.parseStrUTF8(request.getParameter("LabNum1"));
...
String ORGANISM = TextUtil.parseStrUTF8(request.getParameter("ORGANISM"));
String ANTIBIOTIC = TextUtil.parseStrUTF8(request.getParameter("ANTIBIOTIC"));
String TEXTRESULT = TextUtil.parseStrUTF8(request.getParameter("TEXTRESULT"));
String DATERPT = TextUtil.parseStrUTF8(request.getParameter("DATERPT"));
...
<%	if (createAction || updateAction) { %>
	<input type="text" id="HospNum" name="HospNum" value="<%=HospNum == null ? "" : HospNum %>" maxlength="6" size="15" />
	<%	} else { %>
...
<%	} %>
  ...
  <tr><td>
  <input type="hidden" id="LabNum" name="LabNum" value="<%=LabNum == null ? "" : LabNum %>" maxlength="12" size="15" />
...
<table cellpadding="0" cellspacing="5" class="contentFrameSearch" border="0">
  <b>Lab Microbiology Result</b>
  <hr style="color:red"></hr>	
  <tr>
	<td class="infoLabel" width="15%">Lab No.</td>
	<td class="infoData" width="10%">
	<%	if (createAction || updateAction) { %>
		<select id="LabNum1" name="LabNum1">		
		</select>
	<%	} else { %>
			<%=LabNum %>
	<%	} %>
  ...
  <tr>
  	<td class="infoLabel" width="15%">Report Date</td>
	<td class="infoData" width="10%">	
	<%	if (createAction || updateAction) { %>
		<div class="box" >
	  		<textarea readonly="readonly" id="DATERPT" name="DATERPT" rows="1" cols="20"><%=DATERPT==null?"":DATERPT %></textarea>
	  	</div>
	<%	} else { %>			
		<div id="DATERPT">
		  <%=DATERPT == null ? "" : DATERPT %>
		</div>
	<%	} %>
	</td>  
  </tr>  
  <tr>
	<td class="infoLabel" width="30%">Text Result</td>	 
	<td class="infoData" width="70%" valign="top">
<%	if (createAction || updateAction) { %>
	<div class="box" >
  		<textarea readonly="readonly" id="TEXTRESULT" name="TEXTRESULT" rows="3" cols="100"><%=TEXTRESULT==null?"":TEXTRESULT %></textarea>
  	</div>
<%	} else { %>			
	<div id="TEXTRESULT">
	  <%=TEXTRESULT == null ? "" : TEXTRESULT %>
	</div>
<%	} %>
  	</td>
  </tr>	
  <tr>
	<td class="infoLabel" width="30%">Organism Found</td>	 
	<td class="infoData" width="70%" valign="top">
<%	if (createAction || updateAction) { %>
	<div class="box" >
  		<textarea readonly="readonly" id="ORGANISM" name="ORGANISM" rows="10" cols="100"><%=ORGANISM==null?"":ORGANISM %></textarea>
  	</div>
<%	} else { %>			
	<div id="ORGANISM">
...
<%	} %>
  ...
  <tr>
	<td class="infoLabel" width="30%">Antibiotics</td>	 
	<td class="infoData" width="70%" valign="top">
<%	if (createAction || updateAction) { %>
	<div class="box" >
  		<textarea readonly="readonly" id="ANTIBIOTIC" name="ANTIBIOTIC" rows="20" cols="100"><%=ANTIBIOTIC==null?"":ANTIBIOTIC %></textarea>
  	</div>
<%	} else { %>		
	<div id="ANTIBIOTIC">
	<%=ANTIBIOTIC == null ? "" : ANTIBIOTIC %>
	</div>
<%	} %>
  	</td>  	  		    
  </tr>	
...
$(document).ready(function() {
	...
	<%} else {%> 	
		loadMicBioInfo(<%=LabNum %>, 0);
	<%}%>
	
	$('input[name=HospNum]').change(function() {
		var temp = this.id.split('_');
		loadLabInfo(this.value, temp[1]);
	});
	
	$('#LabNum1').change(function() {
//		alert('selected $(\'#LABNUM1 option:selected\').val()='+$('#LABNUM1 option:selected').val());
		var temp = this.id.split('_');
		loadMicBioInfo($('#LabNum1 option:selected').val(), temp[1]);
	});	

	
	loadMicBioCMB(<%= HospNum %>, 0);
});

function loadLabInfo(hospnum, rowNum) {
	var param = {
		hospnum:	hospnum,
		command: '<%=command %>'
	};
...
function loadMicBioCMB(hospnum, rowNum) {	
	  var param = {
			hospnum:	hospnum,
			labnum: '<%=LabNum %>', 
			command: '<%=command %>'
	  };
	//$('input[name=LABNUM1]').load('InpatMicBioCMB.jsp', param);
		$('#LabNum1').load('InpatMicBioCMB.jsp', param, function() {
			loadMicBioInfo($('#LabNum1 option:selected').val(), 0);	
		});	
	};
		

function loadMicBioInfo(labnum, rowNum) {
	var param = {
		labnum:	labnum,
		command: '<%=command %>'
	};
		//$.getJSON('getPatInfoByLabNum.jsp', param, function(data) {
		$.getJSON('getMicBioInfoByLabNum.jsp', param, function(data) {
			var items = [];
			$.each(data, function(key, val) {
				var id_span = '#' + key;
				var id = '#' + key;
...
function submitAction(cmd, stp) {
	if (stp == 1) {
		if (cmd == 'create' || cmd == 'update') {
			if (document.form1.HospNum.value == '') {
				alert("Please input Hosp. No.");
				document.form1.HospNum.focus();
				return false;
			}
		}
		if (cmd == 'create' || cmd == 'update') {
