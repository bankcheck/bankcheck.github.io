...
if ("bld".equals(icType)) { 
	request.setAttribute("progress", ICBldMrsaEsblDB.getListBld(labNo, FromDateIn, ToDateIn, icType ));
} else { 
	request.setAttribute("progress", ICBldMrsaEsblDB.getList(labNo, FromDateIn, ToDateIn));
}
...
<form name="form1" action="bld_mrsa_esbl.jsp" method="post">
<%	if (!createAction && !updateAction) {%>
<bean:define id="functionLabel"><bean:message key="function.ic.bld_mrsa_esbl.list" /></bean:define>
...
<%	
	if (!"bld".equals(icType)) {
%>
	<display:column property="fields14" title="MDRO Type"  />
<%	}%>
