...
if ((tmpDate.getTime() - currentDate.getTime()) / (24 * 60 * 60 * 1000) < 7)
	showNext = true;
...
<form name="form1" id="form1" action="booking_calendar.jsp" method="post">
...
%>			
		...
		<td align="center" width="50%">Staff: <font color='green'><b><%=staffName %></b></font></td>
...
<table width="100%" border="1"  style="width:1000px">
	<tr class="calendarHeader">
		<th width="10%" class="calendarDayText">Time</th>
		<th width="15%" class="calendarDaySunText"><%=heading1[0] %><br/><%=heading2[0] %></th>
		<th width="15%" class="calendarDayText"><%=heading1[1] %><br/><%=heading2[1] %></th>
		<th width="15%" class="calendarDayText"><%=heading1[2] %><br/><%=heading2[2] %></th>
		<th width="15%" class="calendarDayText"><%=heading1[3] %><br/><%=heading2[3] %></th>
		<th width="15%" class="calendarDayText"><%=heading1[4] %><br/><%=heading2[4] %></th>
		<th width="15%" class="calendarDayText"><%=heading1[5] %><br/><%=heading2[5] %></th>
	</tr>
<%
	try {
...
<%
				for (int j = 0; j < 6; j++) {
					...
					String available = FitnessDB.getAvailable(heading2[j], slotNo, staffID);
					...
					if ((tmpDate.compareTo(currentDate) > 0) && showLink) {												
%>
		<td><a href="javascript:reserve('<%=heading2[j] %>', '<%=slotNo %>');"><%=available %></a></td>
<%
					} else {
%>
		<td><%=available %></td>						
...
<!--
	function reserve(date, timeslot) {

		callPopUpWindow("reserve.jsp?bookDate=" + date + "&timeslot=" + timeslot);
	}
