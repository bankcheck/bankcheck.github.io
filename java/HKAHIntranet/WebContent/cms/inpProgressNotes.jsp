...
try {
	...
	//=== load patient info
	sql  = " select a.patno, a.patsex, patfname||' '||patgname as name, to_char(patbdate,'dd/mm/yyyy') as dob, i.bedcode ";
	sql += "   , (case when i.inpddate<=sysdate-14 then 'true' else 'false' end) as readonly ";
	sql += " from hat_patient a, hat_inpat i, hat_reg r ";
	sql += " where a.patno=r.patno and i.inpid=r.inpid and r.regsts ='N' and regid=? ";
	
	ArrayList patInfo = UtilDBWeb.getReportableListCIS( sql, new String[]{regid});
	...
	if (patInfo.size() > 0) {
		...
		patdob = row.getValue(3);
		bedcode = row.getValue(4);
		readonly = row.getValue(5);
	}

	if ("create".equals(action)) {
			
		sql = "INSERT INTO IPD_PRG_NOTE ";
		sql += "(id, patno, regid, note_date, note_type, title, description, status, create_user, create_date, update_user, update_date) ";
		sql += "VALUES (seq_ipd_prg_note.nextval, ?, ?, to_date(?,'dd/mm/yyyy hh24:mi'), 'CMS', ?, ?, 'A', ?, sysdate, ?, sysdate)";
		
		if ( UtilDBWeb.updateQueueCIS( sql, new String[]{ patno, regid, colDate+" "+colTime, colTitle, colDesc, user, user }) ) {
	...
	sql = " select id, patno, to_char(regid), note_date, 'CMS' as note_type, title, description, update_date, update_user ";
	sql += " , (select user_name from ah_sys_user x where x.user_id=a.update_user) as update_name ";
	sql += " from ipd_prg_note a where status<>'X' and regid=?  UNION ALL ";
	sql += "select 0, patno, regid, note_date, 'NIS', title, description, modify_date, modify_id, modify_name ";
	sql += " from nis_progress_note where regid=? order by 4 desc";

	record = UtilDBWeb.getReportableListCIS( sql, new String[]{regid, regid});
	...
	for (int i = 0; i < record.size(); i++) {
		...
		rowJSON.put("regid", row.getValue(2));
		rowJSON.put("note_date", row.getValue(3));
		rowJSON.put("note_type", row.getValue(4));
		rowJSON.put("title", row.getValue(5));
		rowJSON.put("description", row.getValue(6));
		rowJSON.put("update_date", row.getValue(7));
		rowJSON.put("update_user", row.getValue(8));
		rowJSON.put("update_name", row.getValue(9));
		recJSON.put(rowJSON);
	}
...
<div class="w3-container w3-row ah-pink" id=header>
	<div class="w3-col m7">
		<h3>Inpatient Progress Notes</h3>
	</div>
...
<div class="w3-row">
	<div id=buttonPanel class="w3-container" style="margin-top:4px;margin-bottom:4px;">
		...
		<div class="w3-right w3-hide">
			<button onclick="editProgressNote()">New Progress Note</button>
	...
	<div id=newPanel class="w3-container w3-row" style="height:110px;margin:12px;">
		<form name=frmNew id=frmNew action="inpProgressNotes.jsp" method="post" autocomplete="off">
...
<div class="w3-modal-content w3-container w3-row">
	...
	<form name=frmEdit id=frmEdit action="inpProgressNotes.jsp" method="post" autocomplete="off">
...
function showList(opt) {
	var count = 0
	var html = '<table class="w3-table w3-striped w3-border">'
	html += '<tr><th width=130px>Date</th><th width=90px">Category</th><th width=260px>Title</th>'
	html += '<th>Description</th><th width=180px>Updated By<th> </th></tr>';
