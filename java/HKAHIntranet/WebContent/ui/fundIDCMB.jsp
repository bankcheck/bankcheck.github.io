<%@ page import="java.util.*"%>
<%@ page import="com.hkah.util.*"%>
<%@ page import="com.hkah.util.db.*"%>
<%@ page import="com.hkah.web.common.*"%>
<%!
	private ArrayList fetchFund() {
		// fetch fund
		StringBuffer sqlStr = new StringBuffer();
		sqlStr.append("SELECT CRM_FUND_ID, CRM_FUND_DESC ");
		sqlStr.append("FROM   CRM_FUNDS ");
		sqlStr.append("WHERE  CRM_ENABLED = 1 ");
		sqlStr.append("ORDER BY CRM_FUND_DESC");

		return UtilDBWeb.getReportableList(sqlStr.toString());
	}
%>
<%
String fundID = request.getParameter("fundID");
boolean allowEmpty = "Y".equals(request.getParameter("allowEmpty"));
if (allowEmpty) {
	String emptyLabel = request.getParameter("emptyLabel");
	if (emptyLabel == null) {
		emptyLabel = "--- All Funds ---";
	}
%><option value=""><%=emptyLabel %></option><%
}
ArrayList record = fetchFund();
ReportableListObject row = null;
if (record.size() > 0) {
	for (int i = 0; i < record.size(); i++) {
		row = (ReportableListObject) record.get(i);
%><option value="<%=row.getValue(0) %>"<%=row.getValue(0).equals(fundID)?" selected":"" %>><%=row.getValue(1) %></option><%
	}
}
%>