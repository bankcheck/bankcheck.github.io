...
try {
	if ("1".equals(step)) {
		if ((createAction || updateAction) && (membershipDesc == null || membershipDesc.length() == 0)) {
			errorMessage = MessageResources.getMessage(session, "error.membership.required");
		} else if (createAction) {
			StringBuffer sqlStr = new StringBuffer();
			sqlStr.append("INSERT INTO CRM_CLUBS ");
			sqlStr.append("(CRM_SITE_CODE, CRM_CLUB_ID, CRM_CLUB_DESC, CRM_CREATED_USER, CRM_MODIFIED_USER) ");
			sqlStr.append("VALUES ");
			sqlStr.append("(?, (SELECT COUNT(1) + 1 FROM CRM_CLUBS), ?, ?, ?)");

			if (UtilDBWeb.updateQueue(
					sqlStr.toString(),
					new String[] { siteCode, membershipDesc, loginID, loginID} )) {
			...
			StringBuffer sqlStr = new StringBuffer();
			sqlStr.append("UPDATE CRM_CLUBS ");
			sqlStr.append("SET    CRM_SITE_CODE = ?, CRM_CLUB_DESC = ?, ");
			...
			if (UtilDBWeb.updateQueue(
					sqlStr.toString(),
					new String[] { siteCode, membershipDesc, loginID, membershipID } )) {
			...
			if (UtilDBWeb.updateQueue(
					sqlStr.toString(),
					new String[] { loginID, membershipID } )) {
	...
	if (!createAction && !"1".equals(step)) {
		if (membershipID != null && membershipID.length() > 0) {
			StringBuffer sqlStr = new StringBuffer();
			sqlStr.append("SELECT CRM_SITE_CODE, CRM_CLUB_DESC ");
			sqlStr.append("FROM   CRM_CLUBS ");
			sqlStr.append("WHERE  CRM_ENABLED = 1 ");
			sqlStr.append("AND    CRM_CLUB_ID = ?");

			ArrayList record = UtilDBWeb.getReportableList(sqlStr.toString(), new String[] { membershipID });
			if (record.size() > 0) {
				ReportableListObject row = (ReportableListObject) record.get(0);
				siteCode = row.getValue(0);
				membershipDesc = row.getValue(1);
			} else {
				closeAction = true;
			}
		} else {
...
<%
	...
	// set submit label
	title = "function.membership." + commandType;
%>
...
<%	} %>
	...
	<tr class="smallText">
		...
		<td class="infoData" width="70%">
<%	if (createAction || updateAction) { %>
			<input type="textfield" name="membershipDesc" value="<%=membershipDesc==null?"":membershipDesc %>" maxlength="100" size="50">
<%	} else { %>
			<%=membershipDesc==null?"":membershipDesc %>
<%	} %>
		</td>
	</tr>
</table>
...
<%	if (createAction || updateAction) { %>
		if (cmd == 'create' || cmd == 'update') {
			if (document.form1.membershipDesc.value == '') {
				alert("<bean:message key="error.membership.required" />.");
				document.form1.membershipDesc.focus();
				return false;
			}
		}
<%	} %>
