...
String question =TextUtil.parseStrUTF8(ParserUtil.getParameter(request, "question"));
String answer = TextUtil.parseStrUTF8(ParserUtil.getParameter(request, "answer"));
String modifieddate = "";
...
%>
<jsp:forward page="newstart_question.jsp">
...
<%
		...
		if ((createAction || updateAction) && (answer == null || answer.length() == 0)) {
			...
			sqlStr.append("WHERE  CRM_SITE_CODE = '" + ConstantsServerSide.SITE_CODE + "' AND CRM_QUESTIONAIRE_ID = ? AND CRM_QUESTIONAIRE_AID = ?");

			if (UtilDBWeb.updateQueue(
					sqlStr.toString(),
					new String[] {answer, loginID, questionnaireID,  questionnaireAID} )) {
...
<%
	...
	// set submit label
	title = "function.crm.question." + commandType;
%>
...
<form name="form1" action="newstart_question_detail.jsp" method="post">
...
%>
			<button onclick="return submitAction('view', 0, '' , 'cancel');" class="btn-click"><bean:message key="button.cancel" /> - <bean:message key="<%=title %>" /></button>
<%
		}
	} else {%>
...
<table width="100%" border="0">
	<tr class="smallText">
		<td align="center">
			<button onclick="return submitAction('<%=commandType %>', 2 , '<%=questionnaireAID %>');" class="btn-click"><bean:message key="button.save" /> - <bean:message key="<%=title %>" /></button>
<%
		if (questionnaireAID != null && !"".equals(questionnaireAID)) {		
%>
			<button onclick="return submitAction('view', 0);" class="btn-click"><bean:message key="button.cancel" /> - <bean:message key="<%=title %>" /></button>
<%		}else{
%>
...
<script language="javascript">
	function submitAction(cmd, stp,aid , type) {
		...
		if(type=='cancel'){
			document.form1.action = "newstart_question.jsp";
		}
