...
request.setAttribute("event_list", ScheduleDB.getList("lmc.crm", null, eventDesc));
...
<display:table id="row" name="requestScope.event_list" export="true" pagesize="<%=userBean.getNoOfRecPerPage() %>" class="tablesorter">
	...
	<display:column titleKey="prompt.action" media="html" style="width:20%; text-align:center">
		<% if (userBean.isAccessible("function.event.view")) { %>
			...
			<button onclick="return submitAction('enrolledClients', '<c:out value="${row.fields2}" />', '<c:out value="${row.fields7}" />');">Enrolled Clients</button>		
		<% } else { %>
			N/A
		<% } %>
	</display:column>
...
<script language="javascript">
	...
	function submitAction(cmd, cid, sid) {		
		if(cmd == "enrolledClients"){
			callPopUpWindow("client_list.jsp?type=enrolledClients&eventID=" + cid + "&scheduleID=" + sid);
		}else{
