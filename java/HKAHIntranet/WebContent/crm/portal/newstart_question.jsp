...
<%!
	...
	private static String getNextQuestionnaireID() {
		String questionnaireID = null;

		ArrayList result = UtilDBWeb.getReportableList(
				"SELECT COUNT(1) + 1 FROM CRM_QUESTIONAIRE");
		if (result.size() > 0) {
			ReportableListObject reportableListObject = (ReportableListObject) result.get(0);
			questionnaireID = reportableListObject.getValue(0);
		}
...
try {
	if ("1".equals(step)) {
		if ((createAction || updateAction) && (topic == null || topic.length() == 0)) {
			...
			StringBuffer sqlStr = new StringBuffer();
			questionnaireID = getNextQuestionnaireID();
			sqlStr.append("INSERT INTO CRM_QUESTIONAIRE (");
			sqlStr.append(" CRM_SITE_CODE, CRM_QUESTIONAIRE_ID, CRM_QUESTIONAIRE_DESC,  ");
			sqlStr.append(" CRM_CREATED_USER, CRM_MODIFIED_USER) ");
			sqlStr.append("VALUES ('" + ConstantsServerSide.SITE_CODE + "', "+questionnaireID+", ?, ?, ?)");
			...
			sqlStr.append("SET    CRM_QUESTIONAIRE_DESC = ?, ");
			...
			if (UtilDBWeb.updateQueue(
					sqlStr.toString(),
					new String[] {topic,loginID, questionnaireID} )) {
...
<form name="form1" action="newstart_question.jsp" method="post">
...
<%	} %>
		if (cmd == 'create.detail') {
			document.form1.action = "newstart_question_detail.jsp";
