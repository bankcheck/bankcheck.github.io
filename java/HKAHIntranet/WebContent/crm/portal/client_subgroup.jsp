...
String subGrpDesc = null;
String parentGrpID = null;
String parentGrpDesc = null;
...
try {	
	if ("1".equals(step)) {			
		...
		if (clientID != null || clientIDSelect2IsEmpty) {
			...
			sqlStr.append("WHERE CRM_GROUP_ID = '"+subGrpID+"' ");
		...
		if (updateAction) {
			...
			if (clientID != null) {
				for (int i = 0; i < clientID.length; i++) {
					...
					sqlStr.append("VALUES ('"+userBean.getSiteCode()+"' , '"+subGrpID+"' ,'team_leader' ,'"+clientID[i]+"' ,1 ,'"+userBean.getLoginID()+"' ,'"+userBean.getLoginID()+"' ) ");
	...
	if(subGrpID != null && subGrpID.length()>0){
		StringBuffer sqlStr = new StringBuffer();
		ArrayList record =null;		
		
		sqlStr.setLength(0);
		sqlStr.append("SELECT CRM_SITE_CODE, CRM_GROUP_DESC, CRM_PARENT_GROUP_ID ");
		sqlStr.append("FROM   CRM_GROUP ");
		sqlStr.append("WHERE  CRM_ENABLED = 1 ");
		sqlStr.append("AND    CRM_GROUP_ID = "+subGrpID+" ");			
		...
		if (record.size() > 0) {
			...
			subGrpDesc = row.getValue(1);
			parentGrpID = row.getValue(2);
			...
			if(parentGrpID != null && parentGrpID.length() > 0){
				...
				sqlStr.append("FROM   CRM_GROUP ");
				sqlStr.append("WHERE  CRM_ENABLED = 1 ");
				sqlStr.append("AND    CRM_GROUP_ID = "+parentGrpID+" ");			
	...
	// load data from database
		if (parentGrpID != null && parentGrpID.length() > 0) {
			...
			record = UtilDBWeb.getReportableList(sqlStr.toString(), new String[] { parentGrpID });
			if (record.size() > 0) {
	...
	if (subGrpID != null && subGrpID.length() > 0) {
		...
		sqlStr.append("and       G.CRM_GROUP_ID = '"+subGrpID+"' ");
		sqlStr.append("and       G.CRM_GROUP_POSITION = 'team_leader' ");
		sqlStr.append("and       C.CRM_GROUP_ID = '"+parentGrpID+"' ");
...
<form name="form1" action="client_subgroup.jsp" method="post">
<table cellpadding="0" cellspacing="5"
	class="contentFrameMenu" border="0">
	<tr class="smallText">
		<td class="infoLabel" width="30%">Team Name</td>
		<td class="infoData" width="70%">
			<%=parentGrpDesc==null?"":parentGrpDesc %>
		</td>
	</tr>
	<tr class="smallText">
		<td class="infoLabel" width="30%">Sub Team Name</td>
		<td class="infoData" width="70%">
			<%=subGrpDesc==null?"":subGrpDesc %>
		</td>
	</tr>
	
	<tr class="smallText">
		<td class="infoLabel" width="30%">Team Leader Name</td>
		<td class="infoData" width="70%">
<%			if (updateAction) { %>
			<table border="0">
				<tr>
					 ...
					 <td>
						<select name="clientID" size="10" multiple id="clientIDSelect2">
							...
							<jsp:include page="../../ui/clientGroupLeaderCMB.jsp" flush="true">
								...
								<jsp:param name="groupID" value="<%=parentGrpID %>" />
							</jsp:include>
						</select>
						<div>
							<bean:message key="button.search" />:&nbsp;
...
<%	if (updateAction) { %>
			...
			<button onclick="return submitAction('view', 0);" class="btn-click"><bean:message key="button.cancel" /> - Update Team Leader</button>
<%	} else { %>
			<button onclick="return submitAction('update', 0);" class="btn-click">Update Team Leader</button>
			<button onclick="window.close();" class="btn-click"><bean:message key="button.cancel" /> - Update Team Leader</button>
<%	}  %>
...
<input type="hidden" name="subGrpID" value="<%=subGrpID%>">			
</form>
