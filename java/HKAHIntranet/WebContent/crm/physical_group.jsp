...
try {
	if ("1".equals(step)) {
		if (createAction) {
			groupID = CRMPhysicalDB.addGroup(userBean, groupDesc);
			if (groupID != null) {
				message = "physical created.";
				createAction = false;
			} else {
				errorMessage = "physical create fail.";
			}
		} else if (updateAction) {
			if (CRMPhysicalDB.updateGroup(userBean, groupID, groupDesc)) {
				message = "physical updated.";
				updateAction = false;
			} else {
				errorMessage = "physical update fail.";
			}
		} else if (deleteAction) {
			if (CRMPhysicalDB.deleteGroup(userBean, groupID)) {
				message = "physical removed.";
				closeAction = true;
			} else {
				errorMessage = "physical remove fail.";
			}
		}
		step = null;
	} else if ("3".equals(step)) {
		if (createAction) {
			figureID = CRMPhysicalDB.addFigure(userBean, groupID, figureDesc, measure, type);
			if (figureID != null) {
				message = "physical figure created.";
				createAction = false;
			} else {
				errorMessage = "physical figure create fail.";
			}
		} else if (updateAction) {
			if (CRMPhysicalDB.updateFigure(userBean, groupID, figureID, figureDesc, measure, type)) {
				message = "physical figure updated.";
				updateAction = false;
			} else {
				errorMessage = "physical figure update fail.";
			}
		} else if (deleteAction) {
			if (CRMPhysicalDB.deleteFigure(userBean, groupID, figureID)) {
				message = "physical figure removed.";
				deleteAction = false;
			} else {
				errorMessage = "physical figure remove fail.";
			}
		}
		step = null;
	...
	if (!createAction && !"1".equals(step)) {
		if (groupID != null && groupID.length() > 0) {
			ArrayList record = CRMPhysicalDB.getGroup(groupID);
			if (record.size() > 0) {
				ReportableListObject row = (ReportableListObject) record.get(0);
				groupDesc = row.getValue(0);
				if (figureID != null && figureID.length() > 0) {
					record = CRMPhysicalDB.getFigure(figureID);
					if (record.size() > 0) {
						row = (ReportableListObject) record.get(0);
						figureDesc = row.getValue(0);
						measure = row.getValue(1);
						type = row.getValue(2);
					} else {
						closeAction = true;
					}
...
<%
	...
	if (levelTwo) {
		...
		title = "function.crm.physicalGroup." + commandType;
	}
%>
...
<%	} else { %>
			<button onclick="return submitAction('update', 0);" class="btn-click"><bean:message key="function.crm.physicalGroup.update" /></button>
			<button class="btn-delete"><bean:message key="function.crm.physicalGroup.delete" /></button>
<%	} %>
		</td>
	</tr>
</table>
...
<%
			...
			request.setAttribute("physicalFigure_list", CRMPhysicalDB.getFigureList(groupID));
%>
<bean:define id="functionLabel"><bean:message key="function.crm.physicalGroup.list" /></bean:define>
