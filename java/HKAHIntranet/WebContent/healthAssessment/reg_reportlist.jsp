...
if(command != null) {
	if (SearchRecord = true) {
		if ((input_date_from == "") || (input_date_from == null)) {
			...
			//input_date_from = current_month[0];
		}
		if ((input_date_to == "") || (input_date_to == null)) {
			input_date_to = DateTimeUtil.getCurrentDateTime().substring(0, 10);
			//input_date_to = current_month[0];
			//input_date_to = current_month[1];
		}
		viewList = HAFormDB.getRegReportList(drCode, patNo, patName, patIDNo, patTel, completed, input_date_from, input_date_to);
		request.setAttribute("reportList", viewList);	
	} else if (SaveTrafficAction) { // save pi remark
//		PiReportDB.updateTrafficLight(patNos, traffics, piremarks, piremarks2, piremarks3);
		...
		<DIV id=mainFrame>
			<DIV id=contentFrame>
				<jsp:include page="../common/page_title.jsp" flush="false">
					<jsp:param name="pageTitle" value="P.E. Registered List" />
				...
				<form name="searchfrom" action="reg_reportlist.jsp" method="post">
				...
				<display:table id="row" name="requestScope.reportList" export="true" pagesize="<%=userBean.getNoOfRecPerPage() %>" class="generaltable"
					...
					<display:column property="fields0" title=" Reg Date" style="text-align:center;width:1%" />
					<c:set var="tempRegDate" value="${row.fields0}"/>
					<display:column property="fields2" title="Reg ID" style="text-align:center;width:1%" />
					<c:set var="tempRegID" value="${row.fields2}"/>					
					<display:column property="fields11" title="Dr Name" style="text-align:center;width:4%" />
					<display:column property="fields1" title="Pat No" style="text-align:center;width:1%" />
					<c:set var="tempPatNo" value="${row.fields1}"/>
					<display:column property="fields3" title="Eng Name" style="text-align:left;width:3%" />
					<display:column property="fields4" title="Chi Name" style="text-align:left;width:3%" />
					<display:column property="fields5" title="ID No" style="text-align:center;width:2%" />
					<display:column property="fields6" title="Birth date" style="text-align:center;width:1%" />
					<display:column property="fields7" title="Sex" style="text-align:center;width:1%" />					
					<display:column property="fields9" title="Tel" style="text-align:center;width:1%" />
					<display:column titleKey="prompt.action" media="html" style="width:5%; text-align:center">
<% 
String tempRegID = (String)pageContext.getAttribute("tempRegID");
String tempPatNo= (String)pageContext.getAttribute("tempPatNo");
String tempRegDate = (String)pageContext.getAttribute("tempRegDate");
if (HAFormDB.NeedAddRegHAForm(tempPatNo, tempRegID, "NORMAL", tempRegDate)) { 
%>
					<button onclick="return editReport('<c:out value="${row.fields1}" />', '<c:out value="${row.fields2}" />', '<c:out value="1" />', '<c:out value="NORMAL" />', '<c:out value="${row.fields0}" />');">Add P.E. Form</button>
<%
}
%>					
					<%--
					<button onclick="return printReport('pdfAction', '<c:out value="${row.fields1}" />', '<c:out value="${row.fields17}" />');"><bean:message key='button.print' /></button>						
					 --%>
					</display:column>
				...
				<script language="javascript">
					...
					function editReport(patNo, regid, seqno, formtype, regDate) {
						//alert(patNo + ' ' + regid);
						newPopup('health_assess_form.jsp?staffID=<%=staffID%>&patNo=' + patNo + '&regID=' + regid + '&seqNo=' + seqno + '&formType=' + formtype + '&regDate=' + regDate);
						return false;
					}

					function printReport(action, haID, formtype) {
						if(action == 'pdfAction') {
							callPopUpWindow("print_report.jsp?command="+action+"&haID="+haID+"&formType=" + formtype);
						}	
						return false;
					}
