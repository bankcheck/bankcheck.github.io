...
<%!
private String getSelectTWAHStaff(boolean isDebug) {
	String tableName = isDebug ? "PERSONAL_DATA_TABLE" : "TW_PERSONAL_DATA_TABLE";
	return "SELECT PERSON_CODE,NAME,SURNAME,GIVEN_NAME,CHIN_FULL_NAME,CALLED_NAME,HOSPITAL_NO,DEPT_CODE,DEPT_NAME,POSITION_CODE,POSITION_NAME,FTE,TO_CHAR(DATE_OF_AVAILABILITY, 'DD/MM/YYYY'),TO_CHAR(RESIGNATION_DATE, 'DD/MM/YYYY'),STAFF_TYPE,FORMER_EMP_ID,EMAIL_ADDRESS FROM " + tableName + "@HR_COL ORDER BY PERSON_CODE";
}
%>
<%
...
<title>HKAH-TW Staff List (COL)</title>
</head>
<body>
<div>Connecting to HKAH-TW Staff List (COL)...</div>
<%
	String run = request.getParameter("run");
	boolean isRun = false;
	if ("1".equals(run)) {
			try {
			   //ArrayList result = UtilDBWeb.getReportableList(getSelectTWAHStaff(ConstantsServerSide.DEBUG));
			   ArrayList result = UtilDBWeb.getReportableList(getSelectTWAHStaff(false));
			   ...
			   if (result.size() > 0) {
				  ...
				  // UtilDBWeb.updateQueue(sqlStr_resetStaff, new String[] { ConstantsServerSide.SITE_CODE });
        		   ...
        		   if (result != null) {
%>
Total number of staff in HKAH-TW HR system: <%=result.size() %><br />
<%
        		   } else {
%>
