...
<%if("S".equals(reqStatus)||"H".equals(reqStatus)){ %>
	<span id="showAmountRange_indicator">
	<select name="amountID" id="amountID" onchange="return getChangeApprovalList(this)">
...
<td style="width:20%;">
	<div></div>
	<div style="display:"";">											
		<input onchange="performChange(this, '0')" type="text" name="itemDesc[0].fields3"  
		value="" size="40" maxlength="100"/>
	</div>
</td>
<td style="width:8%;">	
		<div></div>
		<div style="display:"";">		
			<input onchange="performChange(this, '0')" type="text" name="itemUnit[0].fields7" 
...
<td style="width:8%; ">	
		<div></div>
		<div style="display:"";">		
			<input onchange="performChange(this, '0')" type="text" name="itemPrice[0].fields6"
			value="0" size="10" maxlength="10" alt="decimal-us"/>          											           	
		</div>
</td>
<td style="width:8%; ">	
		<div></div>
		<div style="display:"";">		
			<input onchange="performChange(this, '0')" type="text" name="itemQty[0].fields4"
...
<td style="width:8%; ">	
		<div></div>
		<div style="display:"";">		
			<input onchange="performChange(this, '0')" type="text" name="itemAmount[0].fields5"
...
<display:table pagesize="" requestURI="" class="tablesorter" name="EPO" sort="page" id="resList">
    ...
    <display:column title="Item Desc" style="width:20%;">
		...
		<div style="display:none;">											
			<input onchange="performChange(this, '<c:out value="${resList_rowNum - 1}" />')" type="text" name="itemDesc[${resList_rowNum - 1}].fields3"  
			value="${EPO[resList_rowNum - 1].fields3}" size="40" maxlength="100"/>
		</div>
	</display:column>
	<display:column style="width:8%;" title="Unit">	
		<div>${EPO[resList_rowNum - 1].fields7}</div>
		<div style="display:none;">		
			<input onchange="performChange(this, '<c:out value="${resList_rowNum - 1}" />')" type="text" name="itemUnit[${resList_rowNum - 1}].fields7" 
			value="${EPO[resList_rowNum - 1].fields7}" size="10" maxlength="50"/>					           											           	
        </div>
    </display:column>
	...
	<display:column style="width:8%; " title="QTY">	
		...
		<div style="display:none;">		
			...
			value="${EPO[resList_rowNum - 1].fields4}"  size="6" maxlength="6"/>					           											           	
        </div>
    </display:column>
...
<%} %>	 
	<input type="hidden" name="appGrp" id="appGrp" value="<%=appGrp==null?"":appGrp %>"/>
	...
	case "itemQty["+index+"].fields4":
		if(objValue==null||objValue==''){
			...
			if(isNaN(objValue)){
				alert('Please enter valid Quality');
				inputObj.value=0;
				document.getElementsByName("itemAmount["+index+"].fields5")[0].value = parseFloat("0").toFixed(2);
				document.getElementsByName("itemAmountHid["+index+"]")[0].value = parseFloat("0").toFixed(2);				
				inputObj.focus();
				inputObj.select();						
			}else if(objValue=='0'){
				...
				document.getElementsByName("itemAmount["+index+"].fields5")[0].value = amount.toFixed(2);
				document.getElementsByName("itemAmountHid["+index+"]")[0].value = amount.toFixed(2);				
			}
		}
...
function addTotalAmount(col,del){
	...
	if(del==0){	
		document.getElementsByName("itemTotalAmount["+ lastRowIndex +"].fields5")[0].value = parseFloat(totalAmt).toFixed(2);
		if(totalAmt>0){
	...
	//		var size5 = new Array(1,20,15);		
		for(i = 0; i < col+1; i++){ 
			...
			if(i==0){
				...
				+ "' value='"+parseFloat(totalAmt).toFixed(2)+"' size='10' maxlength='10' disabled='disabled'/>";
				// fails in IE > 8 and other browsers
				try {	
					inputObj = document.createElement(objString);			
				} catch (err) {
...
// without total amount					
		...
		for(i = 0; i < col+1; i++){ 
			...
			if(i==0){
				...
				+ "' value='"+parseFloat("0").toFixed(2)+"' size='10' maxlength='10' disabled='disabled'/>";
				// fails in IE > 8 and other browsers
				try {	
					inputObj = document.createElement(objString);			
				} catch (err) {
...
function getTotalAmount(totalAmt){
	var appGrp = document.getElementById("appGrp").value;
	
	$.ajax({
		type: "POST",
		url: "epo_hidden.jsp",
		data: 'p1=4&p2=' + totalAmt + '&p3=' + appGrp,
		async: false,
		success: function(values){			
			if(values != null) {				
				if(document.getElementById("amountID").value != trim(values)){
					getChangeApprovalListByVal(trim(values));
				}				
...
function changeAmtRange(amtID,appGrp,reqStatus) {
	$.ajax({
		type: "POST",
		url: "../ui/approvalAmountCMB.jsp" ,
		data: "amountID=" + amtID + "&appGrp="+appGrp,
		success: function(values){
			if(values != '') {
				$("#showAmountRange_indicator").html("<select name='amountID' id='amountID' onchange='return getChangeApprovalList(this)'>" + values + "</select>");
			}//if
		}//success
	});//$.ajax			
}
