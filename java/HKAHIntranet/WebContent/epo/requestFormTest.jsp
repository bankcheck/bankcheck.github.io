...
//String reqSiteCode = StaffDB.getStaffSiteCode(userBean.getLoginID());
...
*/
		ArrayList record1 = ApprovalUserDB.getEpoAppUserList("1", "1", sendAppTo, null, "HKAH");
		if(record1.size()>0){
			isSamePos = true;
		}
		...
		if(isSamePos){
			...
			reqNo = EPORequestDB.addReqHdr(userBean, reqDate, reqSiteCode, reqDept, shipTo, reqDesc, reqBugcode, amtID, reqRmk, sendAppTo, folderID, sysDate, adCouncil, boardCouncil, financeComm, purRmk, reqStatus);
		}else if("360".equals(reqDept)){
			reqStatus = "H";			
			reqNo = EPORequestDB.addReqHdr(userBean, reqDate, reqSiteCode, reqDept, shipTo, reqDesc, reqBugcode, amtID, reqRmk, "clara.leung", folderID, sysDate, adCouncil, boardCouncil, financeComm, purRmk, reqStatus);			
		}else{
			reqStatus = "S";			
			reqNo = EPORequestDB.addReqHdr(userBean, reqDate, reqSiteCode, reqDept, shipTo, reqDesc, reqBugcode, amtID, reqRmk, sendAppTo, folderID, sysDate, adCouncil, boardCouncil, financeComm, purRmk, reqStatus);			
		}
		...
		for( int i=0;i<=Integer.parseInt(lstRowIdx);i++) {
			...
			successAddDtl = EPORequestDB.addReqDtl(userBean, reqNo, Integer.toString(i+1), supplier, itemDesc, qty, amount, price, uom, itemRmk, sysDate, "0", null);             			
		...
		if ((reqNo!=null||reqNo.length()> 0)&&successAddDtl){
			if(EPORequestDB.sendEmail(reqNo, userBean.getStaffID(), userBean.getLoginID(), approvalStaff, reqDesc, reqStatus, folderID, "1", "1")){
		...
		for( int i=0;i<=Integer.parseInt(lstRowIdx);i++) {
			...
			if(itemDesc!=null && itemDesc.length()>0){			
				successAddDtl = EPORequestDB.addReqDtl(userBean, reqNo, Integer.toString(i+1), supplier, itemDesc, qty, amount, price, uom, itemRmk, sysDate);
				EPORequestDB.addReqDtlLog(userBean, reqNo, Integer.toString(i+1), supplier, itemDesc, qty, amount, price, uom, itemRmk, sysDate, "0", null);
			}
		}
...
// load data from database
		...
		if (reqNo != null && reqNo.length() > 0) {	
			...
			if(EPORequestDB.sendEmail(reqNo, null, userBean.getLoginID(), sendAppTo, reqDesc, reqStatus, folderID, "E", "1")){
...
<table cellpadding="0" cellspacing="5"
	...
	<tr class="smallText">
		<td align="center">
 		<%if(("S".equals(reqStatus)||"H".equals(reqStatus)) && !"admin".equals(userBean.getLoginID())){ %>				
			...
			<%} %>			
		<%} %>
 		<%if(("S".equals(reqStatus)||"H".equals(reqStatus))&& reqNo!=null && reqNo.length()>0 && !"admin".equals(userBean.getLoginID())){ %>				
