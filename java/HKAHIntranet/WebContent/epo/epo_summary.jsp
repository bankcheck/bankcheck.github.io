...
private ArrayList getHkahRecord(String fromDate, String toDate) {
	...
	sqlStr.append("(SELECT CD.CO_DEPARTMENT_DESC FROM CO_DEPARTMENTS@hkah CD WHERE CD.CO_DEPARTMENT_CODE = REQ_DEPT_CODE) AS DEPT_DESC,");
//	sqlStr.append("NVL(BC_DESC,NVL(AC_DESC,NULL)) AS REQ_DESC,");	
	sqlStr.append("REQ_DESC,");
	sqlStr.append("(SELECT CS.CO_STAFFNAME FROM CO_STAFFS@hkah CS WHERE CS.CO_STAFF_ID = APPROVED_BY AND CS.CO_SITE_CODE = 'hkah') AS APPROVED_BY,"); 
...
//	sqlStr.append("(SELECT EB.CODE FROM EPO_BUDGET@hkah EB WHERE EB.CODE = M.BUDGET_CODE AND EB.TYPE = 'BC') AS BC_DESC, (SELECT EB.CODE FROM EPO_BUDGET@hkah EB WHERE EB.CODE = M.AD_COUNCIL_NO AND EB.TYPE = 'AC') AS AC_DESC ");	
	sqlStr.append("null AS BC_DESC, null AS AC_DESC ");
	sqlStr.append("FROM EPO_REQUEST_M@hkah M, EPO_REQUEST_D@hkah D ");
...
private ArrayList getTwahRecord(String fromDate, String toDate) {
	StringBuffer sqlStr = new StringBuffer();
	...
	sqlStr.append("NVL(BC_DESC,NVL(AC_DESC,NULL)) AS REQ_DESC,");
//	sqlStr.append("REQ_DESC,");	
	sqlStr.append("NVL((SELECT CS.CO_STAFFNAME FROM CO_STAFFS CS WHERE CS.CO_STAFF_ID = APPROVED_BY AND CS.CO_SITE_CODE = 'twah'),(SELECT CS.CO_STAFFNAME FROM CO_STAFFS CS WHERE CS.CO_STAFF_ID = APPROVED_BY AND CS.CO_SITE_CODE = 'hkah')) AS APPROVED_BY,");
	sqlStr.append("REQ_DESC AS SUPPLIER_NAME, AMOUNT, (NVL(BUDGET_CODE,'NIL')||'/'||NVL(AD_COUNCIL_NO,'NIL')||'/'||NVL(BOARD_COUNCIL_NO,'NIL')||'/'||NVL(FINANCE_COMM_NO,'NIL')) AS BUDGET_CODE ");
	sqlStr.append("FROM"); 
	sqlStr.append("(SELECT M.REQ_NO, TO_CHAR(M.REQ_DATE,'DD/MM/YYYY') AS REQ_DATE, M.REQ_DEPT_CODE, M.REQ_DESC, M.APPROVED_BY, D.SUPPLIER_NAME, SUM(D.REQ_AMOUNT) AS AMOUNT, M.BUDGET_CODE, M.AD_COUNCIL_NO, M.BOARD_COUNCIL_NO, M.FINANCE_COMM_NO, ");
	sqlStr.append("(SELECT EB.CODE FROM EPO_BUDGET EB WHERE EB.CODE = M.BUDGET_CODE AND EB.TYPE = 'BC') AS BC_DESC, (SELECT EB.CODE FROM EPO_BUDGET EB WHERE EB.CODE = M.AD_COUNCIL_NO AND EB.TYPE = 'AC') AS AC_DESC ");
//	sqlStr.append("null AS BC_DESC, null AS AC_DESC ");
	...
	sqlStr.append("HAVING SUM(D.REQ_AMOUNT)>=10000) ");
	sqlStr.append("ORDER BY 1 ");
	System.err.println("[getTwahRecord]:"+sqlStr.toString());
	return UtilDBWeb.getReportableList(sqlStr.toString());
}
%>
<%
...
if(type!=null && type.length()>0){
	if("pdf".equals(type)){
		jasperName = "/report/epo_summary.jasper";
		jasperName_sub1 = "/report/epo_summary_Sub1.jasper";
		jasperName_sub2 = "/report/epo_summary_Sub2.jasper";
	}else if("xls".equals(type)){
		jasperName = "/report/epo_summary_xls.jasper";
		jasperName_sub1 = "/report/epo_summary_xls_Sub1.jasper";
		jasperName_sub2 = "/report/epo_summary_xls_Sub2.jasper";	
	}else{
		jasperName = "/report/epo_summary.jasper";
		jasperName_sub1 = "/report/epo_summary_Sub1.jasper";
		jasperName_sub2 = "/report/epo_summary_Sub2.jasper";	
	...
	File subReportFile2 = new File(application.getRealPath(jasperName_sub2));
	...
	if (reportFile.exists()) {
		if("pdf".equals(type) || "xls".equals(type)){
			...
			parameters.put("SubDataSource1", 
					new ReportMapDataSource(getTwahRecord(fromDate,toDate), new String[]{"REQ_NO", "REQ_DATE", "DEPT_DESC", "REQ_DESC", "APPROVED_BY", "SUPPLIER_NAME", "AMOUNT", "BUDGET_CODE"}, new boolean[]{false,false,false,false,false,false,true,false}));
		...
		<DIV id=mainFrame>
			<DIV id=contentFrame>
				<jsp:include page="../common/page_title.jsp" flush="false">
					<jsp:param name="pageTitle" value="EPO Summary" />
					<jsp:param name="category" value="Report" />
				</jsp:include>

				<form name="search_form" action="epo_summary.jsp" method="get">
