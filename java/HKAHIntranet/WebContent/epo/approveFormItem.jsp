...
int alreadyApproval = Integer.parseInt(request.getParameter("alreadyApproval"));
...
<%System.err.println("[appGrp]:"+appGrp); %>
<%if("S".equals(reqStatus) || "F".equals(reqStatus)){ %>
...
<display:table pagesize=""  requestURI="" class="tablesorter" name="EPO" sort="page" id="resList">	   
	...
	<display:column style="width:20%;" title="Supplier Name">
		<input type="hidden" name="itemSupp[${resList_rowNum - 1}].fields2"
		value="${EPO[resList_rowNum - 1].fields2}"/>		
		<div>${EPO[resList_rowNum - 1].fields2}</div>
    </display:column>    				           	
    <display:column title="Item Desc" style="width:20%;">
		<input type="hidden" name="itemDesc[${resList_rowNum - 1}].fields3"
		value="${EPO[resList_rowNum - 1].fields3}"/>    
		<div>${EPO[resList_rowNum - 1].fields3}</div>
	</display:column>
	<display:column style="width:8%;" title="Unit">
		<input type="hidden" name="itemUnit[${resList_rowNum - 1}].fields7"
		value="${EPO[resList_rowNum - 1].fields7}"/>	
	...
	<display:column style="width:8%; " title="Unit Price">	
		<div>			
			...
			value="${EPO[resList_rowNum - 1].fields6}" size="10" maxlength="10" readonly="readonly" />
		</div>
    </display:column>    	
	<display:column style="width:8%; " title="QTY">	
		<div style="display:"";">
<%if("A".equals(reqStatus)||"R".equals(reqStatus)||"H".equals(reqStatus)||("F".equals(reqStatus)&&alreadyApproval>0)){ %>			
			<input onchange="performChange(this, '<c:out value="${resList_rowNum - 1}" />')" type="text" name="itemQty[${resList_rowNum - 1}].fields4"
			value="${EPO[resList_rowNum - 1].fields4}"  size="6" maxlength="6" readonly="readonly" />
<%}else{ %>
		<logic:equal name="resList" property="fields13" value="0">
			<input onchange="performChange(this, '<c:out value="${resList_rowNum - 1}" />')" type="text" name="itemQty[${resList_rowNum - 1}].fields4"
			value="${EPO[resList_rowNum - 1].fields4}"  size="6" maxlength="6" />
		</logic:equal>
		<logic:equal name="resList" property="fields13" value="-1">
			<input onchange="performChange(this, '<c:out value="${resList_rowNum - 1}" />')" type="text" name="itemQty[${resList_rowNum - 1}].fields4"
			value="${EPO[resList_rowNum - 1].fields4}"  size="6" maxlength="6" readonly="readonly" />
		</logic:equal>
		<logic:equal name="resList" property="fields13" value="1">
...
<%} %>								           											           	
	...
	<display:column style="width:8%; " title="Amount">	
		<div style="display:"";">		
			...
			value="${EPO[resList_rowNum - 1].fields5}"  size="10" maxlength="10" readonly="readonly" />					           											           	
        </div>
    </display:column>        
...
<%}else if("F".equals(reqStatus)&&alreadyApproval==0){ %>
		<logic:equal name="resList" property="fields13" value="-1">
			<select name="itemApproval[${resList_rowNum - 1}].fields13" disabled="disabled">						
				<option value="1"<c:if test="${EPO[resList_rowNum - 1].fields13 == '1'}">selected</c:if>>Approved</option>
				<option value="-1"<c:if test="${EPO[resList_rowNum - 1].fields13 == '-1'}">selected</c:if>>Rejected</option>			
		...
		<logic:equal name="resList" property="fields13" value="1">
			<select name="itemApproval[${resList_rowNum - 1}].fields13" >						
				<option value="1"<c:if test="${EPO[resList_rowNum - 1].fields13 == '1'}">selected</c:if>>Approved</option>
				<option value="-1"<c:if test="${EPO[resList_rowNum - 1].fields13 == '-1'}">selected</c:if>>Rejected</option>			
			</select>
		</logic:equal>
...
function checkRowApproval(approveFlag){
	...
	case "F":
		if(x>0){
			...
			alert("All items are in reject status, no further approve needed, please select again.");
			return false;
		}else {
			return true;
		}							
...
function changeAmtRange(amtID,appGrp,reqStatus) {
	$.ajax({
		...
		success: function(values){
			if(values != '') {
				$("#showAmountRange_indicator").html("<select name='amountID' id='amountID'>" + values + "</select>");
			}//if
		}//success
	});//$.ajax			
}
