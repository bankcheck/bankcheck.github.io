create or replace
Function "NHS_LIS_MANPOWER_ENQ2"
(V_perf_dept IN varchar2, V_req_dept IN varchar2,
...
--    S_Week := '16';
    ...
    OPEN OUTCUR FOR
    select pp.PMP_SITE_CODE, pp.PMP_SERVICE_CAT, pp.PMP_REQUEST_DEPARTMENT_CODE,
       pp.PMP_PROJECT_ID, pp.PMP_MODULE_ID, pp.PMP_TASK_ID, pp.PMP_CURRENT_STATUS, 
       pp.PMP_PERFORM_USER_ID,
    ...
    WHERE pp.pmp_perform_DEPARTMENT_code like '%' || V_perf_dept || '%' and
          pp.PMP_REQUEST_DEPARTMENT_CODE like '%' || V_req_dept || '%' and
          pp.PMP_PROJECT_ID LIKE '%' || V_PROJECT || '%' and pp.PMP_MODULE_ID like '%' || v_module  || '%' and 
          pp.PMP_TASK_ID like '%' || v_task || '%'
    order by pp.PMP_PERFORM_USER_ID;

    RETURN OUTCUR;
END NHS_LIS_MANPOWER_ENQ2;