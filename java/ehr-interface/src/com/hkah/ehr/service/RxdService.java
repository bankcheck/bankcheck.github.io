...
public class RxdService {
	private static Logger logger = Logger.getLogger(RxdService.class);
	...
	public Response uploadBlank(List<String> patnos) {
		logger.info("[uploadBlankRxd]");
		
		Map<Participant, RxdDetail> map = new HashMap<Participant, RxdDetail>();
		for (String patno : patnos) {
			map.put(getDomainCommonService().getEhrHl7Participant4DomainByPatNo(patno), null);
		}
		 
		return uploadToEhr(map, ConstantsEhr.UPLOAD_MODE_CODE_BLM, false);
	}
	
	public Response uploadToEhr(Map<Participant, RxdDetail> map, String mode, boolean isUpdatePatInit){
		...
		msg = heprWebServiceClient.uploadRxdData(map, fileMap_BL_M, uploadMode);
        ...
        if (!msg.getRecordDetail().isEmpty()) {
            ...
            for (Entry<Participant, RxdDetail> entry : map.entrySet()) {
        	...
        	if (isSuccess) {
	            ...
	            for (int i = 0; i < patientUploadResps.size(); i++) {
	            	...
	            	if(uploadPatInitBirth == true) {
	            		domainCommonService.updatePatInit(patientUploadResps.get(i).getPatNo(), ConstantsEhr.DOMAIN_CODE_DISP);
	            	}
	            }
	        }
        }
