	...
	private void generateKey() {
		...
		try {
            ...
            if (rs.next()) {
                ...
                System.out.println("[RxdScheduler] gen key:" + this.recordKey);         	
            }
                        
        } catch (Exception e) {
            System.out.println("[RxdScheduler] Cannot generate record key");
            e.printStackTrace();
        	...
        	try {
                ...
                System.out.println("[RxdScheduler] Cannot close connection");
                e.printStackTrace();
        	}
        }
	...
	private Participant participant(String patno){
		...
		try {
        	...
        	*/
            ...
            System.out.println("[RxdScheduler] Participant Error, patno:" + patno );
            e.printStackTrace();
...
//	@Scheduled(cron="0 0/10 * * * ?")
	public void ehrSchedule(){
        ...
        System.out.println("[RxdScheduler] Start");
		...
		try {
			...
			while (rs.next()) {
	            System.out.println("[RxdScheduler] query pooling" );

            	patno = rs.getString("patno");

				System.out.println("[RxdScheduler] patno: " + patno );
				...
				// production
	            System.out.println("[RxdScheduler] debug 1 : ehrdob_dtm = " + rs.getString("ehrdob_dtm") );
	        	...
	        	*/
					...
					System.out.println("[RxdScheduler] debug : hcp_id, hcp_name = " + FactoryBase.getInstance().getSysparamValue("hcp_id") + ", " + FactoryBase.getInstance().getSysparamValue("hcp_name") ) ; 
					...
					//if (response!= null) {
					//	System.out.println("[RxdScheduler] Return code: " + response.getResponseCode() + " Return message: " + response.getResponseMessage());
					//}
					//
		            ...
		            //System.out.println("[RxdScheduler] debug 2 : p.getBirthDate() 2 " + p.getBirthDate() );
            ...
            if ( pMap_BL.size() > 0 ) {
    			System.out.println("[RxdScheduler] call heprWebServiceClient, BL");
            ...
            if ( pMap_BL_M.size() > 0 ) {
    			System.out.println("[RxdScheduler] call heprWebServiceClient, BL-M");
            ...
            System.out.println("[RxdScheduler] Complete");
			
        } catch (Exception e) {
            System.out.println("[RxdScheduler] Error");
            e.printStackTrace();
