...
public class EncounterScheduler {
	protected static Logger logger = Logger.getLogger(EncounterScheduler.class);	
	private static final String DEFAULT_STARTDT = "01/01/2017";
...
//  production :    
//	@Scheduled(cron="0 45 3 * * ?")		// PROD start at 3:45am everyday
	public void schedule(){
		logger.info("EncounterScheduler Start");
				
		Map<Participant, List<EncounterDetail>> dmMap = new HashMap<Participant, List<EncounterDetail>>();
        Map<Participant, List<EncounterDetail>> incMap = new HashMap<Participant, List<EncounterDetail>>();		
		
		Map<Participant, List<EncounterDetail>> dmMapDisc = new HashMap<Participant, List<EncounterDetail>>();
        Map<Participant, List<EncounterDetail>> incMapDisc = new HashMap<Participant, List<EncounterDetail>>();		
        ...
        try {
			...
			String delay = getEhrEncDelay();
			logger.info("Fetch data delay (day)="+delay);
			
			
			/**
			 * HKAH Scenario:
				SCN1 Create appointment for inpatient				YES
				SCN2 Create appointment for outpatient				YES
				SCN3 Create appointment for other encounter type	YES (Daycase)
				SCN4 Update appointment for inpatient
				SCN5 Update appointment for outpatient
				SCN6 Update appointment for other encounter type
				SCN7 Cancel appointment for inpatient
				SCN8 Cancel appointment for outpatient
				SCN9 Cancel appointment for other encounter type
				SCN10 Create admission for inpatient				YES
				SCN11 Create admission for A&E patient				
				SCN12 Create attendance for outpatient				YES
				SCN13 Create attendance for other encounter type	YES (Daycase)
				SCN14 Update admission for inpatient
				SCN15 Update admission for A&E patient
				SCN16 Update attendance for outpatient
				SCN17 Update attendlance for other encounter type
				SCN18 Cancel admission for inpatient
				SCN19 Cancel admission for A&E patient
				SCN20 Cancel attendance for outpatient
				SCN21 Cancel attendance for other encounter type
				SCN22 Discharge of inpatient						YES
				SCN23 Discharge of A&E patient
				SCN24 Cancel discharge of inpatient
				SCN25 Cancel discharge of A&E patient
			 ...
			 */
			getEhrEncounter(dmMap, "BL-M", delay);
			getEhrEncounter(incMap, "BL", delay);
			...
			debugParticipant(dmMap, "BLM");
			debugParticipant(incMap, "BL");
	...
	private void uploadToEhr(Map<Participant, List<EncounterDetail>> map, String mode, Date startDate, Date endDate){
		...
		if (!map.isEmpty()) {
	        ...
	        Response msg = wsClient.uploadEncounterData(map, uploadMode);
	        	...
	        	if (isSuccess) {
		            ...
		            for (int i = 0; i < patInfos.size(); i++) {
		            	...
		            	if(uploadPatInitBirth == true) {
		            		updatePatInitEnctr(patInfos.get(i).getPatNo());
		            	}
		            }
		        }
	        }
        	        ...
        	        if (transactionProfileTypePrefix != null && transactionProfileTypePrefix.startsWith("ADM")) {	// admission
	        	       ...
	        	       if("I".equals(regType)){	// SCN10 (ADM-IP)
	        			...
	        			String inpid = rs.getString("inpid");
    	       			...
    	       			if (dischargeType != null && !dischargeType.isEmpty() && !"null".equals(dischargeType)) {
	...
	private void updateDataUploadLog(List<PatientInfo> patInfos, List<String> patErrorList, Date uploadDate, 
		...
		try {
			...
			for (PatientInfo pInfo : patInfos) {
				int i = 1;
				ps.setString(i++, ConstantsEhr.DOMAIN_CODE_ENCTR);
	...
	private void updatePatInitEnctr(String patno) {
        ...
        String sql;
		try {
			sql = "update ehr_pmi set InitEnctr = SYSDATE where patno = '" + patno + "'";
			conn = ConnUtil.getDataSourceHATS().getConnection();
        	ps = conn.prepareStatement(sql);		
			rs = ps.executeQuery();
		} catch (Exception e) {
	...
	private Date getLastEndDate() {
		...
		try {
			sql = "select param1 from sysparam where parcde = 'EHRUL_ENC'";
			conn = ConnUtil.getDataSourceHATS().getConnection();
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
            ...
            logger.error("getLastEndDate Error");
            e.printStackTrace();
        } finally {
        	try {
        		if (rs != null)
                ...
                logger.error("getLastEndDate Cannot close connection");
                e.printStackTrace();
        	}
        }		
		return lastEndDate;
	...
	private void saveLastEndDate(String dbDateStr) {
		logger.info("Update EndDate (EHRUL_ENC): " + dbDateStr);
		...
		try {
			sql = "update sysparam set param1 = ? where parcde = 'EHRUL_ENC'";
			conn = ConnUtil.getDataSourceHATS().getConnection();
			ps = conn.prepareStatement(sql);
	...
	private String getEhrEncDelay() {
       PreparedStatement ps = null;
		...
		try {
			sql = "select param1 from sysparam where parcde = '"+ConstantsEhr.SYSPARAM_EHRENCDLY+"'"; 
			conn = ConnUtil.getDataSourceHATS().getConnection();
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			
			if (rs.next()) {
				delay = rs.getString(1);
			}
		} catch (Exception e) {
            logger.error("getEhrEncDelay Error");
            e.printStackTrace();
        } finally {
        	try {
        		if (rs != null)
                ...
                logger.error("getEhrEncDelay Cannot close connection");
                e.printStackTrace();
        	}
        }		
	...
	private void debugParticipant(Map<Participant, List<EncounterDetail>> map, String mapName) {
		logger.debug("Map:" + mapName + " size=" + map.size());
		
		StringBuffer logStr = new StringBuffer();
		for (Map.Entry<Participant, List<EncounterDetail>> pair : map.entrySet()) {
			Participant key = pair.getKey();
			List<EncounterDetail> details = map.get(key);
			logStr.append("Participant="+key+"(patno="+key.getPatientKey()+")");
			logStr.append(", EncounterDetails size="+details.size());
			for (EncounterDetail detail : details) {
				logStr.append(",[detail=" + detail + 
						", adm size="+detail.getAdmission().size() + 
						", appt size="+detail.getAppointment().size() + 
						", disch size="+detail.getDischarge().size() + "]");
			}
		}
	...
	private void saveEhrEncLog(Map<Participant, List<EncounterDetail>> map, String mode, Response msg) {
		...
		try {
			...
			for (Entry<Participant, List<EncounterDetail>> entry : map.entrySet()) {
            	...
            	for ( EncounterDetail ed : entry.getValue() ) {
            		...
            		for ( Discharge d : ed.getDischarge() ) {
            			...
            			ps.setString(34, null);
            			ps.setString(35, null);
            			ps.setString(36, id.getDischargeType() == null ? "null" : id.getDischargeType());
            			ps.addBatch();
            		}
            	}
            ...
            logger.error("saveEhrEncLog Error");
            e.printStackTrace();
        } finally {
        	try {
        		if (rs != null)
                ...
                logger.error("saveEhrEncLog Cannot close connection");
                e.printStackTrace();
        	}
        }		
	}
	...
	private class PatientInfo {
		...
		private String initEnctr;
		
		public PatientInfo(String patNo, String ehrNo, String initEnctr) {
			super();
			this.patNo = patNo;
			this.ehrNo = ehrNo;
			this.initEnctr = initEnctr;
		}
		
		public String getPatNo() {
			return patNo;
		}		
		public String getInitEnctr() {
			return initEnctr;
		}
